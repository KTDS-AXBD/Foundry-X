...
- **PRD**: [[FX-SPEC-PRD-V5]] (`docs/specs/prd-v5.md`) — v5: 통합 플랫폼 비전
- **Phase**: Phase 4 Conditional Go — Sprint 26~31 완료, 온보딩 데이터 대기
- **Sprint**: 31
...
| Sprint 38 | 🔧 진행 중 (F140 SecurityAgent + F141 QAAgent) — Agent Evolution Track A 역할 에이전트 |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 714테스트 ✅ (API 패키지) + CLI 112 + Web 48 + ~55 E2E |
| API endpoints | 118개 (28 routes) |
| API services | 60개 |
| D1 tables | 34개 (migrations 0001~0022) |
| D1 remote | 0001~0020 적용 완료 |
...
| Sprint 38 | SecurityAgent(F140) + QAAgent(F141) — Agent Evolution 역할 에이전트 | 🔧 |
...
- [ ] security-agent.ts — SecurityAgent 서비스 (scanVulnerabilities + analyzePRDiff + checkOWASPCompliance) (FX-REQ-140)
- [ ] security-agent-prompts.ts — 보안 분석 시스템 프롬프트 3종 (FX-REQ-140)
- [ ] qa-agent.ts — QAAgent 서비스 (runBrowserTest + validateAcceptanceCriteria + detectRegressions) (FX-REQ-141)
- [ ] qa-agent-prompts.ts — QA 테스트 시스템 프롬프트 3종 (FX-REQ-141)
- [ ] agent.ts routes — 4개 엔드포인트 추가 (security 2 + qa 2) (FX-REQ-140, FX-REQ-141)
- [ ] agent.ts schemas — Security + QA 요청/응답 스키마 (FX-REQ-140, FX-REQ-141)
- [ ] agent-orchestrator.ts — security-review→SecurityAgent, qa-testing→QAAgent 위임 (FX-REQ-140, FX-REQ-141)
- [ ] security-agent.test.ts — 15개+ 테스트 (FX-REQ-140)
- [ ] qa-agent.test.ts — 15개+ 테스트 (FX-REQ-141)
- [ ] typecheck + lint + 기존 테스트 회귀 0건
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F116 | KT DS SR 시나리오 구체화 — 첫 타겟 SR 유형 정의 + 워크플로우 설계 (FX-REQ-116, P1) | v3.0 | 📋 | PRD v5 Q4 |
...
| F140 | SecurityAgent — OWASP Top 10 보안 취약점 스캔 + PR diff 분석 (FX-REQ-140, P1) | Sprint 38 | 🔧 | Agent Evolution A6 |
| F141 | QAAgent 브라우저 테스트 — Playwright/Chromium 실제 UI 테스트 실행 (FX-REQ-141, P1) | Sprint 38 | 🔧 | Agent Evolution A7 |
...
| F143 | 모델 비용/품질 대시보드 — 에이전트별 토큰 사용량, 비용, 품질 점수 시각화 (FX-REQ-143, P1) | Sprint 35 | ✅ | ModelMetricsService+2 endpoints+D1 0021, Match 85% (UI 이관) |
| F144 | Fallback 체인 — 모델 응답 실패 시 자동 대체 모델 전환 (FX-REQ-144, P1) | v3.0 | 📋 | Agent Evolution A10 |
| F145 | InfraAgent — 샌드박스 환경에서 인프라 변경 시뮬레이션 + IaC 출력 (FX-REQ-145, P2) | v3.0+ | 📋 | Agent Evolution A11, Fluid 패턴 |
| F146 | 에이전트 역할 커스터마이징 — 사용자 정의 역할(프롬프트/도구/모델) (FX-REQ-146, P2) | v3.0+ | 📋 | Agent Evolution A12 |
| F147 | 멀티모델 앙상블 투표 — 3~5개 모델 병렬 처리 + 결과 종합 (FX-REQ-147, P2) | v3.0+ | 📋 | Agent Evolution A13, Six Hats 제안 |
| F148 | 에이전트 자기 평가 — 자기 반성 루프 (FX-REQ-148, P2) | v3.0+ | 📋 | Agent Evolution A14 |
| F149 | 프라이빗 프롬프트 게이트웨이 — 코드 요약/추상화만 LLM 전송, 보안 근본 차단 (FX-REQ-149, P1) | v3.0 | 📋 | Agent Evolution A15, Six Hats 핵심 대안 |
| F150 | AI-휴먼 하이브리드 피드백 루프 — 자동화 실패 시 즉시 피드백 수집+학습 (FX-REQ-150, P1) | v3.0 | 📋 | Agent Evolution A16, Six Hats 제안 |
| F151 | 자동화 품질 리포터 — 주간 자동화 품질 리포트 자율 생성 (FX-REQ-151, P2) | v3.0+ | 📋 | Agent Evolution A17, Six Hats 제안 |
| F152 | 에이전트 마켓플레이스 — 역할/프롬프트 공유 내부 마켓 (FX-REQ-152, P2) | v3.0+ | 📋 | Agent Evolution A18, Six Hats 제안 |
