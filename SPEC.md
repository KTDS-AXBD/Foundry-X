...
| F484 | 파이프라인 진행률 UI 개선 — 현재 단계 강조 + 상태 라벨 + pulse 애니메이션 (FX-REQ-476, P1) | Sprint 237 | 📋 | PipelineProgressStepper 리디자인 |
| F485 | 발굴 분석 결과 표시 + HITL 피드백 루프 — 완료 단계 결과 펼쳐보기 + 피드백 반영 재실행 (FX-REQ-477, P1) | Sprint 238 | 📋 | DiscoveryStageStepper 결과뷰 + 재실행 |
| F486 | 9기준 체크리스트 UX 정리 — 역할/가이드 명확화, AI 자동 평가 연동 (FX-REQ-478, P2) | Sprint 238 | 📋 | DiscoveryCriteriaPanel UX 개선 |
| F487 | 발굴 리포트 500 에러 수정 — ax_discovery_reports item_id→biz_item_id 컬럼 불일치 해소 (FX-REQ-479, P0) | Sprint 237 | 📋 | discovery-report-service.ts 컬럼명 수정 |
