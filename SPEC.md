...
| F542 | **MetaAgent 프롬프트 품질 개선 + 모델 전환 실험** — Phase 43 Dogfood에서 agent_improvement_proposals=0건 원인 해소. (a) `meta-agent.agent.yaml` systemPrompt 강화 — rawValue=0/low-signal 입력 명시적 지침 + few-shot 2~3건 + 출력 형식 재검증, (b) Haiku 4.5 → Sonnet 4.6 A/B 실험 — config flag `META_AGENT_MODEL` 런타임 전환 + 동일 DiagnosticReport에 대한 두 모델 결과 비교, (c) proposals 품질 rubric 점수화(재현성/실행가능성/근거명시). Dogfood P2(실측 산출물 ≠ 0건) 체크리스트 PASS 필수. 전제: Phase 43 F537 hotfix 완료(✅) (FX-REQ-571, P1) | — | 📋(idea) | Idea. Phase 44 병행 가능(MSA와 domain 독립 `core/agent/`). W+6+ 구체화. 근거: S276 Dogfood retrospective — proposals 빈 배열 원인 불명 상태에서 Phase 43 종료 |
