...
- **Phase**: **Phase 27** ✅ 완료 — BD Quality System F461~F470 (Sprint 225~230, 10/10 ✅) | **Phase 28** ✅ 완료 — Discovery 동기화 파이프라인 F478~F487 (Sprint 233~238, 10/10 ✅, 평균 Match ~98%)
- **Sprint**: 148~238 (✅)
...
> ```
> **마지막 실측** (Sprint 236, 2026-04-09): ~9 routes, ~40 services (adapters 13 포함), ~13 schemas, D1 0122, tests ~3262 (E2E 263) — Phase 20-21 MSA 재조정으로 core/modules가 gate-x·harness-kit으로 이전
...
| F488 | `/ax:req-manage new` `--create-issue` 기본화 (β: 스마트) + req-integrity 2카테고리 리포트 — gh/token 존재 시 자동 Issue 생성, 부재 시 `/tmp/req-issue-skip.log` 경고 기록. req-integrity는 *구조적 공백*(F100+ 미등록)과 *실시간 drift*(상태 불일치) 분리 집계 (FX-REQ-480, P0) | Sprint 240 | 📋 | `--no-issue` opt-out, 8곳 동기화(S185) 필수 |
| F489 | `/ax:gov-retro` 회고 통합 소급 등록 루틴 — Phase 완료 시 해당 Phase의 미등록 F-items 일괄 GitHub Issue 등록. Phase 27→26 역순 선별 전략, Issues 인플레이션 통제 (FX-REQ-481, P2) | — | 📋 | F488 선행 권장, 독립 트랙 |
