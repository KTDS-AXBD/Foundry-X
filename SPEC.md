...
- **Phase**: **Phase 11** ✅ 완료 (Sprint 113~121) — IA 대개편 12/12 완료 (F288~F299 전체 ✅)
- **Sprint**: 123 (완료) — BD 산출물 UX 연결성 (F301), PR #251
- **Phase 11 F-items**: F288~F299 ✅ (12건 전체 완료)
- **후속**: F300 E2E 정비 ✅ (Sprint 122) + F301 BD UX ✅ (Sprint 123)
...
| Sprint 124 | 예정 (F302) — E2E 상세 페이지 커버리지: 파라미터(:id) 8건 + mock factory + skip 재활성화 | 인프라/품질. F300 후속 |
| Sprint 129 | 예정 (F309) — Marker.io 비주얼 피드백 통합: 위젯 + GitHub Issues 연동 + 팀 온보딩 | 인프라/DX. FX-PLAN-013 Phase A |
| Sprint 130 | 예정 (F310) — TinaCMS 호환성 PoC: Vite+RR7 충돌 검증 + E2E 회귀 확인 | 인프라/DX. FX-PLAN-013 Phase B PoC |
| Sprint 131 | 예정 (F311) — TinaCMS 인라인 에디팅 본구현: config + 콘텐츠 구조 + useTina + TinaCloud | 인프라/DX. FX-PLAN-013 Phase B. F310 선행 |
...
> ```
> **마지막 실측** (Sprint 123, 2026-04-04): ~85 routes, ~195 services, ~100 schemas, D1 0088, tests ~3061
...
| F302 | E2E 상세 페이지 커버리지 확장 — 파라미터(:id) 상세 페이지 8건 E2E 추가 + mock factory 패턴 도입 + skip 6건 재활성화 검토 (FX-REQ-294, P2) | Sprint 124 | 🔧 | F300 후속. 대상: bdp/:id, bmc/:id, ideas/:id, sr/:id, discovery/items/:id, outreach/:id, offering/:id, offering/:id/brief |
...
| F# | 제목 (REQ, Priority) | Sprint | 상태 | 비고 |
|----|----------------------|:------:|:----:|------|
| F303 | SkillCatalog API 전환 — Web bd-skills.ts → skill_registry API 호출 + 실시간 검색/필터/메트릭 표시 (FX-REQ-295, P0) | Sprint 125 | 📋 | D1 해소. api-client 메서드 추가 + SkillCatalog/ProcessGuide 전환. **배치 1**, F304와 병렬 가능 (Web↔API 분리) |
| F304 | 벌크 레지스트리 API + sf-scan 연동 — POST /api/skills/registry/bulk + sf-scan --api-register 옵션 (FX-REQ-296, P0) | Sprint 125 | 📋 | D2 해소. upsert 로직 + 210+ 스킬 벌크 등록. **배치 1**, F303과 병렬 가능 |
| F305 | 스킬 실행 메트릭 수집 — usage-tracker.sh → POST /api/skills/metrics/record + CC 훅 연동 (FX-REQ-297, P0) | Sprint 126 | 📋 | D4 해소. 기존 recordExecution() 활용 + hook API 호출 추가. **배치 2** |
| F306 | DERIVED/CAPTURED → SKILL.md 자동 생성 + Deploy API + marketplace 등록 플로우 (FX-REQ-298, P0) | Sprint 127 | 📋 | D3 해소. 승인 시 SKILL.md 생성 + 다운로드/Git push. **배치 2**, F305와 병렬 가능 (다른 테이블) |
| F307 | SkillEnrichedView 대시보드 + 진화 계보 시각화 — registry+metrics+lineage 통합 뷰 (FX-REQ-299, P1) | Sprint 128 | 📋 | 기존 getEnriched() API 활용 + lineage 트리. **배치 3**, F303~F306 선행 |
| F308 | Skill Unification 통합 QA + 데모 데이터 + Production 배포 (FX-REQ-300, P1) | Sprint 128 | 📋 | E2E + 벌크 등록 검증 + demo seed. **배치 3**, F303~F306 선행 |
...
| F# | 설명 | Sprint | 상태 | 비고 |
|----|------|:------:|:----:|------|
| F309 | Marker.io 비주얼 피드백 통합 — MarkerWidget.tsx + VITE_MARKER_PROJECT_ID 환경변수 + GitHub Issues 연동(visual-feedback 라벨) + 팀 온보딩 가이드 (FX-REQ-301, P1) | Sprint 129 | 📋 | Phase A. ~4h. 계정: ktds.axbd@gmail.com, 14일 Trial. CI/CD 자동 배포 |
| F310 | TinaCMS 호환성 PoC — feat/tinacms-poc 브랜치에서 Vite 8 + React Router 7 충돌 검증 + /admin 라우팅 + pnpm build/typecheck/e2e 전체 통과 확인 (FX-REQ-302, P2) | Sprint 130 | 📋 | Phase B PoC. ~4h. Go/No-Go 게이트: E2E 전체 통과 시 F311 진행, 실패 시 Phase B 연기 |
| F311 | TinaCMS 인라인 에디팅 본구현 — tina/config.ts + content/ 디렉터리 구조화 + useTina hook 컴포넌트 연결 + /admin 3중 방어(_redirects+RR7+Vite) + TinaCloud 권한 설정 + CF Pages 배포 (FX-REQ-303, P3) | Sprint 131 | 📋 | Phase B 본구현. ~16h. F310 PoC 통과 선행. TinaCloud Free(2users). Phase B 트리거: §1.3 참조 |
