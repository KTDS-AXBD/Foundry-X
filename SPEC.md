...
| F549 | **fx-ai-foundry-os: KG XAI 뷰어 (read-only)** — `/ai-foundry-os/ontology` — Decode-X Neo4j 노드(SubProcess/Method/Condition/Actor/Requirement/DiagnosisFinding)를 D3 force-graph로 표시, 클릭 시 "이 결정의 근거 경로" 자동 하이라이트. Neo4j 조회 엔드포인트 없으면 mock 데이터 10+ 노드로 fallback. PRD: `docs/specs/fx-ai-foundry-os/prd-final.md` (FX-REQ-585, P0) | Sprint 298 | ✅ | Open Issue #4 (Neo4j 접근 경로) H+4 마감. 구현 복잡도 높음 → M4 버퍼 필요 |
...
| B3 | B | Dashboard 파이프라인 원 안 숫자 모호성 해소 (FX-REQ-586) | — | PLANNED | task orchestrator |
<!-- /fx-task-orchestrator-backlog -->
