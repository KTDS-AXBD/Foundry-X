...
| F175 | 사업 아이템 분류 Agent — Type A/B/C 자연어 3턴 대화 분류 + 유형별 분석 경로 제안 (FX-REQ-175, P0) | TBD | 📋 | Phase 5b 사업개발 프로세스 자동화, [[FX-SPEC-BDP-001]] |
| F176 | 유형별 분석 파이프라인 — 2-1~2-7 단계별 HITL 워크플로우 (레퍼런스→시장→경쟁→도출→선정→고객→BM) (FX-REQ-176, P0) | TBD | 📋 | Discovery-X 핵심, 7단계 분석 경로 |
| F177 | 발굴 결과 패키징 — Discovery Output Report 자동 생성 + GTM 초안 + Beachhead 확인 (FX-REQ-177, P1) | TBD | 📋 | 2-8 형상화 Input Deck |
| F178 | AI 멀티 페르소나 사전 평가 — KT DS 8개 역할 에이전트 자동 평가 + 레이더 차트 + G/K/R 판정 (FX-REQ-178, P0) | TBD | 📋 | 2-9 사전 검증, 기존 6종 에이전트 아키텍처 확장 |
