...
- **Phase**: **Phase 21** ✅ 완료 — Gate-X 독립 서비스 F402~F413 | **Phase 22** ✅ 완료 — Offering Skill v2 + Builder v2 F414~F431 | **Phase 23** ✅ 완료 — Sprint Automation v2 F432~F433 (Match 98%) | **Phase 24** ✅ 완료 — Discovery Native F434~F440 (Match 97%) | **Phase 25** 📋 계획 — Discovery Pipeline v2 F441~F450 (Sprint 213~218, 10건)
- **Sprint**: 148~164 (✅)
...
> ```
> **마지막 실측** (Sprint 212, 2026-04-08): ~9 routes, ~37 services (adapters 10 포함), ~13 schemas, D1 0118, tests ~3168 (E2E 263) — Phase 20-21 MSA 재조정으로 core/modules가 gate-x·harness-kit으로 이전
...
| F441 | 파일 업로드 인프라 — R2 Presigned URL + 멀티 파일 업로드 UI + 파일 메타 D1 테이블 (FX-REQ-433, P0) | Sprint 213 | 🔧 | R2 버킷 + presigned URL 서명 + 업로드 UI 컴포넌트 |
| F442 | 문서 파싱 엔진 — PDF/PPT/DOCX → 텍스트 추출 + 구조화 (FX-REQ-434, P0) | Sprint 213 | 🔧 | Workers AI 또는 외부 파싱 API. 병렬: F441 |
| F443 | 자료 기반 발굴 입력 — 업로드 문서 파싱→아이템 등록 + 분석 컨텍스트 자동 주입 (FX-REQ-435, P0) | Sprint 214 | 🔧 | F441+F442 선행. 온보딩 위저드 확장 |
| **Phase 25-B: 생성 고도화 (M2, P0)** | | | | |
| F444 | 사업기획서 편집기 — 섹션별 인라인 편집 + AI 재생성 + 버전 이력 (FX-REQ-436, P0) | Sprint 215 | 🔧 | 기존 생성 결과에 편집 레이어 추가. F440 기반 |
| F445 | 기획서 템플릿 다양화 — 용도별 3종(내부보고/제안서/IR피치) + 톤/분량 커스텀 (FX-REQ-437, P0) | Sprint 215 | 🔧 | 템플릿 선택 UI + 생성 파라미터. 병렬: F444 |
| F446 | 내보내기 강화 — 사업기획서 PDF/PPTX 내보내기 + 디자인 토큰 적용 (FX-REQ-438, P0) | Sprint 216 | 🔧 | F444 선행. 기존 offering-export 패턴 활용 |
| **Phase 25-C: E2E 흐름 보완 (M3, P1)** | | | | |
| F447 | 파이프라인 상태 추적 — 아이템별 온보딩→발굴→형상화→Offering 전체 진행률 시각화 (FX-REQ-439, P1) | Sprint 217 | 🔧 | 스테퍼/타임라인 UI + 상태 집계 API |
| F448 | 단계 간 자동 전환 — 발굴 완료→형상화/기획서 자동 제안 + 원클릭 진행 (FX-REQ-440, P1) | Sprint 217 | 🔧 | 파이프라인 이벤트 + CTA 버튼. 병렬: F447 |
| **Phase 25-D: 운영 품질 (M4, P1)** | | | | |
| F449 | 에러/로딩 UX — API 실패 재시도 UI + 스켈레톤 로딩 + 빈 상태 안내 (FX-REQ-441, P1) | Sprint 218 | 🔧 | 공통 ErrorBoundary + LoadingSkeleton 컴포넌트 |
