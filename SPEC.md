...
> ```
> **마지막 실측** (Sprint 240, 2026-04-09): ~9 routes, ~40 services (adapters 13 포함), ~13 schemas, D1 0122, tests ~3262 (E2E 263) — Phase 20-21 MSA 재조정으로 core/modules가 gate-x·harness-kit으로 이전
...
| F494 | 발굴 파이프라인 단계 전진 구조 버그 — `confirmStage`가 9/9 완료돼도 `pipeline_stages`를 DISCOVERY→FORMALIZATION으로 전진시키지 않음. `REGISTERED→DISCOVERY` 전환은 `biz-items.ts:327` 한 곳만 존재, 후속 전환 코드 부재. 영향: `PipelineProgressStepper`가 영원히 REGISTERED(0% 또는 25%)에 고정, `biz_items.status='evaluated'`와 `pipeline_stages='REGISTERED'` 영구 불일치. 실제 사례: bi-koami-001 세션 #247 DEBUG 제보 — `biz_item_discovery_stages` 2-1~2-9 모두 completed + `biz_items.status='evaluated'`인데도 pipeline 진행률 미매칭, 체크리스트 9건 pending, `bd_artifacts` 0건으로 분석결과 상세 보기 불가. 수정: (1) `stage-runner-service.ts::confirmStage`에서 모든 9단계 완료 + `gateStatus==='ready'` 시 `pipeline_stages` DISCOVERY → FORMALIZATION 자동 advance (2) `DiscoveryReportView` 빈 `bd_artifacts` fallback — "재실행 필요" CTA + 재분석 버튼 (3) bi-koami-001 데이터 복구 SQL (pipeline advance + criteria completed) (FX-REQ-486, P1) | — | 🔧 | 세션 #247 debug 트랙. Test 모드 직접 master push. 테스트: confirmStage 9/9 전진 + REGISTERED→FORMALIZATION 멱등성 검증 |
