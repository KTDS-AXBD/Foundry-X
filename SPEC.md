...
| F432 | Sprint Pipeline 종단 자동화 — Phase 6(Gap Analyze 집계) + Phase 7(Auto Iterator) + Phase 8(Session-End) 추가 (FX-REQ-424, P0) | Sprint 243 | 📋 | sprint-pipeline 스킬 확장. 전체 Sprint 완료 후 gap 집계→iterate→session-end 자동 실행 |
| F433 | Sprint Monitor 고도화 — Pipeline 전체 Phase(6~8) 진행률 Gist 표시 + Monitor 생존 감시 + 자동 재시작 (FX-REQ-425, P0) | Sprint 243 | 📋 | sprint-watch 스킬 확장. 병렬: F432와 동일 Sprint |
...
| C10 | C | CC 다중 구독 계정 환경 통합 — HOME 경로 정규화 + 스킬/플러그인/standards/rules 공유 체계 구축 (FX-REQ-493, P1) | — | 📋 | 별도 세션 계획. 회사(ktds.axbd)+개인(sinclairseo) 2계정, 향후 추가 가능 |
| C11 | C | Sprint 자동 Monitor+Merge 파이프라인 — Sprint WT 생성 후 Master에서 signal 감시→권한승인→review→merge→cleanup 자동 실행 체인 구축. 현재: merge-monitor.sh/sprint-watch 존재하나 자동 트리거 부재. 해결: (A) /loop Signal 폴링 (B) sprint-pipeline Phase 4c 강제 수행 (C) Hook 기반 자동 트리거 (FX-REQ-494, P1) | — | 📋 | C10과 독립. sprint-merge-monitor.sh + sprint-watch + sprint-pipeline Phase 4c~8 연결 |
<!-- /fx-task-orchestrator-backlog -->
