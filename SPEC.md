...
- **Sprint**: 85 (계획) — Next.js → Vite 전환 (F246~F247)
...
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **2119** + CLI **149** + Web **172** = **2440** + E2E ~98 (27 specs) |
| API endpoints | **~345개** (63 routes) |
| API services | **153개** |
| API schemas | **69개** |
| D1 migrations | **0001~0065** |
| D1 remote | 0001~0065 전체 적용 완료 |
| Workers | foundry-x-api.ktds-axbd.workers.dev ✅ Sprint 82~84 (f2004d43) |
| Pages | fx.minu.best ✅ |
...
| F246 | Next.js → Vite + React Router 전환 — 빌드 인프라 교체. 빌드 메모리 750MB→<300MB, 빌드 시간 22초→<10초. SWC 바이너리 276MB 제거, next/link·navigation 훅 42곳 치환, 환경변수 NEXT_PUBLIC→VITE 9파일 (FX-REQ-238, P1) | Sprint 85 | 📋 | [[FX-PLAN-013]], [[FX-DSGN-013]] |
| F247 | Vite 전환 검증 + 배포 — unit 172 + E2E 59 전량 통과, Cloudflare Pages dist/ 배포, 빌드 벤치마크 DoD 달성 (FX-REQ-239, P1) | Sprint 85 | 📋 | F246 선행 |
