...
| F61 | MCP 실 구현 — McpAgentRunner + SseTransport (FX-REQ-061, P1) | v0.12 | 📋 | F58 설계 기반, tool.call() + 연결 관리 |
| F62 | v1.0.0 릴리스 준비 — 안정화 + 문서 + 배포 (FX-REQ-062, P1) | v0.12 | 📋 | CHANGELOG 정비, README, 프로덕션 최종 배포 |
| F63 | 테스트 커버리지 강화 — E2E + API 통합 보강 (FX-REQ-063, P2) | v0.12 | 📋 | F56(88%) 보강, agents 컴포넌트 테스트 |
...
- [ ] MCP SseTransport 구현 — EventSource 기반 MCP 서버 연결 (FX-REQ-061)
- [ ] McpAgentRunner 구현 — taskType → MCP tool.call() 변환 + 결과 파싱 (FX-REQ-061)
- [ ] MCP 서버 연결 설정 UI — 대시보드 설정 페이지 (FX-REQ-061)
- [ ] CHANGELOG v0.12.0 + README 업데이트 + 온보딩 가이드 갱신 (FX-REQ-062)
- [ ] 프로덕션 최종 배포 + smoke test + v1.0.0 태그 준비 (FX-REQ-062)
- [ ] agents/page E2E 보강 — SSE 실시간 + 위젯 인터랙션 흐름 (FX-REQ-063)
- [ ] API 통합 테스트 확대 — MCP runner + 서비스 간 호출 검증 (FX-REQ-063)
