...
- **Phase**: Phase 2 (API + Web + 에이전트 오케스트레이션) — Sprint 12 완료
- **Version**: 0.12.0
...
| Sprint 13 | 📋 계획 (F64 MCP Sampling/Prompts + F65 에이전트 자동 PR + F66 v1.1.0) |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 352테스트 ✅ (CLI 106 + API 201 + Web 45) + 20 E2E |
...
| v1.1.0 | Sprint 13: MCP Sampling/Prompts + 에이전트 자동 PR 파이프라인 (41 endpoints, ~407 tests) | 📋 |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F64 | MCP Sampling + Prompts 확장 — 양방향 MCP 통합 완성 (FX-REQ-064, P1) | v1.1 | 📋 | SamplingHandler + PromptsClient + 4 endpoints + UI 브라우저 |
| F65 | 에이전트 자동 PR 파이프라인 — branch→PR→review→merge 전체 자동화 (FX-REQ-065, P0) | v1.1 | 📋 | PrPipelineService + ReviewerAgent + auto-merge + SDD 검증 |
| F66 | v1.1.0 릴리스 + 안정화 (FX-REQ-066, P2) | v1.1 | 📋 | D1 migration + 배포 + E2E + CHANGELOG |
...
- [ ] MCP SamplingHandler — 서버→클라이언트 LLM 호출 위임 (FX-REQ-064)
- [ ] MCP PromptsClient — prompts/list + prompts/get (FX-REQ-064)
- [ ] MCP API 4 endpoints + Prompts 브라우저 UI (FX-REQ-064)
- [ ] PrPipelineService — branch→commit→PR→merge 전체 자동화 (FX-REQ-065)
- [ ] ReviewerAgent — cross-agent PR 리뷰 (FX-REQ-065)
- [ ] GitHubService 확장 — createBranch, createPR, mergePR (FX-REQ-065)
- [ ] Auto-merge 판정 — SDD Score + CI + Security 게이트 (FX-REQ-065)
- [ ] SSE agent.pr.* 이벤트 4종 + 대시보드 PR 상태 UI (FX-REQ-065)
- [ ] D1 migration 0007 — agent_prs + mcp_sampling_log (FX-REQ-064, FX-REQ-065)
- [ ] CHANGELOG v1.1.0 + version bump + 프로덕션 배포 (FX-REQ-066)
- [ ] E2E — PR 파이프라인 3건 + MCP Prompts 2건 (FX-REQ-066)
- [ ] typecheck + build + tests ~407건 통과 + 25 E2E specs
