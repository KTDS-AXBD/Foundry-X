...
> ```
> **마지막 실측** (Sprint 256, 2026-04-11): ~9 routes, ~40 services (adapters 13 포함), ~13 schemas, D1 0125, tests ~3447 (E2E 263) — 수치 변동 없음 (S256은 task orchestrator + cross-repo docs 위주, packages/api 미변경)
...
| C20 | C | task-complete.sh meta-only empty commit 감지 + signal 거부 (FX-REQ-514) | — | 📋 | S258 후속 defense-in-depth. tmux 3.5a로 pane 소멸 해결됐지만 다른 원인(프롬프트 오해/권한/네트워크)으로 worker 무활동 재발 가능. 개선: task-complete.sh가 `git diff --numstat HEAD~1 HEAD` 결과 additions=0·deletions=0이면 (a) PR 생성 거부 (b) signal 미작성 (c) task 상태 rollback + 오류 로그. C14~C17 형태의 빈 PR 재발 차단 |
| C21 | C | phase_recover worker inactivity → retry queue (FX-REQ-515) | — | 📋 | S258 후속 defense-in-depth. daemon phase_recover가 worker pane 소멸 감지 시 현재는 바로 meta-only commit+PR 생성. 개선: recover 전 `git -C $WT diff --numstat` additions 확인 → 0이면 (a) recover 중단 (b) `/tmp/task-retry/{TASK}.json` 재시도 큐 등록 (c) Master pane StatusLine/Monitor 알림 (d) 사용자 승인 후 task-start 재호출. C20(task-complete 측) + C21(daemon 측) 2중 방어 |
| C22 | C | task-complete.sh empty commit 감지 + signal 거부 (FX-REQ-516) | — | PLANNED | task orchestrator |
<!-- /fx-task-orchestrator-backlog -->
