...
| Sprint 132 | 예정 (F312+F313) — 형상화 자동 전환 + 상태 머신/실패 관리 | fx-discovery-v2 M1. F312+F313 병렬 |
| Sprint 133 | 예정 (F314) — 발굴 연속 스킬 파이프라인 + HITL 체크포인트 | fx-discovery-v2 M2. F312+F313 선행 |
| Sprint 134 | 예정 (F315) — 상태 모니터링 + 알림 + 권한 제어 | fx-discovery-v2 M2. F314 선행 |
| Sprint 135 | 예정 (F316) — Discovery E2E 테스트 | fx-discovery-v2 M3. F314 선행 |
| Sprint 136 | 예정 (F317) — 데이터 백업/복구 + 운영 계획 | fx-discovery-v2 M3. F315 선행 |
...
| F311 | TinaCMS 인라인 에디팅 본구현 — tina/config.ts + content/ 디렉터리 구조화 + useTina hook 컴포넌트 연결 + /admin 3중 방어(_redirects+RR7+Vite) + TinaCloud 권한 설정 + CF Pages 배포 (FX-REQ-303, P3) | Sprint 131 | ✅ | PR #264, Match 100%. landing+wiki 2 collection. content-loader.ts fallback 패턴. TinaCloud 수동 설정 대기 |
| F312 | 형상화 자동 전환 + Phase A~F 자동 실행 — 2-10 최종보고서 완료 시 형상화 Phase A~F 자동 파이프라인 트리거 + 발굴 산출물 전달 (FX-REQ-304, P0) | Sprint 132 | 🔧 | PRD: fx-discovery-v2/prd-final.md. M1 핵심 |
| F313 | 파이프라인 상태 머신 + 실패/예외 관리 — 이벤트 기반 상태 머신 오케스트레이션 + 재시도/건너뛰기/중단 옵션 + 에러 표준화 (FX-REQ-305, P0) | Sprint 132 | 🔧 | F312 병렬. 상태 저장 D1 스키마 설계 포함 |
| F314 | 발굴 연속 스킬 파이프라인 + HITL 체크포인트 — 2-0~2-10 자동 순차 실행 + 사업성 체크포인트(Commit Gate) 사용자 승인 UI (FX-REQ-306, P1) | Sprint 133 | 📋 | F312+F313 선행 |
| F315 | 상태 모니터링 + 알림 + 권한 제어 — 파이프라인 진행 대시보드 + 실시간 알림 + HITL 승인 권한 관리 (FX-REQ-307, P1) | Sprint 134 | 📋 | F314 선행 |
| F316 | Discovery E2E 테스트 — 위저드 전체 흐름 + 아이템 상세 + 스킬 실행 + HITL 리뷰 E2E (FX-REQ-308, P2) | Sprint 135 | 📋 | F314 선행. Playwright |
| F317 | 데이터 백업/복구 + 운영 계획 — 산출물 백업 + 복구 시나리오 + 운영 정책 + Hotfix 체계 (FX-REQ-309, P2) | Sprint 136 | 📋 | F315 선행 |
