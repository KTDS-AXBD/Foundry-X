...
- **Phase**: **Phase 8** — IA 구조 개선 + 신규 사업 Prototyping (Sprint 82~)
- **Sprint**: 86 (완료) — 인증 가드 + E2E 보강 (F248~F250)
...
| Sprint 87 | 🔧 진행 중 (F251+F252) — 팀 계정 일괄 생성 + 온보딩 가이드 고도화 |
| Sprint 88 | 📋 계획 (F253+F254) — 팀 데이터 공유(Org-scope) + NPS 피드백 수집 |
| Sprint 89 | 📋 계획 (F258+F259) — BD 프로세스 가이드 UI + 스킬 카탈로그 UI |
| Sprint 90 | 📋 계획 (F260+F261) — BD 스킬 실행 엔진 + 산출물 저장·버전 관리 |
| Sprint 91 | 📋 계획 (F262) — BD 프로세스 진행 추적 + 사업성 신호등 |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **2119** + CLI **149** + Web **207** = **2475** + E2E (25 specs) |
| API endpoints | **~345개** (63 routes) |
| API services | **153개** |
| API schemas | **78개** |
| D1 migrations | **0001~0074** |
| D1 remote | 0001~0074 전체 적용 완료 |
| Workers | foundry-x-api.ktds-axbd.workers.dev ✅ Sprint 85 (42e92ca9, wrangler 4.78) |
| Pages | fx.minu.best ✅ |
...
| Sprint 89 | Phase 9: GIVC Ontology PoC 1차(F255) | 📋 |
| Sprint 90 | Phase 9: GIVC PoC 2차(F256) + 추가 BD 아이템(F257) | 📋 |
...
| F251 | 팀 계정 일괄 생성 + Org 초대 — AX BD팀 7명 + 공용계정 1개 = 8계정. Signup API 활용 + Org 초대 자동화 스크립트. 역할 배정(admin/member) (FX-REQ-243, P0) | Sprint 87 | 🔧 | Phase 9a 온보딩 기반 |
| F252 | 온보딩 가이드 + 사용법 투어 고도화 — F172 인터랙티브 온보딩 투어 + F242 ProcessStageGuide를 팀 실사용 수준으로 보완. 역할별 시작 경로 안내 (FX-REQ-244, P1) | Sprint 87 | 🔧 | F172/F242 기반 고도화 |
| F253 | 팀 데이터 공유 — 아이디어/BMC/인사이트/Discovery 산출물을 Org 기준 팀 전체 조회. 기존 tenant 격리 위에 Org-scope 공유 뷰 추가 (FX-REQ-245, P0) | Sprint 88 | 📋 | F197~F204 BMC + F201 인사이트 기반 |
| F254 | 팀 피드백 수집 + NPS — F174 인앱 피드백 위젯 + 주간 NPS 서베이 자동 트리거. 피드백 대시보드에 팀별 집계 뷰 추가 (FX-REQ-246, P1) | Sprint 88 | 📋 | F174 기반, KPI K1 측정 |
| F255 | GIVC Ontology PoC 1차 — 산업 공급망 지식그래프 스키마 설계 + 샘플 데이터 로드 + 기본 질의 API (FX-REQ-247, P2) | Sprint 89 | 📋 | F245 구체화. KG 스키마 + D1 or 외부 DB |
| F256 | GIVC PoC 2차 — 4대 시나리오(이벤트 연쇄/대체 공급처/EWS 영향/리스크맵) 중 1개 MVP 구현 (FX-REQ-248, P2) | Sprint 90 | 📋 | F255 선행 |
| F257 | 추가 BD 아이템 탐색 — Discovery 파이프라인으로 신규 사업 아이템 1~2건 발굴 + 사업성 체크포인트 통과 (FX-REQ-249, P2) | Sprint 90 | 📋 | BD Pipeline(F232~F240) 실사용 |
| **BD 스킬 통합 — AX BD 프로세스 v8.2 + 공용 스킬 체계를 Foundry-X 웹에 풀스택 반영** | | | | 참조: docs/specs/axbd-skill/ |
| F258 | BD 프로세스 가이드 UI — 2-0~2-10 단계별 프로세스 설명 + 유형별(I/M/P/T/S) 강도 매핑 테이블 + 사업성 체크포인트 가이드. /ax-bd/discovery 페이지 확장 (FX-REQ-250, P0) | Sprint 89 | 📋 | 병렬 가능. axbd-skill/01_AX사업개발_프로세스설명.html 기반 |
| F259 | BD 스킬 카탈로그 UI — 76개 스킬 + 36개 커맨드 검색·필터·카테고리 뷰. 단계별 추천 스킬 하이라이트 + 스킬 상세(설명/입력/산출물) 팝업 (FX-REQ-251, P0) | Sprint 89 | 📋 | 병렬 가능. CLAUDE_AXBD/.claude/skills/ + commands/ 기반 |
| F260 | BD 스킬 실행 엔진 — 웹에서 스킬 선택 → API 호출 → Anthropic LLM 실행 → 산출물 반환. skill-execution.md 프레임워크 정의를 서버 측 프롬프트로 변환 (FX-REQ-252, P0) | Sprint 90 | 📋 | F259 선행. F149 프롬프트 게이트웨이 + F212 ai-biz 스킬 기반 |
| F261 | BD 산출물 저장 + 버전 관리 — 스킬 실행 결과를 biz-item별 산출물로 D1 저장 + 버전 히스토리. 2-0~2-10 단계별 산출물 연결 (FX-REQ-253, P0) | Sprint 90 | 📋 | F260 선행. F234 BDP 버전관리 패턴 활용. D1 마이그레이션 필요 |
| F262 | BD 프로세스 진행 추적 — biz-item별 현재 단계(2-0~2-10) + 사업성 신호등(Go/Pivot/Drop 누적) + Commit Gate 상태. 파이프라인 대시보드 연동 (FX-REQ-254, P0) | Sprint 91 | 📋 | F258+F261 선행. F232 파이프라인 대시보드 + F239 의사결정 워크플로 기반 |
