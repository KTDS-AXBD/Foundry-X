...
- **Phase**: **Phase 14** ✅ 완료 | **Phase 15** ✅ 완료 | **Phase 16** ✅ 완료 | **Phase 17** 📋 등록 — Self-Evolving Harness v2 F357~F362 (Sprint 161~164, 0/6)
- **Sprint**: 148~160 (✅)
...
> ```
> **마지막 실측** (Sprint 160, 2026-04-06): ~103 routes, ~229 services, ~119 schemas, D1 0106, tests ~3155 (9 fail feedback-queue 기존)
...
| F357 | 데이터 상태 진단 + 기준선 수립 — execution_events/task_histories 데이터 양·기간·품질 진단. 반복 실패 패턴 추출 가능성 확인. 기준선 보고서 생성 (FX-REQ-349, P0) | Sprint 161 | 📋 | — |
| F358 | 반복 실패 패턴 감지 + Rule 초안 생성 — PatternDetector(event_type × error_category × task_family 클러스터링) + RuleGenerator(LLM 기반 자연어 Rule 초안, 패턴 근거 주석 포함) (FX-REQ-350, P0) | Sprint 161 | 📋 | F357 선행 |
| **Sprint 162 — 승인 플로우 + Rule 배치** | | | | |
| F359 | 세션 내 Rule 승인 플로우 — /ax:session-start 통합. "새 Rule 제안 N건" 알림 → 근거·내용 표시 → 승인/거부/수정 → .claude/rules/ 자동 배치 + 감사 로그 (FX-REQ-351, P0) | Sprint 162 | 📋 | F358 선행 |
| **Sprint 163 — O-G-D Loop 범용화** | | | | |
| F360 | O-G-D Loop 범용 인터페이스 + 어댑터 레지스트리 — OGDRequest/OGDResult 도��인 독립 API. BD형상화·코드리뷰·문서검증·Prototype 4개 도메인 어댑터. 어댑터 등록/조회 API (FX-REQ-352, P0) | Sprint 163 | 📋 | F355 기반 |
| **Sprint 164 — 운영 지표 + 효과 측정** | | | | |
| F361 | 에이전트 자기 평가 연동 + Rule 효과 측정 — F148 데이터를 PatternDetector 가중치로 ��용. Rule 배치 전/후 동일 유형 실패 빈도 비교. Rule별 효과 점수 (FX-REQ-353, P1) | Sprint 164 | 📋 | F359 선행 |
| F362 | 운영 지표 대시보드 — DERIVED/CAPTURED 스킬 재사용률(skill_executions×skill_lineage JOIN) + 에이전���/스킬 월간 활용률 집계 + 미사용 항목 하이라이트. F337 Dashboard 탭 추가 (FX-REQ-354, P1) | Sprint 164 | 📋 | — |
