...
- **Phase**: Phase 2 (API + Web + 에이전트 오케스트레이션) — Sprint 15 완료
- **Version**: 1.3.0
...
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 313테스트 ✅ (API 패키지 313건 pass) + CLI 106 + Web 45 + 20 E2E |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F80 | AI Foundry MCP 연동 — 설계 문서 + 서비스 등록 흐름 + 외부 MCP 호출 경로 (FX-REQ-080, P1) | v1.5 | 📋 | PoC 완료(48b5bee), mcp-runner/registry 활용, 설계 문서 + 등록 API |
| F81 | AgentInboxPanel 스레드 뷰 — parentMessageId 기반 대화 맥락 UI + 스레드 라우트 (FX-REQ-081, P1) | v1.5 | 📋 | DB parent_message_id + getThread() 존재, 라우트 노출 + UI 구현 |
| F82 | PlannerAgent → Orchestrator 실 연동 — createPlanAndWait 승인 대기 + executePlan 라이프사이클 (FX-REQ-082, P1) | v1.5 | 📋 | LLM 분석 완성, 승인 폴링 + 실행 상태 추적 + D1 컬럼 확장 |

### 단독 작업 — Production E2E + AXIS Design System 리디자인

| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F78 | Production 사이트 E2E 테스트 — fx.minu.best smoke + 크리티컬 패스 검증 (FX-REQ-078, P1) | — | 📋 | 프로덕션 health check + 랜딩→대시보드→에이전트 핵심 흐름 E2E |
| F79 | UI/UX 전면 리디자인 — AXIS Design System 연동 (FX-REQ-079, P1) | — | 📋 | 랜딩+대시보드 전체를 AXIS DS 기반으로 재설계, shadcn/ui 대체 |
...
- [ ] multitenancy.design.md + phase-3-roadmap.md 작성 (FX-REQ-069 — Sprint 15+)
...
- [ ] AI Foundry MCP 연동 설계 문서 작성 — 서비스 등록 흐름 + 인증 전략 (FX-REQ-080)
- [ ] MCP 서버 등록 API 확장 — AI Foundry 전용 프리셋 + 연결 검증 (FX-REQ-080)
- [ ] PlannerAgent 외부 MCP 호출 경로 — AI Foundry tool.call() 통합 (FX-REQ-080)
- [ ] AgentInbox 스레드 라우트 — GET /agents/inbox/:parentMessageId/thread (FX-REQ-081)
- [ ] AgentInboxPanel 스레드 UI — 대화 맥락 그룹핑 + 토글 뷰 (FX-REQ-081)
- [ ] api-client getInboxThread() 함수 추가 (FX-REQ-081)
- [ ] createPlanAndWait() 승인 대기 메커니즘 — 폴링 또는 SSE 구독 (FX-REQ-082)
- [ ] executePlan() 라이프사이클 — executing/completed/failed 상태 추적 (FX-REQ-082)
- [ ] agent_plans 테이블 확장 — execution_* 컬럼 추가 (D1 migration 0010) (FX-REQ-082)
- [ ] Plan 관리 API 4 endpoints — approve/reject/execute/get (FX-REQ-082)
- [ ] typecheck + build + tests 통과

### 단독: F78 Production 사이트 E2E 테스트
- [ ] Playwright 프로덕션 config — baseURL: fx.minu.best (FX-REQ-078)
- [ ] smoke test E2E — health check + 랜딩 페이지 로딩 검증 (FX-REQ-078)
- [ ] 크리티컬 패스 E2E — 랜딩→대시보드→에이전트→spec-generator 흐름 (FX-REQ-078)
- [ ] CI 연동 — 배포 후 프로덕션 E2E 자동 실행 (FX-REQ-078)

### 단독: F79 UI/UX 전면 리디자인 — AXIS Design System 연동
- [ ] AXIS DS 토큰 체계 분석 + Foundry-X tailwind 매핑 (FX-REQ-079)
- [ ] 랜딩 페이지 AXIS DS 기반 리디자인 (FX-REQ-079)
- [ ] 대시보드 레이아웃 + 컴포넌트 AXIS DS 전환 (FX-REQ-079)
- [ ] shadcn/ui → AXIS DS 컴포넌트 마이그레이션 (FX-REQ-079)
