...
> ```
> **마지막 실측** (Sprint 241, 2026-04-10): ~9 routes, ~40 services (adapters 13 포함), ~13 schemas, D1 0125, tests ~3447 (E2E 263) — Phase 20-21 MSA 재조정으로 core/modules가 gate-x·harness-kit으로 이전
...
| F490 | E2E workflow shard 병렬화 — `.github/workflows/e2e.yml` Playwright shard matrix(3~4) + `timeout-minutes` 상향(15→30). PR #394에서 동일 runner 2회 연속 15m17s timeout 관측, 테스트 suite 증가로 단일 job 한계 초과 (FX-REQ-482, P2) | — | 📋 | TD-03 승격, 독립 트랙. 느린 spec 식별 + smoke/full 2-tier 분리 고려 |
...
| F493 | 발굴 단계 평가결과서 v2 — `/discovery/report` 전면 개편: AX BD 9단계(2-1~2-9) 리치 리포트 (레퍼런스/시장/경쟁/도출/선정/고객/BM/패키징/페르소나 평가). 기존 F296 `evaluation_reports` 스키마 확장(`report_data` JSON blob 추가 + migration) + 구조화 JSON ↔ React 9탭 렌더러 (카드/메트릭/표/BMC/SWOT/insight/next-step/Chart.js). 3개 아이템 수동 fixture seed (KOAMI, XR Studio, IRIS) 동봉 + prod D1 insert 스크립트. 샘플 HTML = `references/03_AX사업개발_발굴단계완료(안).html` (1522줄, Fooding AI 예시) (FX-REQ-485, P1) | Sprint 242 | 🔧 | F296 in-place 재작성, `/discovery/report` 경로 유지. 자세한 내용은 `docs/01-plan/F493_plan.md` |
...
| ID | 제목 | Sprint | 상태 | 비고 |
|----|------|--------|:----:|------|
| C1 | discovery analysis process audit (FX-REQ-pending) | — | ❌ | cancelled — C2와 중복 |
| C2 | discovery analysis process audit (FX-REQ-pending) | — | ✅ | completed |
| X1 | task-orchestrator s-beta heartbeat signal (FX-REQ-pending) | — | ✅ | completed — lib.sh write_signal/check_liveness/update_heartbeat 반영 |
| C3 | claude-code 200k context token optimization (FX-REQ-pending) | — | ✅ | completed — CLAUDE.md 82% 압축 (#427) |
| C4 | discovery e2e workflow test biz-ai (FX-REQ-pending) | — | 🔧 | task orchestrator |
| F498 | E2E workflow shard parallelization F490 TD-03 (FX-REQ-pending) | — | 🔧 | task orchestrator |
| C5 | task orchestrator spec sync and skill docs update (FX-REQ-pending) | — | 🔧 | task orchestrator |
| X2 | self-healing watch agent diagnostic and pattern fix (FX-REQ-pending) | — | 🔧 | task orchestrator |
| C6 | E2E workflow walkthrough item-discovery-report-offering-PRD-prototype (FX-REQ-pending) | — | 🔧 | task orchestrator |
| C7 | Task-REQ-Sprint-Phase 거버넌스 매핑 자동화 (FX-REQ-pending) | — | 🔧 | task orchestrator |
| C8 | Marker.io 피드백 자동처리 점검 및 보강 (FX-REQ-pending) | — | 🔧 | task orchestrator |
<!-- /fx-task-orchestrator-backlog -->
