...
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 307테스트 ✅ (API 패키지 307건 pass) + CLI 106 + Web 45 + 20 E2E |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F75 | PlannerAgent LLM 실 연동 — Mock→Claude API 전환 + JSON 파싱 + 폴백 (FX-REQ-075, P1) | v1.4 | 📋 | planner-agent.ts lines 60-89 교체, Haiku 모델 |
| F76 | AgentInboxPanel UI + AgentPlanCard shared import 정리 (FX-REQ-076, P1) | v1.4 | 📋 | AgentInboxPanel 신규 + AgentPlanCard inline→shared + api-client 확장 |
| F77 | v1.4.0 프로덕션 배포 + D1 migration 0009 remote (FX-REQ-077, P2) | v1.4 | 📋 | D1 0009 remote + Workers/Pages 재배포 + 버전 범프 |

### 단독 작업 — 프로젝트 소개 페이지 개편

| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F74 | 프로젝트 소개 페이지 전면 개편 — 정체성 재정립 + 아키텍처/로드맵/블루프린트 (FX-REQ-074, P1) | v1.4 | 📋 | F73 결과 반영, Pricing 삭제, 구조 설명 추가 |
...
- [ ] PlannerAgent createPlan() Mock→Claude API 실 호출 전환 (FX-REQ-075)
- [ ] PlannerAgent 전용 시스템 프롬프트 + JSON schema 응답 파싱 (FX-REQ-075)
- [ ] API 에러 시 Mock 폴백 로직 (FX-REQ-075)
- [ ] planner-agent.test.ts 확장 — LLM mock + 파싱 성공/실패 (FX-REQ-075)
- [ ] AgentPlanCard.tsx shared import 전환 (inline 타입 삭제) (FX-REQ-076)
- [ ] AgentInboxPanel.tsx 신규 — 메시지 목록 + 타입별 렌더링 + ack + SSE (FX-REQ-076)
- [ ] agents/page.tsx inbox 탭 추가 + AgentPlanCard 통합 (FX-REQ-076)
- [ ] api-client.ts 확장 — plan/inbox API 함수 6개 (FX-REQ-076)
- [ ] D1 migration 0009 remote 적용 (FX-REQ-077)
- [ ] Workers + Pages 프로덕션 재배포 + smoke test (FX-REQ-077)
- [ ] version bump v1.4.0 + CHANGELOG + SPEC + git tag (FX-REQ-077)
- [ ] typecheck + build + tests 통과

### 단독: F74 프로젝트 소개 페이지 전면 개편
- [ ] 랜딩 페이지 Hero 섹션 — 정체성 재정립 문구 반영 (FX-REQ-074)
- [ ] Features 섹션 — F73 포지셔닝 결과 기반 재구성 (FX-REQ-074)
- [ ] Pricing 섹션 삭제 (FX-REQ-074)
- [ ] Architecture 섹션 신규 — 시스템 아키텍처 구조 설명 (FX-REQ-074)
- [ ] Roadmap 섹션 신규 — 서비스 개발 로드맵 (FX-REQ-074)
- [ ] BluePrint 섹션 신규 — 전체 시스템 청사진 (FX-REQ-074)
- [ ] Navbar/Footer 업데이트 — 버전 표기 + 네비게이션 반영 (FX-REQ-074)
