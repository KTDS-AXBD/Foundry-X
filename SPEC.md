...
| **Phase 11** | 🔧 진행 — IA 대개편 F288~F299 (7/12 완료) | 11-A ✅ (113~114), 11-B 4/6 ✅ (115~117), 11-C 📋 (118~121) |
| Sprint 122 | 📋 E2E 테스트 종합 정비 (F300) — 16 fail 수정 + API-only 정리 + redirect 검증 + 주기적 점검 체계 | 인프라/품질. Phase 11과 병렬 |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **2432** + CLI **149** + Web **287** = **2868** + E2E (35 specs, **158 tests**: 135 pass / 16 fail / 7 skip) |
...
| F# | 제목 (REQ, Priority) | Sprint | 상태 | 비고 |
|----|----------------------|:------:|:----:|------|
| F300 | E2E 테스트 종합 정비 — 실패 16건 수정 + API-only 4 spec 이관/삭제 + redirect 검증 추가 + uncovered 라우트 커버 확대 + 주기적 점검 체계 수립 (FX-REQ-292, P1) | Sprint 122 | 📋 | 세션 #185 감사 결과 기반. 현재 158 tests (135 pass / 16 fail / 7 skip). 목표: 100% pass + 커버율 85%+ |
