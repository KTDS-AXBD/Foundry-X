...
| F541 | **Offering 도메인 분리** — fx-offering Worker 신규 생성, 12 routes / 23 services → 독립 Worker. Shaping 다음 파이프라인 단계. 범위: (a) `core/offering/*` 코드 + D1 schema(offering_* 테이블) 이전, (b) fx-gateway Service Binding 통합, (c) packages/api offering mount 제거 + msa-lint 통과, (d) cross-domain import(shaping↔offering) 전수 grep + contract 분리, (e) F540 Shaping 선례 활용으로 Sprint Plan/Design 단순화, (f) Smoke Reality: bi-koami-001 Offering 단계 Graph. 전제: F540 완료 (FX-REQ-580, P0) | Sprint 298 | 📋(idea) | F540 선례 기반 방식 확정. 선제 체크리스트 + Phase Exit P1~P4 동일 적용 |
