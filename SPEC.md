...
| C33 | C | eslint no-explicit-any cleanup — packages/cli/src/ui/render.tsx:52 + packages/cli/src/harness/lint-rules/require-zod-schema.ts:32 (FX-REQ-528) | — | PLANNED | 1분 수정, deploy.yml run에서 경고로 노출 중, S260 /ax:daily-check 후속 |
| C34 | C | GitHub Actions Node 24 migration — actions/checkout@v4 + actions/setup-node@v4 + pnpm/action-setup@v4 + dorny/paths-filter@v3 업그레이드 (FX-REQ-529) | — | PLANNED | June 2, 2026 Node 20 강제 전환 대응. feedback_deploy_concurrency_traps.md와 묶어 deploy.yml 전반 리팩토링 후보 |
...
| C36 | C | sidebar/menu에 `/work-management` 라우트 링크 추가 — Sprint 261 F509 Walking Skeleton의 orphan route 해소 (FX-REQ-531) | — | PLANNED | Phase 33 UI 통합, LOW. 현재 URL 직접 타이핑만 가능 |
| C37 | C | autopilot Gap% 측정 범위 확장 — E2E/IA/integration test 차원 포함 또는 autopilot 종료 시 `/ax:e2e-audit` 자동 호출 옵션 (FX-REQ-532) | — | PLANNED | Sprint 261 Gap 98% 자체 보고가 E2E 0 커버리지 놓친 한계 해결, MEDIUM. pdca check 확장 |
<!-- /fx-task-orchestrator-backlog -->
