...
| C67 | C | velocity 메트릭 누락 — `scripts/velocity/record-sprint.sh`가 Master에 존재하지 않는 `.sprint-context`만 참조하여 F_ITEMS/MATCH_RATE/TEST_RESULT/CREATED 전부 빈 값으로 `docs/metrics/velocity/sprint-NNN.json` 기록. Sprint 293 실측: `{"f_items":"","match_rate":null,"duration_minutes":0,"test_result":"unknown"}`. 원인: `.sprint-context`는 WT 생성 시 WT 내부에만 생성됨, Master에서 sprint-merge-monitor가 호출하는 시점엔 부재. 수정안: (a) Master 기준으로 WT 경로(`$CLAUDE_WT_BASE/Foundry-X/sprint-NNN/.sprint-context`) fallback 추가, 또는 (b) signal 파일(`/tmp/sprint-signals/Foundry-X-NNN.signal`)에서 F_ITEMS/MATCH_RATE/TEST_RESULT 직접 읽기(이미 값 존재 확인). (b)가 단순 — signal은 이미 merge-monitor가 읽는 SSOT. task-promotion 기준 미충족(1파일, 내부 메트릭) → Backlog C-track. sprint-293.json 소급 재기록 포함 | — | 📋(idea) | Idea. 수정 후 sprint-293.json 재실행 필요 |
<!-- /fx-task-orchestrator-backlog -->
