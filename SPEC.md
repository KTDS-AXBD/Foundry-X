...
- **Phase**: **Phase 7** — BD Pipeline End-to-End 통합 (FX-BD-V1, Sprint 79~81)
- **Sprint**: 79 (계획), Phase 6 완료 → Phase 7 진입
...
| Sprint 79 | 📋 계획 (F232+F233+F239) — 파이프라인 대시보드 + 산출물 공유 + 의사결정 워크플로 |
| Sprint 80 | 📋 계획 (F234+F235+F237) — BDP 편집/버전관리 + ORB/PRB 게이트 + 사업제안서 자동 생성 |
| Sprint 81 | 📋 계획 (F236+F238+F240) — Offering Pack + MVP 추적 + IR Bottom-up 채널 |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **1965** + CLI **149** + Web **121+** = **2235+** + E2E ~55 |
| API endpoints | **~330개** (54 routes) |
| API services | **143개** |
| API schemas | **69개** |
| D1 migrations | **0001~0065** |
| D1 remote | 0001~0065 전체 적용 완료 |
| Workers | foundry-x-api.ktds-axbd.workers.dev ✅ Sprint 77 (05d22d2e) |
| Pages | fx.minu.best ✅ |
...
| F232 | 파이프라인 통합 대시보드 — 칸반/파이프라인 뷰 전환, 아이템별 단계·진행률·다음 액션 조회. D1 pipeline_stages 테이블 (FX-REQ-224, P0) | Sprint 79 | 📋 | |
| F233 | 산출물 공유 시스템 — 인증 기반 공유 링크 생성 + 만료 설정 + 리뷰 요청 + 인앱/이메일 알림. D1 share_links + notifications (FX-REQ-225, P0) | Sprint 79 | 📋 | |
| F234 | BDP 편집/버전관리 — 사업계획서 마크다운 에디터 + 버전 히스토리 + diff 뷰 + 최종본 잠금. D1 bdp_versions (FX-REQ-226, P0) | Sprint 80 | 📋 | |
| F235 | ORB/PRB 게이트 준비 — 산출물 자동 수집 → 게이트 문서 패키지 구성 → ZIP 다운로드. D1 gate_packages (FX-REQ-227, P0) | Sprint 80 | 📋 | |
| F236 | Offering Pack 생성 — 영업/제안용 번들(사업제안서+데모링크+기술검증+가격). D1 offering_packs (FX-REQ-228, P0) | Sprint 81 | 📋 | |
| F237 | 사업제안서 자동 생성 — 사업계획서(BDP)에서 게이트 제출용 요약본 LLM 자동 추출 (FX-REQ-229, P1) | Sprint 80 | 📋 | |
| F238 | MVP 추적 + 자동화 — MVP 상태(In Dev/Testing/Released) 추적 + PoC 배포 자동화 파이프라인. D1 mvp_tracking (FX-REQ-230, P1) | Sprint 81 | 📋 | |
| F239 | 단계별 의사결정 워크플로 — Go/Hold/Drop 버튼 + 팀장 승인/반려 + 코멘트 + 이력 + 자동 단계 전환. D1 decisions (FX-REQ-231, P0) | Sprint 79 | 📋 | |
| F240 | IR Bottom-up 채널 — 사내 현장(엔지니어/영업) 제안 전용 등록 폼 → biz-item 자동 변환 (FX-REQ-232, P2) | Sprint 81 | 📋 | |
