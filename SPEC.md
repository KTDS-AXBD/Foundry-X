...
| Sprint 137 | 🔧 (F319+F320) — Marker.io 피드백 자동화: Webhook→D1큐→Claude Code Agent→PR→Review→배포 | FX-PLAN-014 P1 |
| Sprint 138 | 📋 (F321) — TinaCMS 네비게이션 동적 관리: 사이드바+랜딩 메뉴 순서 CMS 관리 | FX-PLAN-014 P2 |
...
> ```
> **마지막 실측** (Sprint 136, 2026-04-05): ~88 routes, ~205 services, ~103 schemas, D1 0093, tests ~3148 (0 fail)
...
| F# | 설명 | Sprint | 상태 | 비고 |
|----|------|:------:|:----:|------|
| F319 | Marker.io 피드백 Webhook 수신 + D1 큐 — GitHub Issue webhook → Workers API endpoint + D1 feedback_queue 테이블(상태 머신: pending→processing→done/failed) + visual-feedback 라벨 필터링 (FX-REQ-311, P1) | Sprint 137 | 🔧 | F309 후속. Marker.io→GitHub Issue는 기존 동작 |
| F320 | 피드백 자동 처리 Agent + PR 생성 — WSL 큐 소비자 스크립트 + Claude Code CLI Agent 연동 + Issue 분석→코드수정→테스트→PR 자동 생성 + 관리자 GitHub Review 알림 (FX-REQ-312, P1) | Sprint 137 | 🔧 | F319 선행. PR merge → deploy.yml 자동 배포(기존) |
| F321 | TinaCMS 네비게이션 동적 관리 — navigation TinaCMS collection + 사이드바 메뉴 순서/표시 content 파일 기반 렌더링 + 랜딩 페이지 섹션 순서 CMS 관리 (FX-REQ-313, P2) | Sprint 138 | 📋 | F311 후속. 비개발자 메뉴 구조 변경 가능 |
