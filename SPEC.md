...
| Sprint 93 | 📋 계획 (F256+F257) — GIVC PoC 2차 + 추가 BD 아이템 탐색 |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **2190** + CLI **149** + Web **249** = **2588** + E2E (27 specs, ~93 tests) |
| API endpoints | **~385개** (69 routes) |
| API services | **160개** |
| API schemas | **83개** |
...
| Sprint 93 | Phase 9: GIVC PoC 2차(F256) + 추가 BD 아이템(F257) | 📋 |
| Sprint 94 | Phase 9: 발굴 UX — 위저드 UI(F263) + 온보딩 투어(F265) | 📋 |
| Sprint 95 | Phase 9: 발굴 UX — Help Agent 챗봇(F264) | 📋 |
| Sprint 96 | Phase 9: 발굴 UX — HITL 인터랙션 패널(F266) | 📋 |
| Sprint 97 | Phase 9: 발굴 UX — 통합 QA + 팀 데모 | 📋 |
...
| F256 | GIVC PoC 2차 — 4대 시나리오(이벤트 연쇄/대체 공급처/EWS 영향/리스크맵) 중 1개 MVP 구현 (FX-REQ-248, P2) | Sprint 93 | 📋 | F255 선행 |
| F257 | 추가 BD 아이템 탐색 — Discovery 파이프라인으로 신규 사업 아이템 1~2건 발굴 + 사업성 체크포인트 통과 (FX-REQ-249, P2) | Sprint 93 | 📋 | BD Pipeline(F232~F240) 실사용 |
...
| F263 | 발굴 프로세스 단계별 안내 UI — 위저드/스텝퍼 재구성 + biz-item별 진행 추적 (FX-REQ-255, P0) | Sprint 94 | 📋 | F241 확장, F258 기반 |
| F264 | Help Agent (개인 비서) — OpenRouter SSE 스트리밍 챗 + 컨텍스트 인식 안내 (FX-REQ-256, P0) | Sprint 95 | 📋 | F57(SSE), F260(스킬 실행) 의존 |
| F265 | 발굴 온보딩 투어 개선 — 인터랙티브 3~5스텝 가이드 (FX-REQ-257, P0) | Sprint 94 | 📋 | F252 연계, F263과 동일 Sprint |
| F266 | HITL 인터랙션 + 결과물 확인 — 인라인 패널 (승인/수정/재생성/거부) (FX-REQ-258, P0) | Sprint 96 | 📋 | F260, F261 기반 |
