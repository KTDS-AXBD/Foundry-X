...
| F545 | **fx-ai-foundry-os: 3-Plane 랜딩 대시보드** — `/ai-foundry-os` 라우트 신설, DeepDive v0.3 Input/Control/Presentation Plane 3 카드 + "라이브 시연" 진입 버튼. 대표 보고(4/17 09:00) 랜딩 페이지. PRD: `docs/specs/fx-ai-foundry-os/prd-final.md` (FX-REQ-581, P0) | Sprint 298 | 📋(plan) | req-interview Round 1 Score 85/100. 42시간 MVP 묶음의 엔트리 포인트 |
| F546 | **fx-ai-foundry-os: fx-decode-bridge 연동 레이어** — `packages/api/src/core/decode-bridge/` 신설, Decode-X Workers(KTDS-AXBD/Decode-X) 5 라우트 프록시: `/analyze`, `/analysis/:documentId/summary`, `/findings`, `/compare`, `/core-processes`. X-Internal-Secret 전달 + Zod 스키마 부분 미러. Service Binding 가능 시 binding 우선, 아니면 fetch. PRD: `docs/specs/fx-ai-foundry-os/prd-final.md` §9 (FX-REQ-582, P0) | Sprint 298 | 📋(plan) | Decode-X URL/secret 확보가 Open Issue #1 (H+2 마감) |
| F547 | **fx-ai-foundry-os: LPON Type 1 시연 페이지** — `/ai-foundry-os/demo/lpon` — Decode-X 3-Pass(scoring/diagnosis/comparison) 결과 시각화 + "Type 1 반제품(Code+Test)" 다운로드 버튼. 다운로드 본체는 Decode-X `반제품-스펙/pilot-lpon-cancel/working-version/` 완성 반제품 패키징 (6종 Spec md + src/ Code + __tests__/ + migrations/0001_init.sql). 성공 KPI: 대표 보고 rehearsal walkthrough 통과. PRD: `docs/specs/fx-ai-foundry-os/prd-final.md` (FX-REQ-583, P0) | Sprint 298 | 📋(plan) | F546 선행 필수. 도메인 선정: 온누리상품권(LPON) — 퇴직연금은 DeepDive A-4 예시이나 반제품 미구현 (2026-04-16 재판정) |
| F548 | **fx-ai-foundry-os: Harness 5종 체크리스트 UI** — `/ai-foundry-os/harness` — KT연계·사업성·리스크·AI-Ready Data·구체화 5종 자동 점검 결과 표시. 현재는 Foundry-X 자체 메트릭 기반 mock + 실시간 지표 1~2개(agent_improvement_proposals 카운트 등). DeepDive B-1 Harness Engineering 가시화. PRD: `docs/specs/fx-ai-foundry-os/prd-final.md` (FX-REQ-584, P0) | Sprint 298 | 📋(plan) | M3 구간. 미달 시 P1로 강등 가능 |
| F549 | **fx-ai-foundry-os: KG XAI 뷰어 (read-only)** — `/ai-foundry-os/ontology` — Decode-X Neo4j 노드(SubProcess/Method/Condition/Actor/Requirement/DiagnosisFinding)를 D3 force-graph로 표시, 클릭 시 "이 결정의 근거 경로" 자동 하이라이트. Neo4j 조회 엔드포인트 없으면 mock 데이터 10+ 노드로 fallback. PRD: `docs/specs/fx-ai-foundry-os/prd-final.md` (FX-REQ-585, P0) | Sprint 298 | 📋(plan) | Open Issue #4 (Neo4j 접근 경로) H+4 마감. 구현 복잡도 높음 → M4 버퍼 필요 |
