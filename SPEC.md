...
- **Phase**: **Phase 21** ✅ 완료 — Gate-X 독립 서비스 F402~F413 | **Phase 22** ✅ 완료 — Offering Skill v2 + Builder v2 F414~F431 | **Phase 23** ✅ 완료 — Sprint Automation v2 F432~F433 (Match 98%) | **Phase 24** ✅ 완료 — Discovery Native F434~F440 (Match 97%) | **Phase 25** ✅ 완료 — Discovery Pipeline v2 F441~F450 (Sprint 213~218, Match ~99%) | **Phase 26** ✅ 완료 — BD Portfolio Management F451~F460 (Sprint 219~223, Match 96%) | **Phase 27** ✅ 완료 — BD Quality System F461~F470 (Sprint 225~230, 10/10 ✅)
- **Sprint**: 148~164 (✅)
...
> ```
> **마지막 실측** (Sprint 230, 2026-04-09): ~9 routes, ~40 services (adapters 13 포함), ~13 schemas, D1 0121, tests ~3262 (E2E 263) — Phase 20-21 MSA 재조정으로 core/modules가 gate-x·harness-kit으로 이전
...
| F478 | STATUS_CONFIG 매핑 보완 — biz_items.status 전체 상태(classifying/classified/evaluating/evaluated) UI 매핑 (FX-REQ-470, P0) | Sprint 235 | 📋 | business-plan-list + discovery-detail |
| F479 | 분석 완료 → pipeline/discovery_stages 자동 전환 — evaluate 완료 시 REGISTERED→DISCOVERY + discovery_stages 동기화 (FX-REQ-471, P0) | Sprint 235 | 📋 | biz-items 라우트 + pipeline-service |
| F480 | AnalysisStepper → Discovery Stage 전체 스텝퍼 리뉴얼 — 3단계 자동→11단계 HITL 스텝퍼, v82 유형별 강도 반영 (FX-REQ-472, P1) | Sprint 236 | 📋 | DiscoveryStageStepper + stage-runner API |
| F481 | 평가결과서 HTML 자동 생성 스킬 — PRD-final 파싱→발굴단계완료 HTML 9탭 포맷 자동 변환, CLAUDE_AXBD 스킬로 통합 (FX-REQ-473, P0) | Sprint 235 | 📋 | CLAUDE_AXBD/commands/ + HTML 템플릿 엔진 |
| F482 | bd_artifacts 자동 등록 파이프라인 — Claude Code 스킬 분석 완료 시 API 호출로 bd_artifacts + discovery_stages 자동 동기화 (FX-REQ-474, P0) | Sprint 235 | 📋 | biz-items API + artifact-sync 서비스 |
| F483 | 웹 평가결과서 뷰어 — Discovery 상세 페이지에 발굴단계완료 HTML 평가결과서 조회/공유 기능 (FX-REQ-475, P1) | Sprint 236 | 📋 | discovery-report-viewer.tsx + share API |
