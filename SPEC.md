...
| C14 | C | E2E smoke A — lib.sh + task-start.sh static checks (FX-REQ-508) | — | CLOSED_EMPTY | S257b: 500자 prompt trunc로 워커가 파일 미작성, 빈 PR 머지. S257b에서 재시도 (C16) |
| C15 | C | E2E smoke B — task-complete + daemon guard checks (FX-REQ-509) | — | CLOSED_EMPTY | S257b: 동일 trunc 버그 + pane 소멸, 빈 PR 머지. S257b에서 재시도 (C17) |
| C16 | C | E2E smoke A retry — lib.sh + task-start checks (FX-REQ-510) | — | PLANNED | task orchestrator |
| C17 | C | E2E smoke B retry — task-complete + daemon guards (FX-REQ-511) | — | PLANNED | task orchestrator |
<!-- /fx-task-orchestrator-backlog -->
