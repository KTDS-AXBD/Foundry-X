...
| F491 | 테스트 공유 Org 모드 — `DEFAULT_SHARED_ORG_ID` env var 도입으로 signup/google/setup-password 3 flow에서 개인 Org 자동 생성 대신 지정된 공유 Org(`org_axbd`)에 멤버십 부여. env 미설정 시 기존 개인 Org 동작 유지(폴백), null-return 헬퍼 패턴. 기존 Google 유저도 첫 재로그인 시 공유 Org에 멱등 backfill (FX-REQ-483, P3) | — | ✅ | 독립 트랙. `packages/api/src/modules/auth/services/shared-org.ts` 헬퍼 신설, `wrangler.toml [vars]` 설정, 신규 테스트 4건(auth-shared-org.test.ts), 기존 auth 30 tests 리그레션 없음 |
