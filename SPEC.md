...
- **Phase**: **Phase 9** — 팀 온보딩 + GIVC PoC + 발굴 UX 개선 (Sprint 87~)
- **Sprint**: 100 (완료) — 발굴 IA & Page 정리 (F269)
...
| Sprint 108 | 🔧 진행 중 (F279+F280) — BD 데모 시딩: 2개 아이디어 × 6단계 데이터 + 산출물 콘텐츠 16건 |
| Sprint 109 | 📋 예정 (F281) — 데모 E2E 검증 + Production 배포 + UI 보정 | F279+F280 선행 |
| Sprint 110 | 🔧 진행 중 (F282+F283) — BD 형상화 Phase A+B+C: 입력 점검 + req-interview + O-G-D 형상화 루프 | **108과 병렬 가능** (파일 충돌 0%) |
| Sprint 111 | 📋 예정 (F284+F285) — BD 형상화 Phase D+E: Six Hats 토론 + 전문가 5종 리뷰 | F283 선행 |
| Sprint 112 | 📋 예정 (F286+F287) — BD 형상화 Phase F: HITL Web 에디터 + 자동 모드 + D1 + E2E | F284+F285 선행 |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **2250** + CLI **149** + Web **265** = **2664** + E2E (35 specs, ~146 tests) |
| API endpoints | **~420개** (73 routes) |
| API services | **169개** |
| API schemas | **87개** |
| D1 migrations | **0001~0080** |
| D1 remote | 0001~0079 전체 적용 완료 (0080 로컬만) |
| Workers | foundry-x-api.ktds-axbd.workers.dev ✅ (세션 #156 배포, D1 0079 적용) |
| Pages | fx.minu.best ✅ |
...
| F276 | Track C: DERIVED 엔진 — BD 7단계 반복 성공 패턴 자동 추출 + 새 스킬 생성 + HITL 승인 (FX-REQ-268, P0) | Sprint 105 | 🔧 | F274 선행, PoC 통과 조건 |
| F277 | Track C: CAPTURED 엔진 — 크로스 도메인 워크플로우 캡처 + 메타 스킬 생성 + F191 방법론 레지스트리 연동 (FX-REQ-269, P1) | Sprint 106 | 📋 | F276 선행 |
| F278 | Track E: BD ROI 벤치마크 — Cold Start vs Warm Run 비교 + BD_ROI 공식 + F262 사업성 신호등 달러 환산 (FX-REQ-270, P1) | Sprint 107 | 📋 | F274, F276 선행 |
| **Phase 10 — BD 데모 데이터 (Production Showcase)** | | | | |
| F279 | BD 데모 시딩 마이그레이션 — D1 0081: 2개 아이디어(헬스케어AI+GIVC) × 12테이블 ~103 rows INSERT. pipeline_stages 7단계 이력 + discovery_stages 11단계 + viability_checkpoints + commit_gates (FX-REQ-271, P0) | Sprint 108 | 🔧 | 없음 |
| F280 | BD 산출물 콘텐츠 생성 — bd_artifacts 16건 상세 output_text(시장조사/경쟁분석/BMC/PRD/MVP 등 한글 1~3p). O-G-D 데모 결과물 직접 활용 + BMC 9블록 + BDP v1~v2 + Offering Pack items (FX-REQ-272, P0) | Sprint 108 | 🔧 | F279 동일 Sprint |
| F281 | 데모 데이터 E2E 검증 — Production 배포 + 6단계 워크쓰루(수집→GTM) + 산출물 상세 Markdown 렌더링 확인 + UI 빈화면/깨짐 수정 (FX-REQ-273, P1) | Sprint 109 | 📋 | F279+F280 선행 |
| **Phase 10 — BD 형상화 파이프라인 (Stage 3→4 자동화)** | | | | |
| F282 | BD 형상화 Phase A — 입력 점검 & 갭 분석 (체크리스트 10항목 + 갭 처리 전략 + ax-bd-shaping 스킬 메인) (FX-REQ-274, P0) | Sprint 110 | 🔧 | PRD: prd-shaping-v1.md. **Sprint 108과 병렬 가능** (파일 충돌 0%) |
| F283 | BD 형상화 Phase B+C — req-interview 연동 + O-G-D 형상화 루프 (Rubric 5차원 + shaping-orchestrator/generator/discriminator 3 에이전트 + 수렴 0.85) (FX-REQ-275, P0) | Sprint 110 | 🔧 | F282 동일 Sprint. Merge 순서: 108→109→110 |
| F284 | BD 형상화 Phase D — 다중 AI 모델 교차 검토 + Six Hats 토론 (OpenRouter 3모델 + 6색 모자 + 합의 매트릭스 + six-hats-moderator 에이전트) (FX-REQ-276, P1) | Sprint 111 | 📋 | F283 선행 |
| F285 | BD 형상화 Phase E — 전문가 AI 페르소나 리뷰 (TA/AA/CA/DA/QA 5종 에이전트 + 교차 영향 분석 + 통합 리뷰 보고서 + Fan-out/Fan-in) (FX-REQ-277, P1) | Sprint 111 | 📋 | F283 선행 |
| F286 | BD 형상화 Phase F — HITL 게시 & 편집 + 자동 모드 (Web PRD 에디터 + 섹션별 승인/수정/반려 + AI 자가 리뷰 3 페르소나 + auto-reviewer 에이전트) (FX-REQ-278, P1) | Sprint 112 | 📋 | F284+F285 선행 |
| F287 | BD 형상화 D1 스키마 + Git 병행 저장 + 통합 E2E (shaping_runs/phase_logs/expert_reviews/six_hats 4테이블 + docs/shaping/ 산출물 + E2E 테스트) (FX-REQ-279, P0) | Sprint 112 | 📋 | F286 동일 Sprint |
