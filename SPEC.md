...
- **Phase**: Phase 4 진행 중 (서비스 통합 + 온보딩) — Sprint 25~28 완료
- **Version**: 2.2.0
...
| Sprint 30 | 📋 계획 (F123~F128: 배포 동기화 + Phase 4 Go 판정 + 품질 강화) |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 566테스트 ✅ (API 패키지) + CLI 106 + Web 48 + ~51 E2E |
| API endpoints | 112개 (25 routes) |
| API services | 45개 |
| D1 tables | 32개 (migrations 0001~0019) |
| D1 remote | 0001~0017 적용 완료, 0018~0019 미적용 |
| Workers | foundry-x-api.ktds-axbd.workers.dev ✅ v2.1.0 |
| Pages | fx.minu.best ✅ |
...
| v2.3 | Sprint 29: 실사용자 온보딩(F114) + Phase 4 마무리 | 📋 |
| v2.4.0 | Sprint 30: 프로덕션 배포 동기화 + Phase 4 Go 판정 준비 + 품질 강화 (F123~F128) | 📋 |
...
- [ ] D1 migration 0018 remote 적용 — kpi_events + reconciliation_runs (FX-REQ-123)
- [ ] Workers v2.2.0 프로덕션 배포 + smoke test (FX-REQ-123)
- [ ] F106 프론트엔드 통합 개선 — 서브앱 네비게이션 통합 + postMessage 데이터 공유 확장 (FX-REQ-124)
- [ ] KPI 추적 대시보드 UI — K7 WAU + K8 에이전트 완료율 + K9 서비스 전환율 표시 (FX-REQ-125)
- [ ] Phase 4 Go 판정 문서 작성 — PRD v5 Go 조건 대비 현황 + 판정 근거 (FX-REQ-125)
- [ ] Harness Evolution Rules 자동 감지 서비스 — 위반 패턴 감지 + SSE 경고 알림 (FX-REQ-126)
- [ ] PRD v5 MVP 체크리스트 갱신 + codegen-core 재활용 판정 문서 (FX-REQ-127)
- [ ] Phase 4 통합 경로 E2E — iframe SSO + BFF 프록시 + 엔티티 레지스트리 크로스 쿼리 (FX-REQ-128)
- [ ] API 에러 응답 표준화 — ErrorResponse 스키마 통일 + 에러 코드 체계 (FX-REQ-128)
- [ ] typecheck ✅ + PDCA

### Sprint 29 (v2.3.0) — 실사용자 온보딩 기반: 가이드 UI + 피드백 API + 체크리스트
- [ ] 온보딩 가이드 UI — /getting-started 페이지 + 인터랙티브 투어 + FAQ 섹션 (FX-REQ-120)
- [ ] 피드백 수집 시스템 — NPS 설문 API + D1 테이블 (onboarding_feedback, onboarding_progress) + 위젯 (FX-REQ-121)
- [ ] 온보딩 체크리스트 — 사용자별 진행률 추적 + 완료 시 KPI 이벤트 기록 (FX-REQ-122)
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F123 | 프로덕션 배포 동기화 — D1 0018 remote + Workers v2.2.0 배포 + smoke test (FX-REQ-123, P0) | v2.4 | 📋 | Sprint 27~28 코드 프로덕션 반영 |
| F124 | 프론트엔드 통합 개선 — F106 잔여(85%→) 네비게이션 일관성 + 데이터 공유 강화 (FX-REQ-124, P1) | v2.4 | 📋 | Phase 4-A 완성도 향상 |
| F125 | Phase 4 Go 판정 준비 — KPI 대시보드(K7/K8/K9) 추적 UI + Go 판정 문서 (FX-REQ-125, P1) | v2.4 | 📋 | PRD v5 Phase 4 판정 조건 대비 |
| F126 | Harness Evolution Rules 자동 감지 — 위반 감지 + 경고 알림 (FX-REQ-126, P2) | v2.4 | 📋 | Q6 해소, 규칙 자동 감지 미구현 |
| F127 | PRD↔구현 정합성 갱신 — MVP 체크리스트 + codegen-core 재활용 판정 (FX-REQ-127, P2) | v2.4 | 📋 | PRD v5 §4 MVP 기준 + §9 codegen-core |
| F128 | E2E 테스트 보강 + 에러 핸들링 — Phase 4 통합 경로 E2E + API 에러 응답 표준화 (FX-REQ-128, P1) | v2.4 | 📋 | Phase 4 Go 판정 시 품질 근거 |
