...
- **Phase**: **Phase 17** ✅ 완료 | **Phase 18** 📋 Offering Pipeline F363~F383 (Sprint 165~174, 0/21) | **Phase 19** 📋 Builder Evolution F384~F391 (Sprint 175~178, 0/8)
- **Sprint**: 148~164 (✅)
...
| F363 | offering-html SKILL.md 등록 + ax-bd/shape/ 디렉토리 + INDEX.md (FX-REQ-355, P0) | Sprint 165 | 📋 | |
| F364 | HTML 템플릿 분리 — base.html + 17종 컴포넌트 + examples/KOAMI (FX-REQ-356, P0) | Sprint 165 | 📋 | |
| F365 | 디자인 토큰 Phase 1 — design-tokens.md 컬러/타이포/레이아웃 (FX-REQ-357, P1) | Sprint 165 | 📋 | |
| F366 | F275 Skill Registry 연동 — evolution: DERIVED 선언 (FX-REQ-358, P1) | Sprint 165 | 📋 | F363 선행 |
| **Sprint 166 — Foundation: Agent 확장 + PPTX 설계** | | | | |
| F367 | offering-pptx SKILL.md 등록 + Cowork 연동 설계 (FX-REQ-359, P1) | Sprint 166 | 📋 | |
| F368 | ax-bd-offering-agent — shaping-orchestrator 확장, 6 capability (FX-REQ-360, P0) | Sprint 166 | 📋 | F363 선행 |
| **Sprint 167 — Data Layer: D1 + CRUD** | | | | |
| F369 | D1 마이그레이션 — offerings/offering_versions/offering_sections/offering_design_tokens (FX-REQ-361, P0) | Sprint 167 | 📋 | |
| F370 | Offerings CRUD API — POST/GET/PUT/DELETE + Zod 스키마 + 서비스 (FX-REQ-362, P0) | Sprint 167 | 📋 | F369 선행 |
| F371 | Offering Sections API — 18섹션 CRUD + 필수/선택 토글 (FX-REQ-363, P0) | Sprint 167 | 📋 | F369 선행 |
| **Sprint 168 — Data Layer: Export + Validate** | | | | |
| F372 | Offering Export API — HTML/PDF export, base.html + 컴포넌트 렌더링 (FX-REQ-364, P0) | Sprint 168 | 📋 | F364, F370 선행 |
| F373 | Offering Validate API — O-G-D Loop(F335) + Six Hats + Expert 호출 (FX-REQ-365, P0) | Sprint 168 | 📋 | F368, F370 선행 |
| **Sprint 169 — Full UI: 목록 + 위자드** | | | | |
| F374 | Offerings 목록 페이지 — Kanban 뷰, 상태 필터, 버전 히스토리 (FX-REQ-366, P0) | Sprint 169 | 📋 | F370 선행 |
| F375 | Offering 생성 위자드 — 발굴 연결 + 목적/포맷/목차 선택 (FX-REQ-367, P0) | Sprint 169 | 📋 | F370 선행 |
| **Sprint 170 — Full UI: 에디터 + 검증 대시보드** | | | | |
| F376 | 섹션 에디터 + HTML 프리뷰 — 실시간 프리뷰, 섹션별 편집 (FX-REQ-368, P0) | Sprint 170 | 📋 | F371, F372 선행 |
| F377 | 교차검증 대시보드 — GAN 추진론/반대론 + Six Hats + Expert 시각화 (FX-REQ-369, P1) | Sprint 170 | 📋 | F373 선행 |
| **Sprint 171 — Integration: 어댑터 + 파이프라인** | | | | |
| F378 | 콘텐츠 어댑터 — 3가지 톤 변환 (executive/technical/critical) + UI (FX-REQ-370, P0) | Sprint 171 | 📋 | F370 선행 |
| F379 | discover → shape 자동 전환 — EventBus(F334) 이벤트 발행/소비 (FX-REQ-371, P0) | Sprint 171 | 📋 | F370 선행 |
| **Sprint 172 — Integration: PPTX 구현** | | | | |
| F380 | offering-pptx 구현 — PPTX 생성 엔진 + 표준 목차 슬라이드 변환 (FX-REQ-372, P1) | Sprint 172 | 📋 | F367 선행 |
| **Sprint 173 — Polish: 토큰 에디터 + Prototype** | | | | |
| F381 | 디자인 토큰 Phase 2+3 — JSON 정규 + API + Web 실시간 에디터 (FX-REQ-373, P1) | Sprint 173 | 📋 | F365, F376 선행 |
| F382 | prototype-builder 연동 — Offering → Phase 16 Builder 자동 호출 (FX-REQ-374, P1) | Sprint 173 | 📋 | F372 선행 |
| **Sprint 174 — Polish: E2E + 메트릭** | | | | |
| F383 | E2E 파이프라인 테스트 + BD ROI 메트릭 수집 F274+F278 연동 (FX-REQ-375, P1) | Sprint 174 | 📋 | 전체 선행 |
...
| F384 | CLI `--bare` rate limit/안정성 PoC — subprocess 통합 + 장시간 테스트 (FX-REQ-376, P0) | Sprint 175 | 📋 | |
| F385 | 5차원 평가 재현성 검증 — 동일 코드 3회 평가 ±10점 이내 + 인간 평가 상관관계 (FX-REQ-377, P0) | Sprint 175 | 📋 | |
| **Sprint 176 — M1: 5차원 스코어링 엔진** | | | | F384, F385 선행 |
| F386 | 5차원 품질 스코어러 구현 — 빌드/UI/기능/PRD반영/코드품질 (FX-REQ-378, P0) | Sprint 176 | 📋 | F385 선행 |
| F387 | 베이스라인 측정 + D1 저장 — 5종 프로토타입 점수 측정 + API 조회 (FX-REQ-379, P0) | Sprint 176 | 📋 | F386 선행 |
| **Sprint 177 — M2+M3: CLI 통합 + 자동 개선** | | | | F384 선행 |
| F388 | CLI 듀얼 모드 — Claude Max CLI primary + API fallback 자동 전환 (FX-REQ-380, P0) | Sprint 177 | 📋 | F384 선행 |
| F389 | Enhanced O-G-D 루프 — 타겟 피드백 + 5라운드 80점 수렴 + 장애 복구 (FX-REQ-381, P0) | Sprint 177 | 📋 | F386, F388 선행 |
| **Sprint 178 — M4: 품질 대시보드** | | | | |
| F390 | Builder Quality 대시보드 — 점수 카드 + 레이더 차트 + 개선 추이 (FX-REQ-382, P1) | Sprint 178 | 📋 | F387 선행 |
| F391 | 사용자 피드백 루프 — BD팀/고객 수동 평가 + 자동 점수 상관관계 캘리브레이션 (FX-REQ-383, P1) | Sprint 178 | 📋 | F386 선행 |
