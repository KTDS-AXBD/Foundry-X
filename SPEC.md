...
| F511 | Work Management 품질 보강 — API 에러 핸들링 UI + syncSessions SQL injection 방어 + E2E edge case 보강 (FX-REQ-534, P1) | Sprint 263 | 📋 | G1=에러 핸들링 fallback UI(401/500 → 메시지+재시도), T1=Sessions E2E edge case(빈 세션/에러/상태전환), T2=polling 갱신 E2E, T3=syncSessions string interpolation→prepared stmt, T4=getSessions ORDER BY 의도 검증 unit test. PR #514(401 인증 버그 핫픽스) 선행 |
