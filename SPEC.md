...
- **Phase**: **Phase 5f** — AX BD 사업개발 체계 수립 (프로세스 v8.2 풀 통합)
- **Sprint**: 71 (완료), Phase 5f 완료
...
| Sprint 75 | 📋 계획 (F220+F222) — Brownfield Init 강화 + Changes Directory |
| Sprint 76 | 📋 계획 (F221+F223) — Agent-as-Code 선언적 정의 + Doc Sharding |
| Sprint 77 | 📋 계획 (F224~F228) — Reference 항목 5건 (컨텍스트 전달/커맨드 UX/Party Mode/Spec Library/Expansion Pack) |
| Sprint 78 | 📋 계획 (F229~F231) — Watch 항목 3건 (Agent Spec 표준/Scale-Adaptive/Multi-repo) |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **1786** + CLI **125** + Web **121+** = **2032+** + E2E ~55 |
| API endpoints | **~304개** (47 routes) |
| API services | **135개** |
| API schemas | **61개** |
| D1 migrations | **0001~0060** |
| D1 remote | 0001~0060 전체 적용 완료 |
| Workers | foundry-x-api.ktds-axbd.workers.dev ✅ Sprint 67 (f15a185a) — Sprint 68~71 재배포 필요 |
| Pages | fx.minu.best ✅ |
...
| F220 | Brownfield-first Init 강화 — `foundry-x init` 시 기존 코드베이스 자동 스캔(tech stack/파일 구조/기존 스펙) → `project-context.md` + `ARCHITECTURE.md` 초안 생성. OpenSpec 참고 (FX-REQ-212, P0) | Sprint 75 | 📋 | discover.ts 확장, RepoProfile 타입 확장 |
| F221 | Agent-as-Code 선언적 정의 레이어 — BMAD `.agent.yaml` 패턴 참고, 에이전트를 YAML/JSON 선언적 정의. `custom_agent_roles` D1 스키마 확장(persona/dependencies/customization/menu) (FX-REQ-213, P1) | Sprint 76 | 📋 | D1 migration + YAML→에이전트 로더 |
| F222 | Structured Changes Directory — OpenSpec `changes/` 패턴 도입. 변경별 proposal/design/tasks/spec-delta 묶음 관리 + `foundry-x sync` Δspec 자동 감지 + Triangle Health Score 반영 (FX-REQ-214, P1) | Sprint 75 | 📋 | SpecParserService 확장, changes/ 디렉토리 규약 |
| F223 | 문서 Sharding 자동화 — BMAD `shard-doc` 참고, 대형 PRD를 에이전트별 관련 섹션만 참조하도록 자동 분할. 에이전트 컨텍스트 윈도우 최적화 (FX-REQ-215, P2) | Sprint 76 | 📋 | shard-doc 서비스, 에이전트별 컨텍스트 분배 |
| F224 | [Ref] SM→Dev 컨텍스트 전달 구조 — BMAD Story 파일 기반 컨텍스트 전달 방식 참고. Sprint 워크플로우(F142)에 적용 검토 (FX-REQ-216, P3) | Sprint 77 | 📋 | 참고 항목, 설계 시 반영 |
| F225 | [Ref] 슬래시 커맨드 UX — OpenSpec `/opsx:` 커맨드 패턴 참고. Phase 2 MCP/IDE 통합 시 커맨드 UX 설계 반영 (FX-REQ-217, P3) | Sprint 77 | 📋 | 참고 항목, IDE 통합 시 반영 |
| F226 | [Ref] Party Mode (다중 에이전트 세션) — BMAD 자유형 토론 방식 참고. Ensemble Voting(F147) 보완적 적용 검토 (FX-REQ-218, P3) | Sprint 77 | 📋 | 참고 항목, 설계 시 반영 |
| F227 | [Ref] Spec Library 구조 — OpenSpec 기능 단위 스펙 조직 방식 참고. Wiki 동기화(F46) 개선 시 적용 검토 (FX-REQ-219, P3) | Sprint 77 | 📋 | 참고 항목, 설계 시 반영 |
| F228 | [Ref] Expansion Packs 모델 — BMAD 도메인 확장 패키징/배포 방식 참고. Agent Marketplace(F152) 개선 시 적용 (FX-REQ-220, P3) | Sprint 77 | 📋 | 참고 항목, 마켓플레이스 개선 시 반영 |
| F229 | [Watch] Agent Spec 표준 — Oracle Open Agent Specification. Phase 3+ 에이전트 이식성 관점 장기 관찰. YAML/JSON 내보내기 포맷 검토 (FX-REQ-221, P4) | Sprint 78 | 📋 | 장기 관찰, Phase 3+ 검토 |
| F230 | [Watch] Scale-Adaptive Intelligence — BMAD 프로젝트 규모별 자동 조절 패턴. Phase 2+ 멀티스케일 지원 시 참고 (FX-REQ-222, P4) | Sprint 78 | 📋 | 장기 관찰, Phase 2+ 검토 |
| F231 | [Watch] Multi-repo Workspace — OpenSpec 조직 전체 복수 저장소 확장. 모노리포 넘어선 확장 시 필요 (FX-REQ-223, P4) | Sprint 78 | 📋 | 장기 관찰, 확장 시 검토 |
