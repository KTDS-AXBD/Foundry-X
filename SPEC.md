...
> ```
> **마지막 실측** (Sprint 258, 2026-04-11): ~9 routes, ~40 services (adapters 13 포함), ~13 schemas, D1 0125, tests ~3447 (E2E 263) — 수치 변동 없음 (S257/S258은 task orchestrator scripts/task/*.sh 위주, packages/api 미변경)
...
| C25 | C | board-sync-spec.sh gh project scope preflight 체크 강화 (FX-REQ-519) | — | PLANNED | S259 C24 dogfood 대상. `scripts/board/_common.sh` `board::require_projects()`가 `gh auth status` scope 감지 부족 시 actionable 에러(`gh auth refresh -s read:project,project` 안내) 출력하도록 강화. `--reuse-id C25` 경로로 실행 |
<!-- /fx-task-orchestrator-backlog -->
