...
| C82 | C | GOV-001 frontmatter drift 일괄 정비 — `/ax:gov-doc check` (S302+ 2026-04-19) 결과 docs/ 452개 중 PASS 127(28%) / FAIL 275(61%) / NOFM 50(11%). 주요 패턴: (a) `sprint-{N}.{plan,design,analysis,report}.md` ~200건 — autopilot 자동 생성 시 GOV 표준 8필드(code/title/version/status/category/created/updated/author) 미주입, (b) `docs/specs/{project}/review-history.md` ~30건 — req-interview 산출물, frontmatter 비대상 여부 모호 (INDEX.md §"specs/ 디렉토리 구조 가이드" 주석은 scorecard/actionable-items/sixhats만 비대상으로 명시), (c) INDEX.md stale — sprint-77/94 features/ 경로 표시되지만 실제 파일 없음(rename/이동 미반영). 해결: (a) autopilot Plan/Design/Report 생성 단계에 GOV frontmatter 자동 주입 훅, (b) 기존 drift 일괄 보정 스크립트 `scripts/governance/fix-frontmatter.sh` (sprint-* 패턴 → SSOT 추출 후 표준 8필드 주입), (c) review-history.md 비대상 명시 (skill spec + INDEX 주석 갱신), (d) INDEX.md stale 항목 정리 (`/ax:gov-doc index` 재실행). 최소 수정 1차 대응: FX-RPRT-309/310 2건만 표준 보정 완료(이번 세션) (FX-REQ-619, P2) | — | PLANNED | gov-doc orchestrator. infra-selfcheck C8 통과 후 GOV-001 본격 적용 단계 |
<!-- /fx-task-orchestrator-backlog -->
