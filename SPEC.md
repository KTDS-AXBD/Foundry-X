...
| v1.2.0 | Sprint 14: MCP Resources + 멀티 에이전트 동시 PR + Phase 3 기반 (50 endpoints, ~446 tests) | 📋 |
...
| F69 | v1.2.0 릴리스 + Phase 3 기반 구축 — 멀티테넌시 설계 + 로드맵 (FX-REQ-069, P2) | v1.2 | 🔧 | 코드 완료, 배포+설계 문서 미착수 |
...
- [ ] McpResourcesClient — listResources, readResource, subscribeResource (FX-REQ-067)
- [ ] McpRunner 확장 — onNotification, resources/updated 핸들링 (FX-REQ-067)
- [ ] MCP API 4 endpoints (resources/list, templates, read, subscribe) (FX-REQ-067)
- [ ] McpResourcesPanel + ResourceViewer UI (FX-REQ-067)
- [ ] MergeQueueService — enqueue, detectConflicts, calculateMergeOrder, processNext (FX-REQ-068)
- [ ] AgentOrchestrator 확장 — executeParallel, executeParallelWithPr (FX-REQ-068)
- [ ] GitHubService 확장 — getModifiedFiles, updateBranch, getPrStatuses (FX-REQ-068)
- [ ] Agent API 5 endpoints (parallel, queue) (FX-REQ-068)
- [ ] D1 migration 0008 — merge_queue + parallel_executions (FX-REQ-068)
- [ ] MergeQueuePanel + ConflictDiagram + ParallelExecutionForm UI (FX-REQ-068)
- [ ] D1 migration 0007 remote 적용 (Sprint 13 잔여) + 0008 remote (FX-REQ-069)
- [ ] v1.2.0 릴리스 — CHANGELOG + version bump + 프로덕션 배포 (FX-REQ-069)
- [ ] multitenancy.design.md + phase-3-roadmap.md 작성 (FX-REQ-069)
- [ ] E2E 테스트 — Merge Queue + MCP Resources (FX-REQ-069)
- [ ] typecheck + build + tests ~446건 통과
