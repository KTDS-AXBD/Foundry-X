...
| F550 | **Codex CLI 설치 + 리뷰어 프로파일 확립** — 공식 Codex CLI(`@openai/codex` 또는 공식 후속 패키지) 설치 자동화 스크립트(`scripts/setup/install-codex.sh`) + `~/.config/codex/config.toml` 분리 + `~/.claude-squad/profiles/codex-reviewer.md` 역할 지시(read-only reviewer, no git ops, JSON 구조화 출력). PRD: `docs/specs/fx-codex-integration/prd-final.md` §4.1 M1/M2. Phase 46 `Dual-AI Verification` 착수 F-item (FX-REQ-587, P0) | Sprint 300 | 📋(plan) | Option B 압축 배치 — F551과 동일 Sprint. 선결: O1(공식 CLI 명칭) + O2(법인 OpenAI 계정) 확정 |
| F551 | **autopilot Phase 5b Codex cross-review 훅 + Composite Verify Report** — `scripts/autopilot/codex-review.sh`로 PRD+diff+PR 본문을 Codex CLI 호출, 결과를 `.claude/reviews/sprint-{N}/codex-review.json` 저장 + `/ax:code-verify` Gap Analysis 결과와 머지한 Composite Verify Report 생성. BLOCK 판정 시 autopilot 중단 + tmux alert. degraded mode fallback(Codex 장애 시 Claude 단독 진행 + 플래그 기록). PRD §4.1 M3. 전제: F550 (FX-REQ-588, P0) | Sprint 300 | 📋(plan) | F550과 동일 Sprint 통합. 자체 dogfood로 F551 자신의 merge부터 Codex 리뷰 적용 |
| F552 | **Dual AI Review D1 테이블 + `/work-management` 대시보드 위젯** — D1 `dual_ai_reviews`(sprint_id, claude_json, codex_json, divergence_score, decision, created_at) + Hono 라우트 + Work Management 페이지에 "Dual AI Review" 섹션 추가(Sprint별 일치율 + BLOCK 사유 top 5). 4주 관측 데이터 축적 기반. PRD §4.1 M4. 전제: F551 (FX-REQ-589, P1) | Sprint 301 | 📋(plan) | D1 migration 신규 1건(`dual_ai_reviews` 테이블). F541 Offering migration과 충돌 없음 |
| F553 | **4주 관측 회고 + 회귀율 리포트 + 모델 튜닝** — `docs/04-report/phase-46-codex-retro.md` — K1(Hotfix/Revert 감소 ≥50%), K2(Codex BLOCK hit rate ≥60%), K3(false-positive ≤25%), K5(월 비용 ≤$50) 실측 + 튜닝 결정(프로파일/모델/임계치). PRD §4.1 FR-08 + §4.2 S3(Codex-only 리팩토링 Sprint 실험 판단). 선택적: free-code 격리 샌드박스 가이드(S1, 별도 문서 `docs/guides/free-code-sandbox.md`). 전제: F552 + 4주 관측 데이터 (FX-REQ-590, P1) | Sprint 305 | 📋(idea) | Sprint 301 merge 후 4주 경과 예상 지점. 관측 데이터 축적 기다림 |
...
| C56 | C | **Phase 44: D1 격리 실행 (옵션 B)** — 도메인별 테이블 접근 ESLint 룰 + migration 분리 태깅. 범위: (a) `packages/api/src/eslint-rules/no-cross-domain-d1.ts` 신규 — discovery 도메인은 discovery_* 테이블만 접근, shaping은 shaping_*, offering은 offering_* 한정, (b) D1 migration 파일명에 도메인 태그 규칙(`0NNN_discovery_*.sql` / `0NNN_shaping_*.sql`), (c) CI msa-lint 확장 + 자가 PR 통과 | — | 📋(ready) | F540 Shaping 분리 **직전** 선행 권장 — Shaping 테이블 격리 룰이 F540 Design 단계에서 필요. 또는 F540과 병행(C-track) |
