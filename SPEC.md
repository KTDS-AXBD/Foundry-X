...
| F325 | 형상화 재구성 + 버전관리 패턴 — 사업기획서/Offering/PRD/Prototype 4메뉴 + 전 산출물 초안→피드백→버전 업데이트→비교 (FX-REQ-317, P2) | Sprint 142 | 📋 | v1.3 §3.4/§2.1. F322 선행 ✅ |
| F326 | 검증 탭 통합 + 산출물 공유 — 4탭(인터뷰·미팅/본부/전사/임원) + Go/Hold/Drop 의사결정 + 공유 메뉴 (FX-REQ-318, P2) | Sprint 142 | 📋 | v1.3 §3.5. F322 선행 ✅ |
...
| F330 | rules/ 5종 신규 작성 — coding-style, git-workflow, testing, security, sdd-triangle (CLAUDE.md에서 추출, CC 자동 로딩) (FX-REQ-322, P1) | Sprint 146 | 🔧 | PRD: fx-harness-rules/prd-final.md. 독립 트랙. 병렬 가능 |
| F331 | PreToolUse git guard — --no-verify, git add ., force push, reset --hard 차단 (pre-bash-guard.sh + settings.json) (FX-REQ-323, P2) | Sprint 146 | 🔧 | PRD: fx-harness-rules/prd-final.md. F330과 동일 Sprint |
