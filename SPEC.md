...
| F398 | 이벤트 카탈로그 8종 스키마 확정 + EventBus PoC + **Web IA 개편** (`/ax-bd/*` redirect, 사이드바 서비스 경계 그룹, "이관 예정" 라벨, 코어 메뉴 정리) (FX-REQ-390, P1) | Sprint 185 | 🔧 | `packages/shared/events/` + sidebar.tsx + router.tsx |
| F399 | Strangler Fig 프록시 레이어 + harness-kit 이벤트 유틸리티 (FX-REQ-391, P1) | Sprint 186 | 📋 | 프록시 미들웨어 |
| **Sprint 187~188 — M4: 통합 검증 + Production** | | | | |
| F400 | E2E 서비스별 태깅 + IA 변경 E2E 검증 + 전체 회귀 테스트 + Gate-X scaffold PoC (FX-REQ-392, P1) | Sprint 187 | 📋 | PoC 서비스 1개 동작 확인 + IA E2E |
