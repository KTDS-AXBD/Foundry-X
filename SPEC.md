...
| **Phase 11** | 🔧 진행 — IA 대개편 F288~F299 (3/12 완료) | 11-A ✅ (113~114), 11-B 📋 (115~119), 11-C 📋 (118~121) |
| npm | foundry-x@0.5.0 published ✅ |
| typecheck | ✅ |
| build | ✅ |
| lint | ✅ (0 error) |
| tests | API **2432** + CLI **149** + Web **287** = **2868** + E2E (35 specs, ~146 tests) |
...
| F# | 제목 (REQ, Priority) | Sprint | 상태 | 비고 |
|----|----------------------|:------:|:----:|------|
| F291 | Discovery-X Agent 자동 수집 — 시장/뉴스/기술 트렌드 Agent 기반 자동 수집 + 1단계 수집에 배치 (FX-REQ-283, P2) | Sprint 115 | 📋 | G1. F179 기반 확장. 병렬 가능 |
| F292 | 사업계획서 HITL 고도화 — F180 기존 BDP 초안에 HITL 패널 + 멀티 템플릿 지원 추가. 섹션별 승인/수정/반려 (FX-REQ-284, P2) | Sprint 118 | 📋 | G4. F286 HITL 패턴 재활용. F297과 묶음 |
| F293 | 초도 미팅용 Offering — 3단계 형상화에 고객 미팅용 자료 자동 생성 페이지 신규 (FX-REQ-285, P2) | Sprint 119 | 📋 | G5. /shaping/offering. 병렬 가능 |
| F294 | 2-tier 검증 + Pre-PRB 분리 — 본부 검증→전사 검증 순차 워크플로 + F235 ORB/PRB 게이트에서 Pre-PRB 분리. F239 의사결정을 2-tier 확장 (FX-REQ-286, P2) | Sprint 116 | 📋 | G7+G8. F295와 묶음. 병렬 가능 |
| F295 | 전문가 인터뷰/미팅 관리 — 4단계 검증 내 오프라인 활동(전문가 인터뷰, 유관부서 미팅) 기록 + 일정 관리 (FX-REQ-287, P2) | Sprint 116 | 📋 | G9. F294와 묶음 (/validation/*) |
| F296 | 통합 평가 결과서 생성 — 2단계 발굴 스킬 결과(2-1~2-8) 종합 보고서 자동 생성 + F261 산출물 연동 (FX-REQ-288, P2) | Sprint 117 | 📋 | G12. /discovery/report. 병렬 가능 |

#### Phase 11-C: 고도화 + GTM

| F# | 제목 (REQ, Priority) | Sprint | 상태 | 비고 |
|----|----------------------|:------:|:----:|------|
| F297 | Prototype HITL 고도화 — F181 기존 Prototype 생성에 HITL 패널 추가 + 다중 프레임워크 지원 (FX-REQ-289, P3) | Sprint 118 | 📋 | G6. F292와 묶음 (HITL 공유). 배치 2 |
| F298 | PoC 관리 분리 — MVP 추적(F238)과 별도로 PoC 전용 진행 추적 + 성과 측정 (FX-REQ-290, P3) | Sprint 120 | 📋 | G10. /product/poc. 병렬 가능. 배치 2 |
| F299 | 대고객 선제안 GTM — 6단계 GTM에 고객 선제안 워크플로 + 제안서 자동 생성 (FX-REQ-291, P4) | Sprint 121 | 📋 | G11. /gtm/outreach. 범위 확정 후 착수. 배치 3 단독 |
