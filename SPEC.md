...
| F554 | **Phase 46 hotfix — Codex 실설치 + autopilot Phase 5b 훅 배선 + dogfood Smoke Reality** — (a) `scripts/setup/install-codex.sh` user-level npm prefix 재작성(sudo 제거) + OpenRouter `base_url` 오버라이드 실검증, (b) `sprint-autopilot` skill Verify 단계에 `scripts/autopilot/codex-review.sh` 호출 배선 + BLOCK verdict 시 autopilot 중단, (c) Sprint 302 자체 PR이 Phase 5b 통과 + `.claude/reviews/sprint-302/codex-review.json` **실측 생성(degraded=false) 확증**, (d) `docs/retrospective/phase-46-dogfood-failure.md` 회고 추가. 근거: F550/F551 Dead Code 실측(S300, 2026-04-16). Phase 43 F534/F536 hotfix와 동형 패턴 — Design Stage Exit D1(주입 사이트 전수 검증) 위반 재발 (FX-REQ-591, P0) | Sprint 302 | 📋(plan) | 선결 없음 (Master Codex v0.120.0 설치 완료) |
