...
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 583테스트 ✅ (API 패키지) + CLI 112 + Web 48 + ~55 E2E |
| API endpoints | 111개 (25 routes) |
| API services | 45개 |
| D1 tables | 33개 (migrations 0001~0020) |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F135 | OpenRouter 게이트웨이 통합 — OpenRouterRunner 구현, 단일 API 키로 300+ 모델 접근 (FX-REQ-135, P0) | v3.0 | 📋 | Agent Evolution A1 |
| F136 | 태스크별 모델 라우팅 — task_type별 최적 모델 자동 선택, D1 routing rules 테이블 (FX-REQ-136, P0) | v3.0 | 📋 | Agent Evolution A2 |
| F137 | Evaluator-Optimizer 패턴 — 생성→평가→개선 루프, 최대 반복+품질 임계값 (FX-REQ-137, P0) | v3.0 | 📋 | Agent Evolution A3 |
| F138 | ArchitectAgent — 설계 문서 검토 + 아키텍처 판단 + 의존성 분석 (FX-REQ-138, P0) | v3.0 | 📋 | Agent Evolution A4 |
| F139 | TestAgent — 변경 코드 기반 테스트 자동 생성 + 커버리지 분석 (FX-REQ-139, P0) | v3.0 | 📋 | Agent Evolution A5 |
| F140 | SecurityAgent — OWASP Top 10 보안 취약점 스캔 + PR diff 분석 (FX-REQ-140, P1) | v3.0 | 📋 | Agent Evolution A6 |
| F141 | QAAgent 브라우저 테스트 — Playwright/Chromium 실제 UI 테스트 실행 (FX-REQ-141, P1) | v3.0 | 📋 | Agent Evolution A7 |
| F142 | Sprint 워크플로우 템플릿 — Think→Plan→Build→Review→Test→Ship→Reflect DAG (FX-REQ-142, P1) | v3.0 | 📋 | Agent Evolution A8 |
| F143 | 모델 비용/품질 대시보드 — 에이전트별 토큰 사용량, 비용, 품질 점수 시각화 (FX-REQ-143, P1) | v3.0 | 📋 | Agent Evolution A9 |
