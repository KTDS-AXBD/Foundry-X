...
| F550 | **Codex CLI 설치 + 리뷰어 프로파일 확립** (FX-REQ-587, P0) | Sprint 300 | ✅(partial) | PR #606 merge. install-codex.sh sudo 결함 + `codex` 실설치 미검증(Master 수동 설치 v0.120.0)로 merge됨 → **F554 hotfix 대상** |
| F551 | **autopilot Phase 5b Codex cross-review 훅 + Composite Verify Report** (FX-REQ-588, P0) | Sprint 300 | ✅(partial) | PR #606 merge. 스크립트(`codex-review.sh`)만 존재, **autopilot skill Phase 5b 훅 배선 누락** → Dead Code 상태, Sprint 300 자체 dogfood 0건 → **F554 hotfix 대상** |
| F552 | **Dual AI Review D1 테이블 + `/work-management` 대시보드 위젯** (FX-REQ-589, P1) | Sprint 303 | 📋(plan) | F554 hotfix 완료 후로 연기 (기존 Sprint 301 → 303). D1 migration 신규 1건 |
| F553 | **4주 관측 회고 + 회귀율 리포트 + 모델 튜닝** (FX-REQ-590, P1) | Sprint 307 | 📋(idea) | F552 merge 후 4주 관측 데이터 기반 (기존 Sprint 305 → 307) |
| F554 | **Phase 46 hotfix — Codex 실설치 + autopilot Phase 5b 훅 배선 + dogfood Smoke Reality** — (a) `scripts/setup/install-codex.sh` user-level npm prefix 재작성(sudo 제거) + OpenRouter `base_url` 오버라이드 실검증, (b) `sprint-autopilot` skill Verify 단계에 `scripts/autopilot/codex-review.sh` 호출 배선 + BLOCK verdict 시 autopilot 중단, (c) Sprint 302 자체 PR이 Phase 5b 통과 + `.claude/reviews/sprint-302/codex-review.json` **실측 생성(degraded=false) 확증**, (d) `docs/retrospective/phase-46-dogfood-failure.md` 회고 추가. 근거: F550/F551 Dead Code 실측(S300, 2026-04-16). Phase 43 F534/F536 hotfix와 동형 패턴 — Design Stage Exit D1(주입 사이트 전수 검증) 위반 재발 (FX-REQ-591, P0) | Sprint 302 | 🔧(design) | 선결 없음 (Master Codex v0.120.0 설치 완료) |
