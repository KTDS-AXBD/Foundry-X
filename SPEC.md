...
| B1 | B | **fx-gateway 정적 경로 라우팅 충돌 — biz-items 3개 경로 fx-discovery 이전** — F539c에서 fx-gateway가 `/api/biz-items/:id` 패턴을 DISCOVERY로 라우팅하면서 정적 경로 3개(summary/portfolio-list/by-artifact)가 `:id`에 매칭되어 fx-discovery로 오유입 → fx-discovery에 핸들러 없어 JWT 미들웨어 401 또는 `:id` 핸들러가 id="summary" 등으로 오동작. 증상: 로그인 후 `/dashboard`에서 `GET /api/biz-items/summary 401` 반복. 해법(옵션 B): 정적 경로 3개를 fx-discovery로 이전 + PortfolioService 2개 메서드 선택 포팅, TDD Red→Green 6 tests. packages/api 원본은 dead code 주석 + Sprint 299 cleanup 예정 | — | DONE | PR #602 (`d0dbacf6`, +388/-0, 4 files). TDD 6/6 PASS, turbo test+typecheck+lint 12 packages PASS. deploy.yml 24483297868 success. Prod smoke: fx-gateway 3 paths 모두 fx-discovery realm 확증 (JWT 미들웨어 응답). 4/17 09:00 리허설 전 브라우저 로그인 검증 필요 |
<!-- /fx-task-orchestrator-backlog -->
