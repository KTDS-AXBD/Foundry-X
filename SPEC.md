...
- **Phase**: Phase 2 완료 (API + Web + 에이전트 오케스트레이션) — Sprint 17 완료
- **Version**: 1.5.0
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F90 | PlannerAgent gatherExternalToolInfo() 프롬프트 연동 — MCP 도구 정보 수집 + LLM 프롬프트 주입 (FX-REQ-090, P2) | v1.5+ | 🔧 | Plan FX-PLAN-021 + Design FX-DSGN-020 |
...
- [ ] gatherExternalToolInfo() 메서드 구현 — mcpRegistry active 서버 도구 수집 (FX-REQ-090)
- [ ] PLANNER_SYSTEM_PROMPT 확장 — external_tool type + externalTool 스키마 가이드 (FX-REQ-090)
- [ ] analyzeCodebase() 수정 — 도구 목록 프롬프트 주입 (FX-REQ-090)
- [ ] 테스트 7건 — mcpRegistry mock + external_tool 파싱 검증 (FX-REQ-090)
- [ ] typecheck + lint + 기존 테스트 통과 (FX-REQ-090)
