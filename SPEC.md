...
| C18 | C | E2E smoke 통합 단건 — lib.sh/task-start/task-complete/daemon guard full pipeline dogfood (tmux 3.5a 회귀 검증) (FX-REQ-512) | — | 🔧 | S258 dogfood. C14~C17 CLOSED_EMPTY 후속 — 원인(tmux 3.4 `pane-border-status top` + Unicode pane title + user-data 조합 segfault) 파악 후 tmux 3.5a server 환경에서 smoke A+B 1회 통합 회귀 검증. **Assertion**: ①worker 파일 작성(additions>0) ②pane 생존(list-panes에 worker pane ID 존재 + 💀 소멸 로그 부재) ③비-empty commit + daemon auto-merge 성공 ④S257 fix 회귀(FX_SIGNAL_DIR SSOT / AUTONOMY_RULE / task-complete fail-fast / daemon PRESERVED guard 정상 동작). 성공 시 `feedback_tmux34_bug.md`에 "tmux 3.5a 활성화 후 검증 성공" 한 줄 추가. 실패 시 S258 🔧 "meta-only empty commit detection" TD 등록 근거로 사용. **실행 조건**: tmux 3.5a server (현 Master 3.4 → 새 master 세션 이동 필수) |
| C19 | C | C18 smoke 통합 단건 dogfood (FX-REQ-513) | — | PLANNED | task orchestrator |
<!-- /fx-task-orchestrator-backlog -->
