...
- **Phase**: Phase 2 (API + Web + 에이전트 오케스트레이션) — Sprint 8 진행 중
- **Version**: 0.6.0
...
| v0.8.0 | Sprint 8: API 실데이터 완성 + SSE + NL→Spec + Wiki Git 동기화 | 📋 |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F44 | SSE 실시간 통신 (FX-REQ-044, P1) | v0.8 | 📋 | Agent/Sync 상태 실시간 스트리밍 |
| F45 | NL→Spec 변환 (FX-REQ-045, P0) | v0.8 | 📋 | 자연어 → 구조화 명세 변환 (LLM 통합) |
| F46 | Wiki Git 동기화 (FX-REQ-046, P2) | v0.8 | 📋 | Wiki CRUD ↔ Git 리포 양방향 동기화 |
| F47 | Production Site Design — fx.minu.best 랜딩+대시보드 통합 (FX-REQ-047, P1) | v0.8 | 📋 | URL 설정 + 메뉴 구성 + 기능 매핑 + UI/UX 디자인 |
...
- [ ] requirements GitHub API + KV 캐시 (FX-REQ-041 잔여)
- [ ] health/integrity/freshness 실데이터 전환 (FX-REQ-041 잔여)
- [ ] SSE 실시간 통신 — Agent/Sync 상태 스트리밍 (FX-REQ-044)
- [ ] NL→Spec 변환 — LLM 통합 파이프라인 (FX-REQ-045)
- [ ] Wiki Git 동기화 — D1 ↔ Git 양방향 (FX-REQ-046)
- [ ] Production Site Design — fx.minu.best 랜딩+대시보드 통합 (FX-REQ-047)
- [ ] Workers 프로덕션 재배포
