...
- **Phase**: Phase 2 (API + Web + 에이전트 오케스트레이션) — Sprint 9 완료, Sprint 10 계획
- **Version**: 0.9.0
...
| v0.10.0 | Sprint 10: 에이전트 실연동 + NL→Spec 충돌 감지 | 📋 |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F52 | 프로덕션 실배포 실행 (FX-REQ-052, P0) | v0.10 | 📋 | Workers secrets + D1 migration remote + deploy + smoke test 검증 |
| F53 | 에이전트 실연동 — Claude API + MCP 어댑터 인터페이스 (FX-REQ-053, P0) | v0.10 | 📋 | AgentRunner 추상화 + ClaudeApiRunner + MCP 인터페이스 설계 + agent_sessions 실데이터 |
| F54 | NL→Spec 충돌 감지 + 사용자 선택 (FX-REQ-054, P1) | v0.10 | 📋 | 기존 명세 시맨틱 diff + 충돌 표시 UI + 수락/거절/수정 |
...
- [ ] Workers secrets 설정 + D1 migration remote 적용 (FX-REQ-052)
- [ ] Workers + Pages 프로덕션 배포 + smoke test 검증 (FX-REQ-052)
- [ ] AgentRunner 추상화 계층 + ClaudeApiRunner 구현 (FX-REQ-053)
- [ ] MCP 어댑터 인터페이스 설계 (FX-REQ-053)
- [ ] 에이전트 실행 → agent_sessions/tasks 실데이터 기록 + SSE 전파 (FX-REQ-053)
- [ ] NL→Spec 기존 명세 충돌 감지 엔진 (FX-REQ-054)
- [ ] 충돌 표시 + 사용자 선택 UI (수락/거절/수정) (FX-REQ-054)
