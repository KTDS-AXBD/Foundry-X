...
- **Phase**: **Phase 18** ✅ 완료 — Offering Pipeline F363~F383 | **Phase 19** ✅ ��료 — Builder Evolution F384~F391 | **Phase 20** ✅ 완료 — AX BD MSA F392~F401 (Match 94%) | **Phase 21** 🔧 진행 중 — Gate-X 독립 서���스 F402~F413 (Sprint 189~197) | **Phase 22** 📋 계획 — Offering Skill v2 + Builder v2 F414~F431 (Sprint 198~207, 18건)
- **Sprint**: 148~164 (✅)
...
> ```
> **마지막 실측** (Sprint 186, 2026-04-07): ~118 routes (flat 8 + core 71 + modules 39), ~262 services, ~133 schemas, D1 0114, tests ~3168 (E2E 263)
...
| F409 | 커스텀 검증 룰 엔진 — 사용자 정의 루브릭 + 검증 기준 관리 (FX-REQ-401, P1) | Sprint 193 | 🔧 | Gate-X 핵심 차별점 |
| F410 | 외부 웹훅 연동 + 멀티테넌시 격리 — 테넌트별 데이터/API 분리 (FX-REQ-402, P2) | Sprint 194 | 📋 | Slack/Teams/HTTP 알림 + RBAC 확장 |
| **Phase 21-D: SaaS 기반 (M4, P2)** | | | | |
| F411 | 과금 체계 — API 호출량 추적 + 요금제 (Free/Pro/Enterprise) (FX-REQ-403, P2) | Sprint 195 | 📋 | 사용량 메트릭 + 결제 연동 |
| F412 | SDK/CLI 클라이언트 — TypeScript SDK + CLI 도구 + API 문서 (FX-REQ-404, P2) | Sprint 196 | 📋 | npm 패키지 배포 |
| **Phase 21-E: Foundry-X 사전 정리 (M5, P2)** | | | | |
| F413 | Foundry-X 수집 코드 격리 — `core/collection/` 분리 (4 routes + 5 services + 5 schemas) Discovery-X 이관 사전 정리 (FX-REQ-405, P2) | Sprint 197 | 🔧 | core/discovery + core/shaping에서 14개 파일 격리. Strangler Fig 사전 작업 |
...
| F416 | 발굴 산출물 자동 매핑 — 2-0~2-8 단계별 섹션 매핑 테이블 (FX-REQ-408, P0) | Sprint 199 | 📋 | 가이드 §2 매핑 기반 |
| F417 | 경영 언어 원칙 적용 — 톤/표현 규칙 스킬 로직 내장 (FX-REQ-409, P0) | Sprint 199 | 📋 | "~할 수 있다"→"~를 추진", "최초" 금지 등 |
| **Phase 22-B: KT 연계 + 교차검증 (M2, P0)** | | | | |
| F418 | KT 연계 원칙 강제 — 추진배경 3축 필수 체크 + KT 미연계 경고 (FX-REQ-410, P0) | Sprint 200 | 📋 | 수익성/KT적합성/실행력 3축 |
| F419 | GAN 교차검증 자동화 — 표준 질문 풀 + 추진론/반대론/판정 자동 생성 (FX-REQ-411, P0) | Sprint 200 | 📋 | 냉철한 톤 강제, No-Go 판정 가능 |
| **Phase 22-C: 확장 기능 (M3, P1)** | | | | |
| F420 | PPTX 변환 — HTML→PPTX 변환 + offering-pptx 스킬 체이닝 (FX-REQ-412, P1) | Sprint 201 | 📋 | 기존 offering-pptx 연계 |
| F421 | 버전 이력 추적 — v0.1→v1.0 변경 이력 + diff 보기 (FX-REQ-413, P1) | Sprint 201 | 📋 | review-history.md 자동 생성 |
| F422 | 피드백 반영 자동화 — 임원 피드백→섹션별 자동 수정 + 변경 마커 (FX-REQ-414, P1) | Sprint 202 | 📋 | 고객 유형별 톤 자동 조정 포함 |
| **Phase 22-D: Prototype Builder v2 (M4, P0) — PRD: `docs/specs/fx-builder-v2/prd-final.md`** | | | | |
| F423 | impeccable 디자인 스킬 통합 — 7도메인 참조문서 + 안티패턴을 Generator 프롬프트에 주입 (FX-REQ-415, P0) | Sprint 203 | 📋 | 타이포/색상/공간/반응형 4도메인 우선 |
| F424 | 디자인 안티패턴 차단 — impeccable 안티패턴 목록을 Discriminator에 추가, 재생성 트리거 (FX-REQ-416, P0) | Sprint 203 | 📋 | 과용 폰트/순수 흑색/카드 중첩 등 |
| F425 | PRD 정합성 LLM 판별 — prd 차원 키워드→LLM 의미 비교 교체 (FX-REQ-417, P0) | Sprint 204 | 📋 | PRD 요구사항 vs 프로토타입 기능 대조 |
| F426 | 5차원 LLM 통합 판별 — 체크리스트→LLM 평가 전환 (FX-REQ-418, P0) | Sprint 204 | 📋 | 정적 분석은 보조 데이터로 유지 |
| F427 | Vision API 시각 평가 — 스크린샷 기반 UI 품질 LLM 평가 (FX-REQ-419, P0) | Sprint 205 | 📋 | 레이아웃/색상/타이포/계층구조/반응형 |
| F428 | max-cli 본격 통합 — WSL CLI→Builder 파이프라인 연결 + API fallback (FX-REQ-420, P0) | Sprint 205 | 📋 | --bare 모드, rate limit 백오프 |
| F429 | max-cli 큐 관리 — 단일 머신 빌드 큐잉 + 순차 실행 + 타임아웃 (FX-REQ-421, P0) | Sprint 206 | 📋 | 동시 빌드 방지 |
| F430 | 디자인 커맨드 파이프라인 — /audit→/normalize→/polish O-G-D 루프 매핑 (FX-REQ-422, P1) | Sprint 206 | 📋 | impeccable 커맨드 체계 도입 |
| F431 | 판별 피드백 구체화 — LLM 판별→구체적 수정 지시 변환 + Generator 자동 주입 (FX-REQ-423, P1) | Sprint 207 | 📋 | 루프 수렴 속도 개선 |
