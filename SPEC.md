...
| Sprint 145 | 📋 (F329) — Blueprint 랜딩 페이지 전면 전환 | 독립 트랙. FX-PLAN-015 + FX-DSGN-015 |
...
| F# | 설명 | Sprint | 상태 | 비고 |
|----|------|:------:|:----:|------|
| F322 | 사이드바 구조 재설계 — 25→12 메뉴 축소, processGroups 전면 재구성, 수집 TBD 접기, 하단 고정(위키+설정), Admin 역할 분리 7메뉴 (FX-REQ-314, P0) | Sprint 139 | 📋 | v1.3 §3.1/§3.2/§3.8/§3.9/§6.1. F322 선행 — 전체 구조 기반 |
| F323 | 대시보드 ToDo List + 업무 가이드 — 파이프라인 현황 + 아이템별 단계 표시 + 체크리스트 + 의사결정 대기 알림 + 4단계 검증·제품화·개발 가이드 (FX-REQ-315, P1) | 📋 | 📋 | v1.3 §5.1/§5.2 |
| F324 | 발굴 탭 통합 — 3탭(대시보드/프로세스/BMC) + 멀티 페르소나 프로세스 내 통합 + 평가 결과서 + 스킬카탈로그→위키 이동 (FX-REQ-316, P1) | 📋 | 📋 | v1.3 §3.3/§4 |
| F325 | 형상화 재구성 + 버전관리 패턴 — 사업기획서/Offering/PRD/Prototype 4메뉴 + 전 산출물 초안→피드백→버전 업데이트→비교 (FX-REQ-317, P2) | 📋 | 📋 | v1.3 §3.4/§2.1 |
| F326 | 검증 탭 통합 + 산출물 공유 — 4탭(인터뷰·미팅/본부/전사/임원) + Go/Hold/Drop 의사결정 + 공유 메뉴 (FX-REQ-318, P2) | 📋 | 📋 | v1.3 §3.5 |
| F327 | 제품화 탭 통합 + Offering Pack — MVP/PoC 2탭 + Offering Pack 버전관리 (FX-REQ-319, P3) | 📋 | 📋 | v1.3 §3.6 |
| F328 | 시작하기 통합 + 공통 메뉴 정리 — 5영역(온보딩+스킬가이드+Cowork+데모+도구가이드) + 팀공유→산출물 흡수 (FX-REQ-320, P3) | 📋 | 📋 | v1.3 §3.1/§4 |
| **Blueprint 랜딩 페이지 비주얼 전환 — 설계도 메타포 디자인** | | | | |
| F329 | Blueprint 랜딩 페이지 전면 전환 — landing.tsx 7개 섹션 설계도 스타일 리디자인 (플로우차트 Process, 시스템 다이어그램 Architecture, Gantt Roadmap, 회로도 Agents), bp-* CSS 적용, 다크 모드, 반응형 유지 (FX-REQ-321, P2) | Sprint 145 | 📋 | 독립 트랙. design-shotgun variant-C 승인 (세션 #198). FX-PLAN-015 + FX-DSGN-015 |
