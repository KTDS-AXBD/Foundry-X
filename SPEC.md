...
- **Phase**: **Phase 14** 🔧 진행 중 — Agent Orchestration Infrastructure F333~F337 | **Phase 15** 📋 등록 — Discovery UI/UX 고도화 F342~F350
- **Sprint**: 148 (✅ 완료) — TaskState Machine (F333), PR #284
- **다음**: Phase 14: Sprint 149~152 (F334~F337) | Phase 15: Sprint 154~157 (F342~F350, 독립 병렬)
...
| Sprint 150 | 📋 (F335) — Orchestration Loop: 3모드(retry/adversarial/fix) + FeedbackLoopContext + AgentAdapter + 텔레메트리 미들웨어 + E2E ~35 tests | Phase 14 Foundation v3. F334 선행 |
| Sprint 151 | 📋 (F336) — Agent Adapter 통합: 기존 에이전트 AgentAdapter 래핑 + Discriminator 역할 태깅 + handleFeedback() | Phase 14 Feature. F335 선행 |
| Sprint 152 | 📋 (F337) — Orchestration Dashboard: 태스크 상태 Kanban + 루프 이력 뷰 + 텔레메트리 대시보드 | Phase 14 Feature. F335 선행 |
| Sprint 154 | 🔧 (F342+F343) — DB 스키마 확장 4테이블(0096~0099) + 강도 라우팅 UI + output_json 렌더링 POC | Phase 15. 독립 착수 |
| Sprint 155 | 📋 (F344+F345) — 멀티 페르소나 평가 UI 6컴포넌트 + Claude SSE 평가 엔진 + 데모 모드 | Phase 15. F342 선행 |
| Sprint 156 | 📋 (F346+F347) — 리포트 공통 컴포넌트 + 9탭 프레임 + 선 구현 4탭(2-1~2-4) | Phase 15. F342 선행 |
| Sprint 157 | 📋 (F348+F349+F350) — 나머지 5탭 + 팀 검토 Handoff + 공유 링크 + PDF Export | Phase 15. F347 선행 |
...
> ```
> ls packages/api/src/routes/ | wc -l     # API routes
> ls packages/api/src/services/ | wc -l   # API services
> ls packages/api/src/schemas/ | wc -l    # API schemas
...
| F335 | Orchestration Loop — 3모드(retry/adversarial/fix) OrchestrationLoop 구현 + FeedbackLoopContext(진입/탈출 관리) + AgentAdapter 인터페이스 + 텔레메트리 수집 미들웨어(Event Bus 구독→D1) + E2E 통합 테스트(Hook→Event→Transition→Loop→Telemetry) ~35건. GAN 잔여이슈 N1(FeedbackContext), N4(Guard서비스), N5(수렴기준) 해결 포함. ~1150 LOC (FX-REQ-327, P0) | Sprint 150 | 📋 | Foundation v3. F334 선행. **전체 사이클 최초 동작** |
| **Feature (기존 에이전트 통합 + 관측성)** | | | | |
| F336 | Agent Adapter 통합 — 기존 에이전트(deploy-verifier, spec-checker, build-validator 등)를 AgentAdapter 인터페이스로 래핑 + Discriminator 역할 태깅(YAML frontmatter role 추가) + handleFeedback() 점진적 추가. 기존 동작 100% 보존, 인터페이스만 추가 (FX-REQ-328, P1) | Sprint 151 | 📋 | Feature. F335 선행. Phase B 마이그레이션 |
| F337 | Orchestration Dashboard — 태스크 상태 뷰(Kanban, 상태머신 시각화) + 루프 이력 뷰(라운드별 품질 점수 추이) + 텔레메트리 대시보드(태스크/스킬/라운드별 비용) (FX-REQ-329, P1) | Sprint 152 | 📋 | Feature. F335 선행. 병렬 가능(F336과) |
| F338 | SPA 라우팅 404 해결 — _redirects catch-all 미동작 근본 원인 분석 + wrangler.toml 연관 확인 + Cloudflare Pages 라우팅 설정 정비 (FX-REQ-330, P1) | Sprint 153 | 📋 | 운영이슈. 독립 트랙. #280 |
| F339 | Marker.io Capture Failed 해결 — 브라우저 확장 프로그램 충돌 원인 분석 + 시크릿 모드 테스트 + 대안 스크린샷 방식 검토 (FX-REQ-331, P1) | Sprint 153 | 📋 | 운영이슈. F309 후속. #281 |
| F340 | JWT 토큰 갱신 — feedback-consumer 장기 토큰 또는 자동 갱신 메커니즘 구현 (FX-REQ-332, P1) | Sprint 153 | 📋 | 운영이슈. 인증 강화. #282 |
| F341 | TinaCMS Navigation 컬렉션 표시 — admin에서 Navigation 미표시 원인 분석 + JSON collection 지원 확인 + 설정 보정 (FX-REQ-333, P1) | Sprint 153 | 📋 | 운영이슈. F311 후속. #283 |
...
| F342 | DB 스키마 확장 — D1 마이그레이션 4건(0096~0099): ax_persona_configs + ax_persona_evals + ax_discovery_reports + ax_team_reviews + API 3 서비스(PersonaConfigService, PersonaEvalService, DiscoveryReportService) + Zod 스키마 (FX-REQ-334, P0) | Sprint 154 | 🔧 | Foundation. 독립 착수 가능 |
| F343 | 유형별 강도 라우팅 UI — WizardStepDetail 확장: intensity indicator(★핵심/○보통/△간소) + 5유형×7단계 매트릭스 시각화 + 간소 단계 스킵 옵션 + output_json 렌더링 POC(기존 데이터 2탭 검증) (FX-REQ-335, P0) | Sprint 154 | 🔧 | F342 선행 |
| **Sprint 155 — AI 멀티 페르소나 평가 (핵심)** | | | | |
| F344 | 멀티 페르소나 평가 UI — PersonaCardGrid(8개 2×4) + WeightSliderPanel(7축 합계 100% 자동보정) + ContextEditor(Split Pane) + BriefingInput(2-1~2-8 자동 요약) + recharts 설치 (FX-REQ-336, P0) | Sprint 155 | 📋 | F342 선행. recharts 신규 의존성 |
| F345 | 멀티 페르소나 평가 엔진 — POST /ax-bd/persona-eval(Claude SSE) + EvalProgress(8단계 순차) + EvalResults(점수+판정+Radar+요약) + 데모 모드(하드코딩 fallback) (FX-REQ-337, P0) | Sprint 155 | 📋 | F344 선행. Claude API Dry-run |
| **Sprint 156 — 발굴 완료 리포트 (9탭 중 4탭 선 구현)** | | | | |
| F346 | 리포트 공통 컴포넌트 + 프레임 — StepHeader + InsightBox + MetricCard + NextStepBox + HITL Badge + 9탭 프레임(discovery-* 시맨틱 토큰) + GET /ax-bd/discovery-report/:itemId (FX-REQ-338, P0) | Sprint 156 | 📋 | F342 선행 |
| F347 | 리포트 탭 4종 선 구현 — ReferenceAnalysisTab(2-1) + MarketValidationTab(2-2) + CompetitiveLandscapeTab(2-3) + OpportunityIdeationTab(2-4). TAM 도넛, Porter Radar, BMC 그리드 (FX-REQ-339, P0) | Sprint 156 | 📋 | F346 선행 |
| **Sprint 157 — 나머지 탭 + 팀 검토 + Export** | | | | |
| F348 | 리포트 탭 5종 완성 — OpportunityScoringTab(2-5) + CustomerPersonaTab(2-6) + BusinessModelTab(2-7) + PackagingTab(2-8) + PersonaEvalResultTab(2-9) (FX-REQ-340, P1) | Sprint 157 | 📋 | F347 선행 |
| F349 | 팀 검토 & Handoff — TeamReviewPanel(Go/Hold/Drop 투표+코멘트) + Executive Summary 자동 생성 + Decision Record + Handoff Checklist(형상화 진입 조건) (FX-REQ-341, P0) | Sprint 157 | 📋 | F346 선행 |
| F350 | 리포트 공유 + PDF Export — 읽기전용 토큰 기반 공유 링크 + html2canvas 클라이언트 PDF 생성 (FX-REQ-342, P1) | Sprint 157 | 📋 | F346 선행. 병렬 가능(F348과) |
