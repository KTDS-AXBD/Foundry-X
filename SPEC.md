...
- **Phase**: Phase 2 (API + Web + 에이전트 오케스트레이션) — Sprint 9 진행 중
- **Version**: 0.8.0
...
| build | ✅ |
| lint | ✅ (0 error) |
| tests | 48파일 241테스트 ✅ (CLI 106 + API 101 + Web 34) |
...
| v0.9.0 | Sprint 9: 프로덕션 배포 + E2E + 에이전트 오케스트레이션 | 📋 |
...
| F# | 제목 (REQ, Priority) | 버전 | 상태 | 비고 |
|----|----------------------|:----:|:----:|------|
| F48 | 프로덕션 배포 파이프라인 완성 (FX-REQ-048, P0) | v0.9 | 📋 | Workers secrets + D1 migration remote + Pages deploy + 배포 Runbook |
| F49 | E2E 테스트 인프라 + 크리티컬 패스 (FX-REQ-049, P1) | v0.9 | 📋 | Playwright setup + login→dashboard flow + API 통합 테스트 |
| F50 | 에이전트 오케스트레이션 기초 (FX-REQ-050, P1) | v0.9 | 📋 | Real capability 정의 + Constraint 강제 + 브랜치 기반 격리 |
| F51 | 옵저버빌리티 + 배포 후 검증 (FX-REQ-051, P2) | v0.9 | 📋 | Smoke test + 에러 추적 + health check 강화 |
...
- [ ] Workers secrets 설정 + D1 migration remote 적용 (FX-REQ-048)
- [ ] Pages deploy job 복원 + 배포 Runbook 작성 (FX-REQ-048)
- [ ] Playwright E2E 인프라 설정 + CI 통합 (FX-REQ-049)
- [ ] 크리티컬 패스 E2E: login→dashboard→agents→spec-generator (FX-REQ-049)
- [ ] API 통합 테스트 — 서비스 간 호출 검증 (FX-REQ-049)
- [ ] 에이전트 Capability 실 정의 + Constraint 강제 로직 (FX-REQ-050)
- [ ] 에이전트 브랜치 기반 격리 — PR 기반 작업 흐름 (FX-REQ-050)
- [ ] Smoke test + health check 강화 (FX-REQ-051)
- [ ] Workers + Pages 프로덕션 배포 검증 (FX-REQ-048)
