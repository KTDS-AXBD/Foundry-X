...
| F509 | fx-work-observability Walking Skeleton — Backlog/REQ/Task/Sprint/Epic 4-channel 통합 뷰(Web UI + JSON API + CLI + Live feed) + 자연어→REQ 자동 분류 파이프라인 (FX-REQ-526, P0) | Sprint 261 | 📋 | [PRD: docs/specs/fx-work-observability/prd-v1.md]. Walking Skeleton M1~M4: M1=JSON aggregator(`SPEC.md + /tmp/task-signals + git log + gh pr list`), M2=Static Kanban(`/work-management` route), M3=Context resume(최근 N commit + WT + daemon log), M4=Claude Sonnet inline NL classifier. Out-of-scope: 편집UI(read-only), RBAC(혼자 모드). Hotfix 4~8시간 예산. Fallback: LLM→regex, WebUI→API-only. 성공 기준: End-to-end 시나리오 S1(§5.2.1) 완주 + 주관 "보인다" 인지. Phase 2 Round 1 73/100 Conditional + ChatGPT flaw high 수동 보강 (review/round-1/, review-history.md v1 revised) |
