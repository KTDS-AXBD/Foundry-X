#!/bin/bash
# PostToolUse hook: warn if .test.ts file is missing for edited .ts/.tsx file
# Non-blocking (exit 0) — warning only, does not prevent edits
...
# Skip test files, type declarations, index/barrel files, and type-only files
...
if ! echo "$FILE" | grep -q '/packages/'; then exit 0; fi
...
if [ ! -f "$TEST_FILE" ] && [ ! -f "$TEST_FILE_TSX" ]; then
  echo "⚠️  테스트 파일 없음: ${STEM}.test.ts — /tdd red ${FILE} 로 생성 가능"
fi
