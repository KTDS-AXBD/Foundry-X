...
2. O-G-D Orchestrator 호출 — task: "2-5 Commit Gate 종합 검증", rubric: "bd-discovery", max_rounds: 2
...
- **2-7**: Rubric R4(수익 모델) 가중치 0.15→0.25, R7(파트너십) 0.10→0.15 유지
...
- **팀원 독립 사용**: 팀원이 직접 발굴 프로세스를 실행하려면 `CLAUDE_AXBD` 폴더(76 skills + 36 commands) 사용. 위치: `docs/specs/axbd-skill/CLAUDE_AXBD/` 또는 GitHub clone
