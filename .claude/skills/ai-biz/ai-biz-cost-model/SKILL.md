...
name: cost-model
description: |
  AI 제품 원가 구조 분석 — 데이터, 학습, 추론, 인프라 비용 산출
  Use when: AI 원가, 비용 구조, GPU 비용, 추론 비용, cost structure, AI cost, inference cost, unit economics
  Triggers: AI 원가, 비용 구조, GPU 비용, 추론 비용, cost structure, AI cost, inference cost, unit economics
  Do NOT use for: 매출/수익 분석 (BM 정의는 별도 단계 2-7)
user-invocable: true
...
- **추론 비용은 모델 세대교체마다 급변 — 분석 시점의 가격을 명시하고, 6개월 후 재검증 필요. GPU 단가는 클라우드 리전별로 2~3배 차이**
