...
Phase 1~46 완료 + Phase 47 진행 (Sprint 1~340, F595 ✅). 상세: SPEC.md §5
...
- **Phase 47: Discovery 인프라 회복 + GAP 시리즈** 🔧 (F582~F595, Sprint 328~340). F582 ✅ Sprint 328 (GAP-4 Discovery 인프라 회복). F585 + F586 ✅ Sprint 332 (services/ 루트 agent 7 cleanup + GAP-2 output_tokens=0 fix). F587 ✅ Sprint 333 (services/ dead code 2 git rm + 도메인 이동 2). F588 ✅ Sprint 334 (work 도메인 본격 분리, routes 2 + services 2 → core/work/). F589 ✅ Sprint 335 (worktree-manager → core/harness/services/, 옵션 A 15회차, autopilot ~8분). F590 ✅ Sprint 336 (pm-skills dead code 2 git rm + guide → core/discovery/services/, 옵션 B F587 패턴 재현, autopilot 7분 37초). F591 ✅ Sprint 337 (prd/prototype 5 files → core/offering/services/, 옵션 A1 16회차 정착화, autopilot self-evaluation Match 100%, services/ 23→18). F592 ✅ Sprint 338 (services/ dead+sibling 2 files 정리, 옵션 D-a 보수, entity-sync rm + methodology-types → core/discovery/services/, autopilot 9분 19초). F593 ✅ Sprint 339 (entity 도메인 신설 + closure 3 files, C105→F593 승격, autopilot ~3분 42초 본 sprint 최단 기록 갱신). F595 ✅ Sprint 340 (sr 도메인 신설 + closure 5 files, 옵션 B F593 entity 패턴 재현, routes/sr + services 3 + schemas/sr → core/sr/, autopilot ~4분 11초 두 번째 빠른 기록, MSA cross-domain 19→21 정확 적용 llm.ts 2건 의식적 인정). **F586 P-m 진정 충족 ✅ (S326)**: 운영자 KOAMI Dogfood → GAP-2 진정 종결. **인터뷰 3~4회 패턴 25회차** (사전 측정 → 옵션 비교 → closure 결정 → 의식적 결정 정착화). **MEMORY.md 압축** (S328): 65.9KB → 15.2KB (-77%, archive/sessions-313-327.md 신규). **24 세션 연속 성공** (S306~S331, F560~F595). 잔존: F594 spec-* 묶음 (modules/portal 4 grandfathered) / F596 infra cluster sse-manager+kv-cache+event-bus / llm.ts 별 도메인 결정 / **MSA 룰 강제 교정 F-item** (lint script 스코프 src/ 전체 + grandfathered 21건 해소) / GAP-3 27 stale proposals / 모델 A/B Opus 4.7 vs Sonnet 4.6 / fx-discovery 404 환경 issue / **AI Foundry W18+ D 액션 (5/15 회의 D-day, 사용자 직접 PM)**
