...
**현재 상태:** v1.5.0 — Sprint 17 완료 (Match Rate 98%, 59 endpoints, 27 services, 313 API tests + 20 E2E)
...
- packages/api: Hono API 서버 (59 endpoints, 27 services, 313 테스트)
...
│   │   └── src/
│   │       ├── routes/     # 13개: agent, auth, freshness, health, inbox, integrity, mcp, profile, requirements, spec, token, webhook, wiki
│   │       ├── services/   # 27개 (agent-inbox, agent-orchestrator, agent-runner, claude-api-runner, conflict-detector, execution-types, freshness-checker, github, health-calc, integrity-checker, kv-cache, llm, logger, mcp-adapter, mcp-registry, mcp-resources, mcp-runner, mcp-sampling, mcp-transport, merge-queue, planner-agent, pr-pipeline, reviewer-agent, spec-parser, sse-manager, wiki-sync, worktree-manager)
...
# API 패키지 단독
cd packages/api
pnpm test                         # vitest run (313 tests)
...
- **Phase 2:** ✅ 완료 (Sprint 6~17) — v1.5.0
  ...
  - Sprint 13: MCP Sampling/Prompts(91%) + 에이전트 자동 PR(93%), v1.1.0
  - Sprint 14: MCP Resources(92%) + 멀티 에이전트 동시 PR(92.5%), v1.2.0
  - Sprint 15: PlannerAgent(92%) + AgentInbox(90%) + WorktreeManager(92%), v1.3.0
  - Sprint 16: PlannerAgent LLM 실 연동(92%) + AgentInboxPanel UI(91%), v1.4.0
  - Sprint 17: AI Foundry MCP(100%) + AgentInbox 스레드(100%) + PlannerAgent Orchestrator(97%), v1.5.0
  - 최종: 27 services, 59 endpoints, 313 API tests + CLI 106 + Web 45 + 20 E2E
  - PDCA 문서: `docs/archive/2026-03/sprint-{N}/` (Sprint 3~13, 17 archived), Sprint 14~16 active
...
- **D1**: 10개 마이그레이션 (`packages/api/src/db/migrations/`), `wrangler d1 migrations apply --remote`
