...
**현재 상태:** Phase 9~10 진행 중 (Sprint 102 완료, ~420 endpoints, 169 services, 2250 API + 149 CLI + 265 Web tests + 35 E2E specs) — Phase 10 O-G-D Agent Loop(F270~F273) + Skill Evolution(F274~)
...
│   │   └── src/
│   │       ├── routes/     # 73개 — 목록은 `ls packages/api/src/routes/` 참조
│   │       ├── services/   # 169개 — 목록은 `ls packages/api/src/services/` 참조
│   │       ├── schemas/    # 87개 Zod 스키마 — 목록은 `ls packages/api/src/schemas/` 참조
│   │       └── index.ts
...
- **Phase 9:** 🔧 진행 중 (Sprint 87~100) — 팀 온보딩 + BD 스킬 통합 + GIVC PoC + 발굴 UX(F263~F266) + BD 스킬 배포(F267) + Plugin 전환(F268) + 발굴 IA 정리(F269)
- **Phase 10:** 🔧 진행 중 (Sprint 101~102) — O-G-D Agent Loop(F270~F273) + Skill Evolution(F274~)
...
- **D1**: 0001~0079 마이그레이션 (`packages/api/src/db/migrations/`), `wrangler d1 migrations apply --remote`
