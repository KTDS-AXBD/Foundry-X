...
│   │   └── src/
│   │       ├── routes/     # 107개 — 목록은 `ls packages/api/src/routes/` 참조
│   │       ├── services/   # 237개 — 목록은 `ls packages/api/src/services/` 참조
│   │       ├── schemas/    # 122개 Zod 스키마 — 목록은 `ls packages/api/src/schemas/` 참조
│   │       └── index.ts
...
- **Phase 20:** 📋 계획 (Sprint 179~188) — AX BD MSA 재조정 F392~F401. 모놀리스 모듈화(20-A) + harness-kit + 이벤트/프록시(20-B). 2단계 접근법: 모듈화→실제분리. PRD: `docs/specs/ax-bd-msa/prd-final.md`
