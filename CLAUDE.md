...
**현재 상태:** v1.8.1 — Sprint 23 완료 (79 endpoints, 33 services, 502 API tests + 51 E2E)
...
### Phase 3 (진행 중, v1.6.0~): 멀티테넌시 + 외부 도구 연동
- Sprint 18: 멀티테넌시 기초 + GitHub/Slack 연동 (v1.6.0, Match Rate 93%)
- Sprint 19: AgentInbox 스레드 답장 (v1.7.0, Match Rate 100%)
- Sprint 20: 멀티테넌시 고도화 (v1.8.0, Match Rate 90%)
- Sprint 21: GitHub 양방향 동기화 고도화 (v1.8.0, Match Rate 93%)
- Sprint 22: Slack 고도화 — Interactive D1 실연동 + 채널별 알림 (v1.8+, Match Rate 99%)
- Sprint 23: PlannerAgent 고도화 + 테스트 커버리지 확장 (v1.8.1, Match Rate 92%)
...
│   │   └── src/
│   │       ├── routes/     # 16개: agent, auth, freshness, github, health, inbox, integrity, mcp, org, profile, requirements, slack, spec, token, webhook, wiki
│   │       ├── services/   # 33개 (agent-inbox, agent-orchestrator, agent-runner, claude-api-runner, conflict-detector, execution-types, file-context-collector, freshness-checker, github, github-sync, health-calc, integrity-checker, kv-cache, llm, logger, mcp-adapter, mcp-registry, mcp-resources, mcp-runner, mcp-sampling, mcp-transport, merge-queue, planner-agent, planner-prompts, pr-pipeline, reviewer-agent, slack, spec-parser, sse-manager, wiki-sync, worktree-manager)
│   │       ├── schemas/    # 14개 Zod 스키마 (agent, auth, common, freshness, health, inbox, integrity, mcp, plan, profile, requirements, spec, token, wiki)
│   │       └── index.ts
...
# API 패키지 단독
cd packages/api
pnpm test                         # vitest run (366 tests)
...
- **Phase 3:** 🔧 진행 중 (Sprint 18~23) — v1.8.1
  - Sprint 18: 멀티테넌시 기초(93%) + GitHub 양방향(95%) + Slack(90%), v1.6.0
  - Sprint 19: AgentInbox 스레드 답장(100%), v1.7.0
  - Sprint 20~23: 멀티테넌시 고도화(90%) + GitHub 고도화(93%) + Slack 고도화(99%) + PlannerAgent 고도화(91%) + 테스트 확장(92%)
  - 현재: 33 services, 79 endpoints, 502 API tests, D1 23 테이블
  - PDCA 문서: `docs/archive/2026-03/sprint-{N}/` (Sprint 3~13, 17~23 archived)
...
- **D1**: 15개 마이그레이션 (`packages/api/src/db/migrations/`), `wrangler d1 migrations apply --remote`
