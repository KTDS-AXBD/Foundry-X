...
**현재 상태:** v1.6.0 — Sprint 19 완료 (61 endpoints, 29 services, 356 API tests + 20 E2E)
...
│   │   └── src/
│   │       ├── routes/     # 14개: agent, auth, freshness, health, inbox, integrity, mcp, profile, requirements, slack, spec, token, webhook, wiki
│   │       ├── services/   # 29개 (agent-inbox, agent-orchestrator, agent-runner, claude-api-runner, conflict-detector, execution-types, freshness-checker, github, github-sync, health-calc, integrity-checker, kv-cache, llm, logger, mcp-adapter, mcp-registry, mcp-resources, mcp-runner, mcp-sampling, mcp-transport, merge-queue, planner-agent, pr-pipeline, reviewer-agent, slack, spec-parser, sse-manager, wiki-sync, worktree-manager)
...
- **Phase 3:** 🔧 진행 중 (Sprint 18~) — v1.6.0
  - Sprint 18: 멀티테넌시 기초(93%) + GitHub 양방향(95%) + Slack(90%), v1.6.0
  - Sprint 19: AgentInbox 스레드 답장(100%), v1.7.0
  - 현재: 29 services, 61 endpoints, 356 API tests, D1 24 테이블
  - PDCA 문서: `docs/archive/2026-03/sprint-{N}/` (Sprint 3~13, 17~19 archived)
...
- **D1**: 12개 마이그레이션 (`packages/api/src/db/migrations/`), `wrangler d1 migrations apply --remote`
