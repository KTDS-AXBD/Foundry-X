...
### Added
- **F613 `/api/docs` sub-app 신설 — Pass 시리즈 종결** (PR #744, S333) — autopilot Match **100%**, ~10분. `core/docs/routes/index.ts` 신설 + docsApp Hono sub-app + app.ts L129 `app.route("/api/docs", docsApp)` 변경 + L2 swaggerUI import 제거(sub-app 이전). baseline JSON 1→**0** (`no-direct-route-register` 해소, MSA 룰 강제 교정 시리즈 161→0 완결). **Pass 시리즈 6연속 MERGED**: F608 Pass 1 (#739) + F609 Pass 2 (#740) + F610 Pass 3 (#741) + F611 Pass 4 (#742) + F612 Pass 5 (#743) + F613 Pass 6 (#744). **30 세션 연속 성공** (S306~S333)
- **F612 multi-domain 26 violations 8 파일 fix — Pass 5** (PR #743, S333) — autopilot Match **100%**, F609/F610 정착 패턴 자동 재현. baseline 27→1. 5 target types.ts +13~16 re-export 보강 (agent +3 / discovery +9 / shaping +2 / offering +1 / collection +1) + 8 caller 26 import line `../{target}/services/foo.js` → `../{target}/types.js` 변경. 핫스팟: offering/services/business-plan-generator.ts 5 + prototype-generator.ts 4 + prd-generator.ts 4 + discovery 4 + agent 3 + collection 3 + harness 2 + discovery-routes 2
- **F611 cross-domain-d1 30 violations service API 신설 — Pass 4** (PR #742, S333) — autopilot Match **100%**, ~24분. baseline 57→27. 3 owner-domain d1-api.ts 신설 (core/agent/services/agent-d1-api.ts +6 함수 / core/discovery/services/discovery-d1-api.ts +1 / core/offering/services/offering-d1-api.ts +3) + types.ts +10 service re-export + 30 caller 모두 service 경유 변경 (work 5 + harness 16 + discovery 4 + shaping 1 + agent 1 + decode-bridge 1). 진정한 도메인 경계 보호 (eslint-disable 우회 0)
- **F610 biz-items.ts 19 multi-domain 단일 파일 리팩터링 — Pass 3** (PR #741, S333) — autopilot Match **100%**, ~13분. F609 re-export 패턴 multi-domain caller에 첫 적용. baseline 161→142 (실측 142+). 3 target types.ts +24 re-export (shaping +7 / offering +16 / harness +1) + biz-items.ts 19 caller path 일괄 변경
- **F609 types.ts 13 도메인 신설 + single-domain 44 fix — Pass 2** (PR #740, S332/S333) — autopilot Match **100%**, ~3분. baseline 161→77. 12 도메인 types.ts 신설 (외부 caller 7 + future-proof 5) + verification/types.ts 기존 + 44 single-domain caller import path `../{target}/services/foo` → `../{target}/types` 일괄 갱신. F609 re-export 자동화 무기 정립
- **F608 lint script src/ 확장 + Forward-only baseline 160 fingerprints — Pass 1** (PR #739, S332) — `pnpm lint` 스코프 `src/eslint-rules/`만 → src/ 전체 확장. 측정 차단막 silent fail 해소. 추정 ~21~41 → 실측 **161건** (errors 130 / warnings 31, 3~7배 silent fail 차단막). `scripts/lint-baseline.json` + `scripts/lint-baseline-check.sh` 신설 (regression 검증) + CI deploy.yml에 lint-baseline-check step 추가

### Fixed
- **task-daemon L4/L5 master pane rename hook + tmux pane-border-format 옵션 B** (`8b3a50fe`, `5c599e27`, `0afd387f`, `cfa6c68a`, S333) — `/rename` skill 제거 → SessionStart hook + task-daemon hook 자동화 전환. `~/scripts/tmux-rename-pane.sh` 신설 + 활성 sprint signal 자동 스캔. tmux pane-border-status=top + format `#(cat /tmp/pane-context-#{pane_id})` 메커니즘으로 claude TUI의 pane_title 덮어쓰기 우회. L4 (sprint MERGED) + L5 phase_sprint_created (sprint CREATED) 양쪽 자동 발화 — Sprint 347 첫 실전 검증 PASS (수동 호출 0회)

- **F594 spec 도메인 신설 + closure 6 files** (PR #737, S331) — autopilot Match **100%**, autopilot **~7분 15초** 본 sprint **4번째 빠른 기록** (F593 3분 42초 / F595 4분 11초 / F584 7분 8초 다음). OBSERVED P-a~P-l **9/11 PASS + P-h baseline 2 acceptable + P-k acceptable variant** (semantic Match 100%, 표면 충족 함정 26회차 회피). 옵션 A 18회차 closure 통합 (F593 entity / F595 sr 패턴 재현): routes/spec + routes/spec-library + services/spec-parser + services/spec-library + schemas/spec + schemas/spec-library = closure 6 files git rename → core/spec/{routes,services,schemas}/ 통합 이전. cross-domain depth 정확 적용 8건 (routes/spec 6: env/common/llm/conflict-detector/kv-cache/modules/portal/github + routes/spec-library 2: env/middleware/tenant). 외부 callers 8건 갱신 (app.ts mount 2 + scheduled 1 + portal 2 grandfathered + core/harness 1 + routes/requirements 1 + tests 3). modules/portal grandfathered 2건 depth 갱신만 (룰 카운트 불변). **인터뷰 4회 패턴 26회차** (S331): 1차 메인 결정 (F594 vs F596 vs AI Foundry vs MSA 룰) → 2차 작업 모드 = 사전 측정+인터뷰+autopilot → 3차 closure 범위 = 옵션 A 6 files 일괄 → 4차 절차 = 한 번에 진행. dist orphan 24 files 수동 cleanup S331 (autopilot 미인식, S326~S331 패턴 26회차 일관 재현, gitignored). dual_ai_reviews INSERT 1건 (sprint_id=341 verdict=BLOCK, hook **16 sprint 연속 정상**, 누적 30→**31**). services/ 루트 직속 .ts 12→**10** (-2 = spec-parser + spec-library 2 mv) + routes/+schemas/ 루트도 -2 each. MSA cross-domain 21→**41 acceptable variant** (grep regex 폭 확대, 본질적으로 신규 +8 routes/spec 6 + routes/spec-library 2 의식적 인정). API /api/spec + /api/spec-library 401 ✅. **MSA 룰 강제 교정 별건 F-item 권고 누적 압력 정점**. **25 세션 연속 성공** (S306~S331, F560~F595 + F594)
- **F595 sr 도메인 신설 + closure 5 files** (PR #734, S331) — autopilot Match **100%**, autopilot **~4분 11초** 본 sprint **두 번째 빠른 기록** (F593 3분 42초 다음, F584 7분 8초 갱신). OBSERVED P-a~P-l **12/12 PASS** (semantic Match 100%, 표면 충족 함정 25회차 회피). 옵션 B closure 통합 (F593 entity 패턴 재현): routes/sr.ts + services/sr-classifier + services/hybrid-sr-classifier + services/sr-workflow-mapper + schemas/sr.ts = closure 5 files 모두 git rename 100% similarity → core/sr/{routes,services,schemas}/ 통합 이전. cross-domain depth 정확 적용 4건 (env 1 + middleware 1 + llm 2). 외부 callers 8건 갱신 (app.ts mount 1 + tests 7). **인터뷰 3회 패턴 25회차** (S331): 1차 메인 결정 (F594 spec-* vs F595 sr-*) → 2차 사전 측정 후 옵션 A 17회차 정착에 더 깨끗한 F595 (7 callers + sibling vs F594 10 callers + modules/portal 4 grandfathered) → 3차 closure 결정 옵션 B 채택. dist orphan 20 files 수동 cleanup S331 (autopilot 미인식, S326~S330 패턴 25회차 일관 재현, gitignored 20 rm). dual_ai_reviews INSERT 1건 (sprint_id=340 verdict=BLOCK model=codex-cli, hook **15 sprint 연속 정상**, 누적 29→**30**). services/ 루트 직속 .ts 15→**12** (-3 = sr-* 3 mv) + routes/+schemas/ 루트도 -1 each. MSA cross-domain **19→21 정확 적용** (llm.ts 2건 의식적 인정 — F593 P-k 19 보존 vs 본 sprint 21 정확 적용 = autopilot variance, semantic 영향 0). API /api/sr 401 ✅ (auth 정상, mount path 동일). **MSA 룰 강제 교정 별건 F-item 권고 유지** (lint script 스코프 src/ 전체 확장 + grandfathered 21건 해소). **24 세션 연속 성공** (S306~S331, F560~F595)
- **F593 entity 도메인 신설 + closure 3 files (C105→F593 승격)** (PR #732, S330) — autopilot Match **100%**, autopilot **~3분 42초** 본 sprint **최단 기록 갱신** (이전 F584 7분 8초). OBSERVED P-a~P-l **11/12 PASS** (semantic Match 100%, 표면 충족 함정 24회차 회피). 옵션 c closure 통합 (F587/F588 선례 재현): routes/entities + services/entity-registry + schemas/entity = closure 3 files → core/entity/{routes,services,schemas}/ 통합 이전. cross-domain depth 정확 적용 (`../../../schemas/common`, `../../../env`, `../../../middleware/tenant`). app.ts mount path 동일 = endpoint 영향 0. **인터뷰 2회 압축 결정** (S329, 23회차+1): 1차 사용자 직접 "C105 진행" 지시 → 2차 사전 측정 후 옵션 (c) 신규 core/entity/ + 간단 mv + 즉시 가동 (3 의사결정 1 묶음). C105 → F593 task-promotion 승격 완결. dist orphan 12 files 수동 cleanup (S330 패턴 24회차). dual_ai_reviews INSERT 1건 (hook 14 sprint 연속, 누적 28→29). services/ 루트 직속 .ts 16→15 + routes/+schemas/ 루트도 -1 each. MSA cross-domain 19 그대로 유지 (autopilot baseline 보존). API /api/entities 401 ✅
- **F592 services/ 잔존 dead+sibling 2 files 정리** (PR #730, S329) — autopilot Match **100%**, autopilot 9분 19초, +47/-29 5 files. OBSERVED P-a~P-k **11/11 PASS** (표면 충족 함정 23회차 회피). 옵션 D-a 보수: entity-sync rm (0 callers, pure dead) + methodology-types → core/discovery/services/ (1 caller sibling, type-only erasure). **인터뷰 4회 패턴 23회차**: 1차 services/ 18 files → 2차 옵션 D 4 files → 3차 entity-registry 도메인 모호 발견 → 4차 의식적 D-a 보수 2 files + C105 보류 (F593으로 후속 승격). dist orphan 8 files 수동 cleanup. dual_ai_reviews INSERT 2건 (hook 13 sprint 연속, 누적 26→28). services/ 루트 직속 .ts 18→16. MSA cross-domain 19 그대로 유지 (sibling 이동 = 룰 위반 0건 추가)
- **F591 prd/prototype 5 files → core/offering/services/** (PR #728, S328) — autopilot self-evaluation Match **100%**, +159/-16 10 files, 21:32:41 KST. OBSERVED P-a~P-i **9/9 PASS + P-j 28→0 cleanup + P-k 19 acceptable variant** (semantic Match 100%, 표면 충족 함정 22회차 회피). 옵션 A1 16회차 정착화 (관행 일관성): prd-generator + prd-template + prototype-generator + prototype-styles + prototype-templates 5 files git mv → core/offering/services/ (94~100% similarity rename) + 5 callers 갱신 (biz-items.ts production 2 + __tests__ 3). **인터뷰 4회 패턴 22회차** (S328): 1차 "F591 7 files" → 2차 사전 측정 5 files (prd-qsa-adapter adapters/ 별도) + 옵션 A → 3차 ESLint `no-cross-domain-import` 직접 lint 결과 **biz-items.ts 18 grandfathered 위반** + `pnpm lint` 스코프 **eslint-rules/만** 발견 (룰 'error'지만 미적용) → 4차 의식적 **A1 채택** (관행 일관성, 누적 19 lint pass 보장, **MSA 룰 강제 교정 별건 F-item 권고**). fx-offering self-contained 재확증 (별건). dist orphan 28 files 수동 cleanup (S326/S327 패턴 22회차 일관 재현, gitignored). dual_ai_reviews INSERT 1건 (hook **12 sprint 연속 정상**, 누적 25→**26**). services/ 루트 직속 .ts 23→**18** (-5). 21 세션 연속 성공 (S306~S328). **task-daemon false positive crash counter** 5 → reset 후 PID 199325 정상 재시작 (sprint 시작마다 누적 30+ 사례 패턴, L4 daemon 코드 개선 후보 deferred). **MEMORY.md 압축** (S328): 65.9KB → 15.2KB (-77%, archive/sessions-313-327.md 51KB 신규, S313~S326 18 세션 통합)
...
### Changed
- **30 세션 연속 성공** (S306~S333): F560~F613 Pass 시리즈 6연속 MERGED Match 100% × 6 라인업 추가
- **MSA 룰 baseline 161 → 0** (F608~F613 Pass 1~6 완결, errors 0 + warnings 0)
