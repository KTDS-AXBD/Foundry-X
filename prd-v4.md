...
**버전:** v4 (Synthnoosh Agentic Harness Bootstrap 분석 반영)
**날짜:** 2026-03-16
**작성자:** AX BD팀
**상태:** ✅ 착수 준비 완료
...
| # | 변경 | 출처 | 핵심 |
|---|------|------|------|
| 1 | `foundry-x init` Brownfield/Greenfield 분기 명세화 | synthnoosh/agentic-harness-bootstrap | 기존 저장소(Brownfield)와 신규 저장소(Greenfield) 처리 로직 분리 |
| 2 | `ARCHITECTURE.md` 산출물 추가 | synthnoosh/agentic-harness-bootstrap | 에이전트 전용 레이어 맵 — "무엇이 어디 있는지"만 기술 |
| 3 | 3계층 경계 (Always/Ask/Never) 명세화 | synthnoosh/agentic-harness-bootstrap | `CONSTITUTION.md` 구조를 3계층으로 명시화 |
| 4 | Semantic Linting 원칙 추가 | synthnoosh/agentic-harness-bootstrap | 린터 메시지 = 에이전트 교육 채널 |
| 5 | `verify-harness.sh` 산출물 추가 | synthnoosh/agentic-harness-bootstrap | 하네스 무결성 지속 검증 스크립트 |
| 6 | Harness Evolution Rules 추가 | synthnoosh/agentic-harness-bootstrap | 모든 하네스 파일에 자기 갱신 규칙 내장 |
| 7 | 에이전트 컨텍스트 지속성 전략 추가 (§7.11 신설) | Anthropic Engineering Blog | `progress.md` 패턴으로 장기 작업 세션 연속성 보장 |
| 8 | CI harness-sync-check 추가 | synthnoosh/agentic-harness-bootstrap | CLAUDE.md ↔ AGENTS.md 섹션 구조 일치 자동 검증 |
| 9 | 리포지토리 구조 갱신 | v4 신규 산출물 반영 | `ARCHITECTURE.md`, `scripts/`, `progress.md` 추가 |
...
| synthnoosh/agentic-harness-bootstrap | 하네스 bootstrap 레퍼런스 구현 | Foundry-X는 SDD Triangle + 조직 협업까지 통합 |
...
| K6 | 하네스 무결성 통과율 | N/A | > 95% | `verify-harness.sh` 실행 로그 |
...
`foundry-x init`으로 표준 하네스를 스캐폴딩한다. v4에서 다음 4가지를 구체화한다.

**[A] Brownfield / Greenfield 분기 처리**

`init` 실행 시 대상 저장소 유형에 따라 처리 방식을 분기한다.

```
Brownfield (기존 저장소 — KT DS SR 처리 등 실 프로젝트):
  Phase 0 — Discover : 언어·프레임워크·빌드 도구·CI·테스트 인프라 스캔 → repo profile 생성
  Phase 1 — Analyze  : repo profile 기반으로 모듈·진입점·아키텍처 패턴 심층 분석
  Phase 2 — Generate : 분석 결과를 반영한 맞춤형 하네스 산출물 생성 (기존 파일 merge, 덮어쓰기 금지)
  Phase 3 — Verify   : 생성 파일 파싱·참조 커맨드 동작·내부 일관성 검증

Greenfield (신규 저장소):
  - "무엇을 만드나?" 인터랙티브 질문 → 스택 선택
  - 선택된 스택에 맞는 디렉터리 구조·.editorconfig·태스크 러너 scaffolding
  - 이후 동일한 Phase 1~3 실행
```

`foundry-x init`의 자동 감지 로직: `package.json`, `go.mod`, `pom.xml`, `Pipfile` 등 프로젝트 마커 파일 존재 여부로 Brownfield 판단. `--mode [brownfield|greenfield]` 플래그로 수동 지정 가능.

**Idempotency 원칙**: `init`은 항상 멱등적으로 동작한다. 이미 파일이 존재하면 읽고 → 사용자 커스터마이징 보존 → 신규 섹션만 merge. 절대 덮어쓰지 않는다.

**[B] 에이전트 전용 아키텍처 맵 (`ARCHITECTURE.md`)**

`foundry-x init` 산출물에 `ARCHITECTURE.md`를 추가한다.

설계 원칙: "에이전트에게 **무엇이 어디 있는지**를 알려준다. 왜 존재하는지는 ADR에 있다."

```markdown
# ARCHITECTURE.md (에이전트용 레이어 맵)

## 레이어 구조
[레이어 다이어그램 — 패키지 간 의존 방향]
...
## 금지 패턴
[이 저장소에서 절대 하면 안 되는 코드 패턴]
```

`ARCHITECTURE.md`는 프로젝트가 성장함에 따라 에이전트가 직접 최신화한다 (Harness Evolution Rules, §7.2-D 참조).

**[C] 3계층 경계 (Always / Ask / Never)**

`CONSTITUTION.md`를 다음 3계층 구조로 명세화한다. 에이전트가 판단할 필요 없이 명확한 행동 지침을 제공하는 것이 목적이다.
...
KT DS SR 처리 시나리오 특화 경계는 `templates/kt-ds-sr/CONSTITUTION.md`에 별도 정의한다.

**[D] Harness Evolution Rules (하네스 자기 갱신 규칙)**

모든 하네스 파일(`CLAUDE.md`, `AGENTS.md`, `ARCHITECTURE.md`)에 다음 규칙을 표준 섹션으로 내장한다.

```markdown
## 하네스 갱신 규칙 (에이전트 필독)

이 파일은 프로젝트와 함께 살아있어야 한다.
다음 조건에서 해당 파일을 업데이트하라:

CLAUDE.md / AGENTS.md 갱신 조건:
  - 새 CLI 커맨드 추가 시
  - 빌드/테스트/lint 커맨드 변경 시
  - 새로운 Always/Ask/Never 경계 결정 시
  - 디렉터리 구조 주요 변경 시

ARCHITECTURE.md 갱신 조건:
  - 새 패키지/모듈 추가 시
  - 패키지 간 의존 방향 변경 시
  - 새 진입점 추가 시

갱신 후: `foundry-x status`로 하네스 무결성 확인 실행
```

**[E] Semantic Linting (에이전트 교육 채널로서의 린터)**

린터 설정 철학: 오류를 탐지하는 것이 아니라, 에이전트에게 올바른 수정 방법을 한 번에 전달하는 것이 목적이다.

```
원칙:
1. 린터 메시지는 "무엇이 잘못됐나"가 아니라 "어떻게 고치나"를 포함한다
2. --fix 자동 수정 가능한 규칙을 우선 채택한다
3. TypeScript strict 모드 활성화 (타입 오류 = 명세 드리프트 신호)
4. 에이전트가 린트 오류 발생 시 최대 2회 자동 수정 시도 후 human escalation

구현:
- ESLint: rule 메시지에 코드 예시 포함
- Prettier: 자동 포맷 (논쟁 불필요)
- TypeScript: strict: true, noUncheckedIndexedAccess: true
- Husky pre-commit: lint → typecheck → plumb-check 순서
```

`foundry-x init`이 대상 저장소의 스택을 감지하여 적합한 lint 설정을 자동 생성한다 (`templates/lint/` 참조).
...
- `progress.md`가 에이전트 세션 간 컨텍스트 연속성 담당 (§7.11 참조)
...
| **Linter** | ESLint + Prettier + TypeScript strict | Semantic Linting 원칙 |
...
Git hook 실패 시 에이전트 복구:
  1) hook 실패 사유를 에이전트에게 전달 (Semantic Lint 메시지 포함)
  2) 에이전트 자동 수정 시도 (최대 2회)
  3) 실패 시 --no-verify로 우회하지 않고 human escalation
  4) 우회 비율 모니터링 (KPI K2)
```

### 7.7 Git ↔ PostgreSQL 정합성
...
│   │   ├── src/
│   │   │   ├── commands/
│   │   │   │   ├── init.ts          # 하네스 스캐폴딩 (Brownfield/Greenfield 분기)
│   │   │   │   ├── sync.ts          # SDD 동기화 검사
│   │   │   │   └── status.ts        # 동기화 상태 + 하네스 무결성 표시
│   │   │   ├── plumb/
│   │   │   │   └── bridge.ts        # Plumb subprocess 래퍼
│   │   │   ├── harness/
│   │   │   │   ├── detect.ts        # Brownfield/Greenfield 감지 로직
│   │   │   │   ├── discover.ts      # Phase 0: 스택 스캔
│   │   │   │   ├── analyze.ts       # Phase 1: 아키텍처 분석
│   │   │   │   ├── generate.ts      # Phase 2: 산출물 생성 (merge 포함)
...
├── templates/                       # 하네스 스타터 킷
│   ├── default/
│   │   ├── CLAUDE.md                # Claude Code 지침 (Harness Evolution Rules 포함)
│   │   ├── AGENTS.md                # Codex 지침 (CLAUDE.md와 섹션 구조 동기화)
│   │   ├── ARCHITECTURE.md          # 에이전트용 레이어 맵 (v4 신규)
│   │   ├── CONSTITUTION.md          # Always/Ask/Never 3계층 경계 (v4 강화)
│   │   ├── specs/
│   │   │   └── .gitkeep
│   │   ├── .plumb/
│   │   │   └── config.json
│   │   └── progress.md              # 에이전트 세션 지속성 파일 (v4 신규)
│   ├── lint/                        # 스택별 lint 설정 템플릿 (v4 신규)
...
│       └── specs/
├── scripts/
│   ├── verify-harness.sh            # 하네스 무결성 검증 (v4 신규)
│   └── check-sync.sh                # CLAUDE.md ↔ AGENTS.md 섹션 구조 검사 (v4 신규)
├── examples/
│   └── sample-project/
├── docs/
│   ├── adr/                         # Architecture Decision Records
│   │   └── ADR-000-harness-bootstrap.md  # 첫 번째 ADR (v4 신규)
│   └── guides/
├── .github/
│   └── workflows/
│       ├── ci.yml                   # 기존 CI
│       └── harness-sync-check.yml   # 하네스 파일 동기화 검증 CI (v4 신규)
...
| A6 | Semantic Linting 메시지가 에이전트의 자동 수정 성공률을 높인다 | Medium | lint 오류 후 자동 수정 성공률 측정 | Month 1~2 |
| A7 | `progress.md` 패턴이 에이전트 컨텍스트 손실을 방지한다 | Medium | 세션 재시작 후 작업 연속성 관찰 | Month 2 |

### 7.11 에이전트 컨텍스트 지속성 전략 (v4 신규)

에이전트는 새 컨텍스트 윈도우로 시작할 때마다 이전 작업 상태를 잃는다. 이를 해결하기 위해 `progress.md` 파일을 세션 간 핸드오프 매체로 활용한다.
...
## 다음 세션 시작 방법
foundry-x status  # 하네스 무결성 확인
git log --oneline -10  # 최근 커밋 확인
cat progress.md  # 이 파일 확인
```

에이전트 행동 규칙:
- 세션 시작 시: `progress.md` 및 최근 git log 먼저 읽기
- 작업 중: 주요 결정 발생 시 즉시 `progress.md` 업데이트
- 세션 종료 시: 완료/미완료/블로커 항목 업데이트 후 commit

`progress.md`는 `CLAUDE.md`의 "Always" 항목에 포함되어, 에이전트가 자동으로 관리한다.
...
- foundry-x CLI: init, sync, status (3개 커맨드)
  - init: Brownfield/Greenfield 분기 + 4단계(discover→analyze→generate→verify)
  - sync: Plumb Track A 통합 + progress.md 업데이트
  - status: SDD 동기화 상태 + 하네스 무결성 표시
- Plumb Track A 통합 (subprocess 래퍼)
- KT DS SR 처리 특화 템플릿 1종
  (CLAUDE.md + ARCHITECTURE.md + CONSTITUTION.md(SR 특화) 포함)
- 내부 개발자 5명 강제 온보딩

산출물:
- npm publish 가능한 CLI 패키지
- 하네스 스타터 킷
  (CLAUDE.md, AGENTS.md, ARCHITECTURE.md, CONSTITUTION.md, specs/, progress.md)
...
- CLI 개선 (피드백 기반)
- Plumb Track A/B 판단
- Harness Evolution Rules 실효성 검증
  (에이전트가 하네스 파일을 얼마나 자주/정확하게 갱신하는지)
...
- 결정 승인 비율 (목표: > 70%)
- 하네스 무결성 통과율 (목표: > 95%) ← v4 신규 KPI K6
...
- **대응 3**: 현실 지표(CLI 호출, --no-verify 비율, 하네스 무결성)로 "착각 방지"
...
| 기존 자산 | 판정 | 이관 대상 | 비고 |
|----------|------|----------|------|
| shared-auth (JWT/RBAC) | Phase 2에서 검증 후 재활용 | foundry-x API | 에이전트 서비스 계정 지원 여부 검증 필요 |
...
| codegen-core | 부분 재활용 검토 | foundry-x SDD | AST/파일 생성 로직 재활용 가능 |
...
| 저장소 | 활용 방식 | 적용 시점 |
|--------|----------|----------|
| [synthnoosh/agentic-harness-bootstrap](https://github.com/synthnoosh/agentic-harness-bootstrap) | `templates/` 구조 및 4단계 bootstrap 로직 레퍼런스 | Phase 1 Sprint 1 |
| Anthropic claude-progress.txt 패턴 | `progress.md` 설계 레퍼런스 | Phase 1 Sprint 2 |
...
| Q5 | `CLAUDE.md` ↔ `AGENTS.md` 섹션 구조 동기화 기준 (어느 수준까지 일치해야 하나?) | 아키텍트 | Sprint 1 |
| Q6 | Harness Evolution Rules 위반 시 에이전트 처리 방식 (경고만? PR 블록?) | 아키텍트 | Sprint 2 |
