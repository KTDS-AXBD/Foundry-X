...
> **버전**: v1.4.0 — **Sprint 착수 확정본**  
> **작성일**: 2026-03-25  
> **상태**: ✅ Sprint 1 착수 승인 — 모든 Blocking OQ 해소 완료  
> **충분도**: 88/100  
> **대상 독자**: KT DS AX BD팀 내부 · AI 에이전트(CLAUDE.md 연계)  
> **변경 이력**: v1.3.0 → v1.4.0 (OQ-3 해소 반영 — 마스킹 조건부 허용 확정)
...
- **보안 필수**: 외부 LLM 전송 전 `PromptGatewayService`(F149) 경유 — KT DS 고유명사·조직명·식별자 마스킹 후 전송. 마스킹 규칙은 거버넌스 관리자 승인 목록 기준
...
- Never: 사용자 확인 없이 Git 커밋, 자동 저장, PromptGatewayService 우회 전송
...
- **보안 필수**: BMCAgent와 동일하게 `PromptGatewayService`(F149) 경유 마스킹 필수
...
- `PromptGatewayService` 마스킹 파이프라인을 우회하여 원문 그대로 외부 LLM 전송
...
| OQ-3 | BMC 데이터 보안 등급 분류 (사내 기밀 여부) | 거버넌스 관리자 | ~~Day 1 오전~~ | ✅ **해소** — **마스킹 조건부 허용** 확정. 고유명사·식별자를 마스킹한 후 외부 LLM 전송 가능. `PromptGatewayService`(Foundry-X F149) 경유 필수 |
...
| 수정 ID | 내용 | 반영 섹션 |
|---------|------|----------|
| OQ3 | OQ-3 해소 — 마스킹 조건부 허용 확정, PromptGatewayService(F149) 경유 필수 | §11 |
| SEC-1 | BMCAgent·InsightAgent 스펙에 마스킹 파이프라인 필수 조건 명시 | §4.2, §4.3 |
| SEC-2 | CONSTITUTION Never 항목에 마스킹 우회 금지 추가 | §6 |
| SEC-3 | BMCAgent 마스킹 검증 AC 추가 (X-Gateway-Processed 헤더 검증) | §4.2 |
...
*v1.4.0 Sprint 착수 확정 — 모든 Blocking OQ 해소 완료 — 마스킹 조건부 허용(OQ-3) + Git 저장소 전략(OQ-4 Week 1 결정) + SSO 대안(Foundry-X JWT 재사용)*
