...
changelog:
  ...
  - version: "2.3"
    ...
    sprint: 201
    changes:
      - "F420: Step 8 이후 PPTX 변환 AskUserQuestion 추가 — 승인 시 offering-pptx 스킬 체이닝"
      - "F420: 3가지 PPTX 보고 목적(proposal/report/review) 안내 + purpose 파라미터 전달"
      - "F421: 매 버전 생성 시 review-history.md 자동 append 로직 추가"
      - "F421: diff 조회 — 이전 버전 대비 변경 섹션 목록 + 사유 표시"
  - version: "2.2"
    date: "2026-04-07"
    sprint: 200
    changes:
...
[9] PPTX 변환 (선택 — offering-pptx 체이닝)
    ├── AskUserQuestion: "PPTX 슬라이드로도 변환할까요? (대외 제안/경영회의 보고/팀 내부 검토)"
    ├── 거절 시 → 완료
    └── 승인 시 → offering-pptx 스킬 체이닝:
        ├── purpose 파라미터 결정:
        │   ├── "대외 제안" → purpose=proposal
        │   ├── "경영회의 보고" → purpose=report
        │   └── "팀 내부 검토" → purpose=review
        └── 전달 값: HTML 파일 경로 + purpose + 사업명 + 발굴 산출물 경로
        ↓
[10] 버전 이력 기록 (자동)
...
매 버전 생성(v0.1, v0.2, ..., v1.0) 시 `review-history.md`에 자동으로 append한다.

### review-history.md 형식

```markdown
# 사업기획서 버전 이력

사업명: {사업명}
파일: 사업기획서_{사업명}_v{최신버전}_{YYMMDD}.html

| 버전 | 날짜 | 변경 요약 | 변경 섹션 |
|------|------|----------|----------|
| v0.1 | 26-04-07 | 초안 생성 | 전체 |
| v0.2 | 26-04-08 | 피드백 반영 | 01-1, 05-2 |
```
...
| 시점 | 행동 |
|------|------|
| Step 4 완료 (v0.1 생성) | `| v0.1 | {날짜} | 초안 생성 | 전체 |` 추가 |
| Step 5 완료 (v0.2~0.4) | `| v0.{N} | {날짜} | {피드백 요약 1줄} | {수정 섹션 목록} |` 추가 |
| Step 8 완료 (v1.0) | `| v1.0 | {날짜} | 최종 확정 | — |` 추가 |

### diff 조회

사용자가 "이전 버전과 뭐가 달라졌어?" 요청 시:
1. review-history.md에서 최근 2행 읽기
2. 변경 섹션 목록 기반으로 해당 섹션의 Before/After 비교 표시
3. `<!-- CHANGED: {사유} -->` 마커가 있는 부분 하이라이트

> **파일 위치**: 사업기획서 HTML 파일과 동일한 디렉토리에 생성.
...
Step 8 완료 후 offering-pptx 스킬을 체이닝하여 동일 내용을 PPTX로도 생성한다.

### 체이닝 인터페이스

```
[offering-html Step 9] → [offering-pptx]

전달 파라미터:
  - html_path: "사업기획서_{사업명}_v1.0_{YYMMDD}.html"
  - business_name: {사업명}
  - purpose: proposal | report | review
  - discovery_path: {발굴 산출물 디렉토리 경로}
```

### purpose별 동작 차이

| purpose | 슬라이드 초점 | Exec Summary 위치 | 교차검증 포함 |
|---------|------------|------------------|-------------|
| proposal | 기술 상세 + 솔루션 강조 | 슬라이드 2 | 포함 |
| report | 경영 언어 + KPI 중심 | 슬라이드 1 | 포함 |
| review | 리스크 중심 + No-Go 기준 | 슬라이드 2 | 강조 (상세) |

> **원칙**: HTML 재생성 없이 이미 완성된 HTML을 참조하여 PPTX 생성.
> "동일 발굴 산출물로 다시 처음부터" 방식이 아닌, "완성된 기획서를 슬라이드로 변환" 방식.
...
### 버전 관리
| 버전 | 의미 | review-history.md |
|------|------|------------------|
| v0.1 | 초안 (목차 + 핵심 내용) | 자동 기록 (초안 생성) |
| v0.2~0.4 | 피드백 반영 수정 (`<!-- CHANGED -->` 마커 포함) | 자동 기록 (피드백 요약 + 변경 섹션) |
| v0.5+ | 본부장/대표 보고용 | 자동 기록 |
| v1.0 | 최종 확정본 | 자동 기록 (최종 확정) |

> review-history.md 위치: HTML 파일과 동일한 디렉토리 (F421 상세 참조)
...
>
> **연동 스킬**:
> - [offering-pptx](../offering-pptx/SKILL.md) — PPTX 변환 (Step 9 체이닝, Sprint 201 F420 ✅)
...
- [ ] review-history.md에 모든 버전이 기록되었는가 (F421)
- [ ] PPTX 변환 여부를 사용자에게 확인했는가 (F420)
