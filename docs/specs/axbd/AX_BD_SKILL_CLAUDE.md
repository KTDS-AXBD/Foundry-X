...
**스킬 목록**:
- `/pm-market-research:competitor-analysis` — 직접·간접 경쟁사 식별, 강점·약점·차별화 분석
- `/pm-market-research:market-segments` — 시장 세그먼트 식별
...
- `/pm-market-research:market-segments` — 고객 세그먼트 3~5개 식별
- `/pm-product-discovery:identify-assumptions-new` — 핵심 가정 식별 (8개 리스크 카테고리)
...
**스킬 목록**:
- `/pm-product-strategy:swot-analysis` — SWOT 분석 + 실행 권고
- `/pm-product-strategy:porters-five-forces` — Porter's 5 Forces 산업 구조 분석
- `/pm-product-strategy:pestle-analysis` — 거시환경 6축 분석 (정치·경제·사회·기술·법률·환경)
- `/pm-go-to-market:competitive-battlecard` — 경쟁사별 세일즈 배틀카드
...
**스킬 목록**:
- `/pm-product-discovery:opportunity-solution-tree` — 목표→기회→솔루션→실험 트리
- `/pm-product-strategy:business-model` — Business Model Canvas 9블록 (초안)
- `/pm-product-strategy:ansoff-matrix` — Ansoff Matrix 성장 전략 방향
- `/pm-product-discovery:brainstorm-experiments-new` — 린 실험(프리토타입) 설계
...
**스킬 목록**:
- `/pm-execution:prioritization-frameworks` — 9개 우선순위 프레임워크 (RICE, ICE, Kano, MoSCoW 등)
- `/pm-product-discovery:prioritize-assumptions` — Impact × Risk 매트릭스
- `/pm-product-discovery:prioritize-features` — 기능 우선순위화
...
**스킬 목록**:
- `/pm-market-research:user-personas` — JTBD 기반 페르소나 3개 생성
- `/pm-market-research:customer-journey-map` — 구매여정 터치포인트·감정·기회 맵핑
- `/pm-product-strategy:value-proposition` — JTBD 6파트 가치제안 설계
- `/pm-go-to-market:ideal-customer-profile` — B2B ICP (이상 고객 프로필) 정의
- `/pm-product-discovery:interview-script` — 고객 인터뷰 스크립트 생성
...
**스킬 목록**:
- `/pm-product-strategy:business-model` — BMC 9블록 완성
- `/pm-product-strategy:lean-canvas` — Lean Canvas
- `/pm-product-strategy:monetization-strategy` — 수익화 전략 브레인스토밍
- `/pm-product-strategy:pricing-strategy` — 가격 모델 상세 설계
- `/pm-product-strategy:startup-canvas` — Product Strategy + Business Model 통합
- `/pm-go-to-market:growth-loops` — 성장 플라이휠 5유형 설계
...
**스킬 목록**:
- `/pm-go-to-market:gtm-strategy` — GTM 전략 수립
- `/pm-go-to-market:beachhead-segment` — Beachhead 시장 선정
- `/pm-go-to-market:gtm-motions` — GTM 모션 7유형 분석
- `/pm-product-discovery:metrics-dashboard` — KPI 대시보드 설계
- `/pm-execution:create-prd` — PRD 8섹션 작성
- `/pm-marketing-growth:north-star-metric` — North Star Metric 정의
...
- `/pm-product-discovery:prioritize-assumptions` — 핵심 가정 우선순위화
- `/pm-execution:pre-mortem` — Pre-mortem 리스크 분류
...
**스킬 목록**:
- `/pm-product-strategy:product-strategy` — 9섹션 Product Strategy Canvas
- `/pm-go-to-market:gtm-strategy` — GTM 전략 최종 검토
- `/pm-execution:stakeholder-map` — 이해관계자 맵 + 커뮤니케이션 계획
- `/pm-product-strategy:product-vision` — 제품 비전 수립
- `/pm-execution:create-prd` — PRD 8섹션 (프로토타입 범위 정의 포함)
