# Self-Evolving Harness v2 PRD

**버전:** v2
<!-- CHANGED: 버전 증가 (v1 → v2) -->
**날짜:** 2026-04-06
**작성자:** AX BD팀
**상태:** 🔄 검토 중
**입력 문서:** FX-STRT-015 v3.0 (자가 발전 하네스 전략), 인터뷰 로그

---

## 1. 요약 (Executive Summary)

**한 줄 정의:**
Claude Code 에이전트가 반복 실패 패턴으로부터 자동으로 Guard Rail을 생성하고, O-G-D Loop을 다중 도메인에 재활용하며, 하네스 인프라의 실제 활용률을 측정하는 자가 발전 시스템을 구축한다.

**배경:**
Phase 10~14에서 자가 발전 하네스의 3단계 인프라(Quality Gate, Self-Healing, Self-Evolving)를 구축했다. TaskState 10상태, Hook/EventBus, Orchestration Loop 3모드, Agent Adapter 5종, Telemetry Dashboard까지 완성되어 인프라는 충분하다. 그러나 **데이터→행동 변화** 루프가 부재하여, 쌓인 텔레메트리가 실질적 품질 개선으로 연결되지 않고 있다.

**목표:**
하네스 인프라가 스스로 학습하고, 반복 실패를 예방하며, 인프라 활용률을 측정하여 미사용 인프라를 정리하는 "자가 발전 루프"를 완성한다.

<!-- CHANGED: 데이터 품질 관리, 승인자 부담 분산, 보안 및 권한 관리, 성능 및 확장성, Rule 생성 품질 및 투명성 등 주요 결함·누락 요소의 반영 예고 -->
**추가 목표:**
- 데이터 품질 저하 및 이상치 발생을 실시간 감지하고 개선하는 모니터링 체계 구축.
- Rule 승인 절차의 승인자 부담 분산 및 자동화 보조 도입.
- 보안 및 권한 관리(파일 쓰기, 승인 플로우, 감사 로그) 체계 강화.
- 대용량 데이터 처리 및 O-G-D 다중 도메인 부하 분산 등 성능·확장성 고려.
- Rule 자동 생성 품질 및 투명성 보장, 인간-AI 피드백 루프 강화.

---

## 2. 문제 정의

### 2.1 현재 상태 (As-Is)

- **반복 실패 미감지:** Sprint마다 유사한 실패가 반복되지만, execution_events 데이터를 분석하여 패턴을 자동 감지하는 시스템이 없다.
- **Rules 정적 배치:** `.claude/rules/` 5종이 세션 #199(2026-04-05) 이후 변경 없이 정적으로 유지. 30+ Sprint 동안 축적된 교훈이 Rule로 반영되지 않음.
- **수동 개입 과다:** 텔레메트리 데이터(execution_events, task_histories)가 D1에 쌓이고 있으나, 해석과 행동 전환이 전적으로 수동.
- **O-G-D Loop 단일 도메인:** O-G-D(Orchestrator-Generator-Discriminator) Loop이 BD 형상화와 코드 품질 판정에만 사용됨. Phase 16 Prototype Auto-Gen(F355)에서 첫 재활용을 시도하지만, 범용적 재활용 인터페이스가 없음.
- **인프라 활용률 미측정:** 에이전트 16종 × 스킬 15종이 등록되어 있지만, 실제 월간 사용 빈도가 불명. 전략 문서 §7.2에서 "built but unused" 위험을 **높음**으로 평가.
<!-- CHANGED: 데이터 품질 저하 및 이상치, 편향, 로그 누락에 대한 문제 인식 추가 -->
- **데이터 품질·편향·누락 위험:** execution_events 데이터에 이상치, 편향, 로그 누락이 발생할 수 있으나, 실시간 감시 및 조치 체계가 부재.
<!-- CHANGED: 승인자 단일화/부담 집중 문제 인식 추가 -->
- **Rule 승인자 집중:** Rule 승인 업무가 특정 개인(Sinclair Seo)에게 집중되어 업무 과부하, 휴가·부재 시 승인 병목 발생 우려.
<!-- CHANGED: 보안 및 권한 관리 미흡 문제 인식 추가 -->
- **보안 및 권한 관리 미흡:** `.claude/rules/` 파일 쓰기, 승인 플로우 접근 등에 대한 명확한 권한 정책 및 감사체계가 부족.

### 2.2 목표 상태 (To-Be)

- **자동 패턴 감지:** execution_events에서 반복 실패 패턴을 자동 추출하여, Rule 초안을 생성하고 세션 시작 시 사람에게 제안.
- **Rules 동적 갱신:** 승인된 Rule이 `.claude/rules/`에 자동 배치되어, 다음 세션부터 Claude Code가 준수.
- **O-G-D 범용 인터페이스:** 도메인 독립적인 O-G-D Loop 호출 인터페이스를 제공하여, BD·코드리뷰·문서검증·Prototype 등 다양한 도메인에 재활용.
- **활용률 대시보드:** 에이전트/스킬별 월간 사용 빈도, DERIVED/CAPTURED 스킬 재사용률을 Dashboard에서 확인 가능.
<!-- CHANGED: 데이터 품질 모니터링 및 실시간 개선 루프 명시 -->
- **지속적 데이터 품질 모니터링:** execution_events, task_histories 등 핵심 데이터의 품질 저하(이상치, 누락, 편향 등)를 실시간 감지·알림·개선 요청하는 모니터링 체계 구축.
<!-- CHANGED: 승인자 부담 분산 및 자동화, UX 개선 목표 명시 -->
- **승인 플로우 개선 및 분산:** Rule 승인 업무를 다중 승인자 분산, 자동화 보조(추천, 내역 리뷰) 및 직관적 CLI UX로 효율화.
<!-- CHANGED: 보안/권한 관리 목표 명시 -->
- **보안 및 권한 관리 강화:** `.claude/rules/` 파일 쓰기, 승인 플로우 접근, 감사 로그 등 권한 분리 및 보안 정책 적용.
<!-- CHANGED: 성능/확장성 목표 명시 -->
- **성능 및 확장성 확보:** 대용량 데이터 처리, O-G-D 다중 도메인 부하 분산, Rule 엔진 충돌 감지 등 확장성 설계.
<!-- CHANGED: Rule 생성 품질 및 투명성·AI 피드백 루프 명시 -->
- **Rule 자동 생성 품질 및 투명성 강화:** LLM 생성 Rule의 품질·근거·신뢰성 명확화 및 인간-AI 피드백 루프(수정→재학습) 구현.

### 2.3 시급성

- **인프라 ROI 증명 시점:** Phase 14에서 ~12,600 LOC + 181 tests의 하네스 인프라를 구축했다. 이 투자가 실제 품질 개선으로 이어지는지 증명해야 한다.
- **Phase 16 재활용 시점:** F355(O-G-D 품질 루프)가 Phase 16에서 시도된다. 이 경험을 범용화 설계에 반영하려면 Phase 16 직후가 적기다.
- **Rules 노후화 가속:** Sprint가 누적될수록 정적 Rules와 현실의 괴리가 커진다.
<!-- CHANGED: 데이터 품질/양 미달 시 전체 프로젝트 중단 리스크 명시 -->
- **데이터 품질/양 미달 위험:** execution_events 내 반복 실패 패턴 추출에 필요한 데이터가 부족하거나 저품질이면, M1 결과에 따라 전체 프로젝트가 중단될 수 있음.

---

## 3. 사용자 및 이해관계자

### 3.1 주 사용자
| 구분 | 설명 | 주요 니즈 |
|------|------|-----------|
| Claude Code 에이전트 | `.claude/rules/`를 자동 로딩하여 세션별 행동을 결정하는 AI 에이전트 | 최신 패턴이 반영된 Rule을 자동으로 준수하여 반복 실패 예방 |

### 3.2 이해관계자
| 구분 | 역할 | 영향도 |
|------|------|--------|
| Sinclair Seo | 승인 게이트 — Rule 제안을 세션 내에서 승인/거부 | 높음 |
| AX BD팀원 | 공유 Rules의 간접 수혜자 — 에이전트 품질 향상의 영향 | 중간 |
<!-- CHANGED: 추가 승인자/운영자, 보안/감사 담당 등 다중 승인자/운영자 명시 -->
| 다중 승인자 (예: BD팀 내 2인 이상) | Rule 승인 분담 및 대체 승인, 승인 내역 리뷰 등 운영 | 중간 |
| 보안/감사 담당 | Rule 파일 쓰기, 승인 플로우 접근 권한 관리 및 감사 로그 모니터링 | 중간 |

### 3.3 사용 환경
- 기기: PC (WSL/Linux 환경)
- 네트워크: 인터넷 (Cloudflare Workers + D1)
- 기술 수준: 개발자 (Claude Code 세션 내 CLI 인터페이스)
<!-- CHANGED: CLI UX 및 보안 정책 적용 환경 명시 -->
- CLI UX: `/ax:session-start` 통합 승인 플로우에 대해 직관적·간결한 명령줄 경험 제공 필요
- 보안/권한: 파일 시스템 권한, 승인 플로우 접근통제, 감사 로그 환경 내재화

---

## 4. 기능 범위

### 4.1 핵심 기능 (Must Have)

| # | 기능 | 설명 | 우선순위 | 선행 조건 |
|---|------|------|----------|-----------|
| M1 | **데이터 상태 진단 (Phase 0)** | execution_events, task_histories D1 데이터의 양·기간·품질을 진단. 반복 실패 패턴 추출 가능 여부 확인. 기준선(baseline) 수립 | P0 | — |
| M2 | **반복 실패 패턴 감지 엔진** | execution_events에서 동일 유형 실패를 자동 클러스터링. 임계값(최소 N회 반복) 기반 패턴 식별. event_type × error_category × task_family 3차원 분석 | P0 | M1 |
| M3 | **Rule 초안 생성기** | 감지된 패턴을 `.claude/rules/` 포맷의 Rule 초안으로 변환. 패턴 근거(실패 사례 N건)를 주석으로 포함. LLM 호출로 자연어 Rule 생성 | P0 | M2 |
| M4 | **세션 내 승인 플로우** | `/ax:session-start`에 통합 — "새 Rule 제안 N건" 알림 → 각 Rule의 근거·내용 표시 → 승인/거부/수정 → 승인된 Rule을 `.claude/rules/`에 자동 배치 | P0 | M3 |
| M5 | **O-G-D Loop 범용 인터페이스** | 도메인 독립적인 O-G-D 호출 API 설계. `OGDRequest { domain, input, rubric, maxRounds }` → `OGDResult { output, score, iterations }`. BD 형상화·코드리뷰·문서검증·Prototype 4개 도메인 어댑터 | P0 | Phase 16 F355 완료 |
| M6 | **O-G-D 도메인 어댑터 레지스트리** | 새 도메인 추가를 위한 어댑터 등록/조회 API. 각 어댑터는 Rubric + Generator + Discriminator 3요소를 정의 | P0 | M5 |
<!-- CHANGED: 데이터 품질 실시간 모니터링 및 개선 요청 기능 추가 -->
| M7 | **데이터 품질 실시간 모니터링 및 개선 요청** | execution_events, task_histories 등 핵심 데이터의 이상치, 누락, 편향을 실시간 감지하여 알림·자동 개선 요청. 품질 저하 시 관리자/운영자에게 경고 및 대응 프로토콜 제공 | P0 | M1 |
<!-- CHANGED: 승인자 분산 및 자동화 보조 기능 추가 -->
| M8 | **다중 승인자 분산 및 자동화 보조** | Rule 승인 플로우에 다중 승인자(2인 이상) 지정, 승인 내역 기록, 자동화 추천/우선순위 정렬, 승인자별 업무 분배 기능 | P0 | M4 |
<!-- CHANGED: 보안 및 권한 관리, 감사 로그 기능 추가 -->
| M9 | **보안·권한 관리 및 감사 로그** | `.claude/rules/` 파일 쓰기·승인 플로우 접근 권한 분리, 변경 내역 및 승인 기록의 상세 감사 로그 자동화 | P0 | M4 |

### 4.2 부가 기능 (Should Have)

| # | 기능 | 설명 | 우선순위 | 선행 조건 |
|---|------|------|----------|-----------|
| S1 | **에이전트 자기 평가 연동** | F148(에이전트 자기 평가) 데이터를 Guard Rail 패턴 감지에 입력으로 연결. 에이전트가 보고한 "어려웠던 점"을 패턴 클러스터링의 가중치로 활용 | P1 | M2 |
| S2 | **Skill Evolution 운영 지표 대시보드** | DERIVED/CAPTURED 스킬의 생성 수 vs 실제 재사용 수 비율. skill_executions × skill_lineage JOIN으로 재사용률 산출. F337 Dashboard에 탭 추가 | P1 | — |
| S3 | **에이전트/스킬 활용률 대시보드** | 16종 에이전트 × 15종 스킬의 월간 사용 빈도 집계. 미사용(월 0회) 항목 하이라이트. F337 Dashboard에 탭 추가 | P1 | — |
| S4 | **Rule 효과 측정** | 배치된 Rule의 "적용 전 N주 vs 적용 후 N주" 동일 유형 실패 빈도 비교. Rule별 효과 점수 산출 | P1 | M4 |
<!-- CHANGED: Rule 충돌 감지 및 복잡성 관리, Rule 생성 품질 보완 기능 추가 -->
| S5 | **Rule 충돌 감지 및 복잡성 관리** | 다수의 Guard Rail Rule 간 충돌, 중복, 비효율 발생 시 자동 감지 및 관리자 알림, 충돌 해결 추천 | P2 | M4 |
| S6 | **Rule 생성 품질 보완 및 투명성 제고** | LLM Rule 생성 프롬프트 및 근거 데이터, 평가 기준, 예외·오류 사례에 대한 투명한 설명 제공. Rule 수동 수정 내역이 LLM 생성 로직에 피드백되어 품질이 지속 개선 | P2 | M3 |
<!-- CHANGED: 에이전트 의도/제약조건 활용, XAI 연동 -->
| S7 | **에이전트 의도·제약조건 Rule 반영** | 에이전트 설계 Intent, Constraints를 Rule 생성 입력에 활용하여 선제적 Guard Rail 생성 | P2 | M3/S1 |
| S8 | **Adversarial Robustness(악용 방지) 옵션** | 입력 데이터가 악의적 공격/오류로 인한 잘못된 Rule을 유발하지 않도록, 입력 검증/이상 탐지 및 관리자 승인 단계 강화 | P2 | M7/M9 |

### 4.3 제외 범위 (Out of Scope)

| 항목 | 제외 이유 | 재검토 조건 |
|------|----------|-----------|
| `.harness/events/` Git 기반 이벤트 저장 | D1이 더 실용적, Git 커밋 노이즈 우려 | CI/CD에서 D1 없이 하네스 상태 확인 필요 발생 시 |
| Guard Rail 완전 자동화 (사람 승인 없이) | 자동 생성 Rule의 품질 보장 불가 | Guard Rail 반자동 운영 6개월+ 후 |
| 에이전트 자율 커밋 | "인간은 판사" 원칙(전략 문서 원칙 3) | 조직 정책 변경 시 |
| 범용 하네스 프로파일 시스템 | 에이전트 16종 역할 분화로 사실상 달성 | 에이전트 30종+ 초과 시 |

### 4.4 외부 연동

| 시스템 | 연동 방식 | 필수 여부 |
|--------|-----------|-----------|
| D1 (execution_events) | SQL 쿼리로 실패 이벤트 조회 | 필수 |
| D1 (task_histories) | SQL 쿼리로 태스크 이력 조회 | 필수 |
| `.claude/rules/` | 파일시스템 Write | 필수 |
| `/ax:session-start` 스킬 | 승인 플로우 통합 | 필수 |
| F337 Orchestration Dashboard | 대시보드 탭 추가 (S2, S3) | 선택 |
| LLM API (Anthropic/OpenRouter) | Rule 초안 생성 시 자연어 변환 | 필수 |
| Phase 14 O-G-D 인프라 | 기존 O-G-D Loop 코드 재활용 | 필수 |
| Phase 16 F355 결과 | O-G-D 범용화의 첫 재활용 사례 입력 | 필수 (M5) |

---

## 5. 성공 기준

### 5.1 정량 지표 (KPI)

| 지표 | 현재값 | 목표값 | 측정 방법 |
|------|--------|--------|-----------|
| 반복 실패 감소율 | [기준선 측정 필요] | ≥ 30% 감소 | Guard Rail 배치 전 N주 vs 후 N주 동일 유형 실패 빈도 비교 |
| Guard Rail 채택률 | — | > 50% | 자동 제안 Rule 중 사람 승인 비율 |
| O-G-D 도메인 간 재활용 수 | 1 (BD) | ≥ 3 | 등록된 O-G-D 도메인 어댑터 수 (BD + Prototype + 2개 이상) |
| 에이전트 활용률 | [미측정] | > 75% 월 1회+ | 16종 중 월 1회 이상 사용된 에이전트 비율 |
| DERIVED 스킬 재사용률 | [미측정] | > 30% | skill_executions × skill_lineage에서 생성 대비 재실행 비율 |
<!-- CHANGED: 데이터 품질 모니터링, 승인자 분산, 보안 감사 등의 KPI 추가 -->
| 데이터 품질 이상 탐지 건수 | [미측정] | 월별 이상치/누락 발생 시점 100% 탐지 | 실시간 이상 감지 및 대응 기록 |
| Rule 승인 분산도 | — | 2인 이상 승인자 분산률 ≥ 80% | 승인자별 승인 건수 집계 |
| 변경/접근 감사 로그 완결성 | — | 100% 기록 | 주요 파일/플로우 변경 내역 자동 기록률 |

### 5.2 MVP 최소 기준

- [ ] M1: execution_events 기준선 측정 완료 (데이터 양, 기간, 패턴 추출 가능성)
- [ ] M2+M3: 최소 1개 반복 실패 패턴 자동 감지 → Rule 초안 생성 성공
- [ ] M4: 세션 시작 시 Rule 제안 알림 + 승인 후 `.claude/rules/` 배치 동작
- [ ] M5: O-G-D 범용 인터페이스로 BD 외 1개 도메인(코드리뷰 또는 문서검증) 호출 성공
<!-- CHANGED: M7~M9 및 보조 기능의 MVP 포함 필요성 강조 -->
- [ ] M7: 데이터 품질 실시간 모니터링 및 이상 탐지 알림 동작
- [ ] M8: 최소 2인 승인자 분산 및 승인 자동화 보조 테스트
- [ ] M9: Rule 파일 변경 및 승인 플로우 접근 감사 로그 자동 저장

### 5.3 실패/중단 조건

- execution_events에 패턴 추출 가능한 데이터가 3개월 분량 미만 → M1에서 중단, 데이터 축적 후 재시도
- Guard Rail 채택률이 3개월간 20% 미만 → 제안 품질 재검토 또는 접근 방식 변경
- O-G-D 범용화에서 2개 이상 도메인에서 Rubric 정렬 실패 → 범용화 대신 도메인별 독립 구현으로 전환
<!-- CHANGED: 데이터 품질/이상치 미탐지, 승인 및 보안 기능 미비 추가 -->
- 데이터 이상치·누락·편향 발생 시 실시간 미탐지, 또는 품질 개선 미이행 → 데이터 모니터링 체계 개선 전 전체 중단
- 승인자 단일화로 인한 병목·과부하·휴가 시 승인 지연/정체 → 승인 플로우 분산 또는 자동화 보조 개선 전 전체 중단
- Rule 파일 접근·변경에 대한 감사 로그 미기록 시, 즉시 운영 중단 및 감사 체계 보완 전 재개 금지

---

## 6. 제약 조건

### 6.1 일정

- **시작 시점:** Phase 16 완료 후 (F351~F356 전체 merge)
- **선행 조건:** F355(O-G-D 품질 루프) 성공적 운영 → M5의 입력
- **예상 규모:** 5~7 Sprint (M1~M6: 4~5 Sprint + S1~S4: 2 Sprint)
<!-- CHANGED: 데이터 품질/양 충분성 및 LLM Rule 품질 PoC가 선행 조건임을 명시 -->
- **추가 선행:** execution_events 데이터 품질·양·기간 검증(M1), LLM 기반 Rule 생성 품질 PoC 성공이 필수

### 6.2 기술 스택

- 프론트엔드: React 18 + Vite 8 (F337 Dashboard 확장)
- 백엔드: TypeScript + Hono (Cloudflare Workers)
- DB: D1 (execution_events, task_histories, 신규 테이블)
- 인프라: Cloudflare Workers + D1 + Pages
- 기존 의존: Phase 14 O-G-D 인프라, F148 에이전트 자기 평가, Phase 10 Skill Evolution
<!-- CHANGED: 보안/권한, 실시간 모니터링, 감사 로그, Rule 충돌 감지 등 추가 모듈 기술 스택 명시 -->
- 보안/권한: 파일시스템 ACL, 승인 플로우 접근 통제, 감사 로그 모듈
- 실시간 모니터링: 이상치 탐지(통계·ML), 관리자 알림(Email/Slack), 품질 개선 워크플로우

### 6.3 인력/예산

- 투입: AX BD팀 (AI 에이전트 기반 개발, Sprint worktree 자동화)
- LLM 비용: Rule 초안 생성 시 Haiku 모델 예상 ~$0.1/건
- API 비용: O-G-D Loop 호출 시 기존 Phase 14 비용 구조 유지
<!-- CHANGED: 승인자 분산, 보안 감사 담당, 데이터 모니터링 운영 인력 필요 명시 -->
- 승인자 분산: 2인 이상 승인자 지정 필요, 승인 내역 검토 및 분산 업무 프로세스 포함
- 보안/감사: 파일 접근 및 승인 플로우 감사 담당자 필요
- 데이터 모니터링: 이상 탐지 및 개선 요청 운영 인력 예산(월 0.2~0.3 FTE)

### 6.4 컴플라이언스

- **자동 커밋 절대 금지:** Guard Rail이 자동 생성하더라도, `.claude/rules/`에 배치 후 커밋은 사람이 수행
- **Rule 감사 추적:** 생성된 Rule의 근거(패턴 N건, 실패 이벤트 ID)를 D1에 기록하여 감사 가능
<!-- CHANGED: 승인 내역, 파일 변경, 승인 플로우 접근 등 모든 중요 행동의 감사 로그 필수화 및 권한 분리 명시 -->
- **승인 내역 및 파일 변경 감사:** 모든 Rule 승인/거부/수정/배치 및 파일 접근·변경 내역을 D1 및 별도 감사 로그에 자동 저장, 승인자/작성자/수정자 구분 기록
- **권한 분리:** 파일 쓰기, Rule 승인, 승인 플로우 접근은 역할별 최소 권한 원칙 적용

---

## 7. 마일스톤 구성 (안)

| Sprint | F-items | 핵심 산출물 | 의존성 |
|--------|---------|------------|--------|
| Sprint A | M1 (데이터 진단) | 기준선 보고서, D1 쿼리 세트, 패턴 추출 PoC | — |
| Sprint B | M2+M3 (패턴 감지 + Rule 생성) | PatternDetector 서비스, RuleGenerator 서비스, D1 신규 테이블 | Sprint A |
| Sprint C | M4 (승인 플로우) | session-start 통합, Rule 배치 자동화, 효과 측정 기반 | Sprint B |
| Sprint D | M5+M6 (O-G-D 범용화) | OGDInterface, DomainAdapter 레지스트리, 2+ 도메인 어댑터 | Phase 16 F355 |
| Sprint E | S1+S2+S3 (운영 지표) | Dashboard 탭 3건, 에이전트 활용률 집계, Skill 재사용률 | — |
| Sprint F | S4 (Rule 효과 측정) + 통합 QA | Rule 효과 점수, 통합 테스트, E2E | Sprint C+D |
<!-- CHANGED: 데이터 품질 모니터링, 승인자 분산, 보안/감사, Rule 충돌감지 등 신규 기능 마일스톤 명시 -->
| Sprint G | M7+M8+M9 (데이터 품질 모니터링, 승인자 분산, 보안/감사) | 실시간 모니터링 시스템, 분산 승인자, 감사 로그 | Sprint B+C |
| Sprint H | S5~S8 (Rule 충돌감지, 품질 보완, 의도/제약, 악용 방지) | Rule 충돌감지, 피드백 개선, XAI 기반 추론, Adversarial 옵션 | Sprint F+G |

---

## 7. 오픈 이슈

| # | 이슈 | 담당 | 마감 |
|---|------|------|------|
| 1 | execution_events 데이터 양/기간 확인 — 패턴 추출 가능성 진단 필요 | M1에서 해결 | Sprint A |
| 2 | 반복 실패 "유형" 분류 기준 정의 — event_type × error_category × task_family 조합이 적절한지 | M2 설계 시 | Sprint B |
| 3 | Rule 초안 생성에 사용할 LLM 모델 선택 — Haiku(비용) vs Sonnet(품질) 트레이드오프 | M3 구현 시 | Sprint B |
| 4 | O-G-D 범용화 시 기존 Phase 14 코드의 리팩토링 범위 — 범용 인터페이스 추출 vs 새로 구현 | M5 설계 시 | Sprint D |
| 5 | Phase 16 F355 결과가 범용화 설계에 어떤 교훈을 주는지 — F355 완료 후 반영 | Phase 16 완료 시 | Sprint D 전 |
<!-- CHANGED: 데이터 품질 모니터링, 승인자 부담, 보안/감사 등 신규 이슈 추가 -->
| 6 | execution_events 데이터 품질 저하(이상치, 누락, 편향 등) 실시간 탐지 및 대응 방안 구체화 | M7 설계 시 | Sprint G |
| 7 | Rule 승인자 분산(2인 이상), 승인 자동화 보조(추천/우선순위) UI/UX 설계 | M8 설계 시 | Sprint G |
| 8 | `.claude/rules/` 파일 접근 권한 및 감사 로그 자동화 설계 | M9 설계 시 | Sprint G |
| 9 | Rule 충돌·중복·비효율 감지 알고리즘 및 관리자 대응 프로토콜 | S5 설계 시 | Sprint H |
| 10 | LLM Rule 생성 품질 보완을 위한 인간 피드백(수정 내역)→LLM 재학습 설계 | S6 설계 시 | Sprint H |
| 11 | 에이전트 Intent/Constraints와 Rule 자동 생성의 연계 설계 | S7 설계 시 | Sprint H |

---

## 8. 리스크 및 고려사항

<!-- CHANGED: 결함, 누락, 리스크 검토 의견 기반 신규 리스크 섹션 신설 및 상세 명시 -->
| # | 리스크 | 상세 내용 | 대응 방안 |
|---|--------|----------|----------|
| 1 | 데이터 품질·양 부족 | execution_events, task_histories 데이터가 반복 실패 패턴 추출에 충분하지 않으면 전체 프로젝트 중단 | M1에서 기준선 진단 및 지속적 데이터 모니터링(M7), 데이터 품질 확보 전 착수 금지 |
| 2 | Rule 자동 생성 품질 불확실성 | LLM 기반 Rule 초안이 부정확/비문맥적/오류 Rule 생성 시 채택률 저하 | LLM 품질 PoC(M3), Rule 근거 명확화, 인간 피드백(수정)→LLM 재학습 루프(S6) |
| 3 | 승인자 업무 과부하·병목 | 단일 승인자 집중 시 부재/과부하로 승인 지연·정체 | 다중 승인자 분산(M8), 승인 자동화 추천 및 승인 내역 리뷰 도입 |
| 4 | 보안 및 권한 관리 미흡 | Rule 파일 무단 변경, 승인 플로우 비인가 접근, 감사 미비 | 권한 분리(M9), 변경/승인 내역 감사 로그 자동화, 접근 통제 강화 |
| 5 | 데이터 이상치·편향·오류 전파 | 데이터 편향/이상치가 Rule 생성에 오탐·누락·역효과 Rule 유발 | 실시간 데이터 품질 모니터링(M7), 입력 전처리, 관리자 알림 및 검증 프로세스 |
| 6 | O-G-D 범용 인터페이스 실패 | Phase 16 F355 결과가 범용화에 부적합 시 일정 지연/도메인별 독립 구현 전환 | F355 결과 반영 전 범용화 설계 금지, 실패 시 독립 구현 플랜 |
| 7 | 인력/일정 병목 | 복합 기능 동시 개발 및 인력 이탈로 전체 프로젝트 지연/중단 | Sprint별 인력 분산, 핵심 인력 백업 지정, 일정 버퍼 확보 |
| 8 | Rule 충돌/복잡성 증대 | Rule 수 증가 시 예상치 못한 충돌, 비효율, 품질 저하 | Rule 충돌 감지(S5), 관리자 알림 및 해결 프로세스 |
| 9 | UX 및 투명성 저하 | 승인 플로우/패턴 감지/Rule 생성이 불투명하거나 복잡하면 신뢰·사용성 저하 | CLI UX 단순화, 감지 논리/LLM 프롬프트/Rule 적용 방식 문서화 |
| 10 | AI-인간 협업 피드백 루프 약화 | Rule 수동 수정이 LLM 품질 개선에 반영되지 않으면 품질 정체 | 수정 내역→LLM 재학습 파이프라인(S6) 구현 |
| 11 | Adversarial Input 악용 위험 | 외부/비정상 입력이 잘못된 Rule을 유발할 위험 | 입력 검증, 관리자 승인 단계 강화(S8) |

---

## 9. Out-of-scope

<!-- CHANGED: 범위 밖 요청 명시 (지적된 Out-of-scope 요청 없음) -->
- 없음

---

## 10. 검토 이력

| 라운드 | 날짜 | 주요 변경사항 | 스코어 |
|--------|------|--------------|--------|
| 초안 | 2026-04-06 | 인터뷰 기반 최초 작성. 전략 문서 FX-STRT-015 v3.0 입력 | — |
<!-- CHANGED: v2 변경사항 및 결함·누락 반영 이력 추가 -->
| 2차 | 2026-04-07 | 데이터 품질 모니터링, 승인자 분산, 보안/감사, 성능/확장성, Rule 품질·충돌관리 및 리스크 섹션 반영 | Conditional |

---

*이 문서는 requirements-interview 스킬에 의해 자동 생성 및 관리됩니다.*
*입력 문서: `docs/specs/self-evolving-harness-strategy.md` (FX-STRT-015 v3.0)*

---