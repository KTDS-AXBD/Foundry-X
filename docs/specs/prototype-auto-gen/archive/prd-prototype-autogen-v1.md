# PRD → Prototype 자동 생성 & 배포 — 현황 분석 및 구현 계획
...
**기존 API 방식 대비 절감 효과:**
- Claude API 직접 호출 시 Prototype 1건당 예상 비용: $2~5 (Sonnet 기준, 코드 생성 + 수정 반복)
- 월 20건 기준: $40~100/월 절감
- 연간: ~$500~1,200 절감
...
- [ ] Max 구독 계정의 비대화형 CLI 사용 허용 여부 확인
- [ ] Max 구독의 일일/시간당 rate limit 확인 (Prototype 건당 ~30턴 소요 추정)
- [ ] Builder Server 호스팅 환경 결정 (자체 서버 vs VPS vs 사내 인프라)
- [ ] Cloudflare Pages 프로젝트 자동 생성 API 권한 확인
...
| 간단한 HTML 시작 | React SPA + Cloudflare Pages | **스코프 확장** — 템플릿 기반으로 초기부터 완성도 높은 산출물 |
