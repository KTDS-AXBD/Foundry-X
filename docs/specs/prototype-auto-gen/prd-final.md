...
**버전:** Final (v4 — 오픈 이슈 해소 반영)
**날짜:** 2026-04-06
**문서 ID:** FX-PLAN-PROTO-001
**작성자:** AX BD팀
**상태:** ✅ 착수 준비 완료
**검토 이력:** R1(63) → R2(69) → Six Hats → R3(68, Gemini Ready) → 오픈 이슈 조사 완료(5/6 해소)
...
| 시스템 | 연동 방식 | 필수 여부 |
|--------|-----------|-----------|
| Claude Code CLI | subprocess (`ANTHROPIC_API_KEY`, `--bare` 모드) | 필수 |
| Claude API | REST API (동일 API 키, 모델 직접 호출 Fallback) | 필수 |
...
| 월간 API 비용 | N/A | ≤ $100/월 | Haiku 주력 시 건당 ~$0.5~1, 월 20건 기준 ~$10~20 |
...
### 5.3 실패/중단 조건
- Claude API 비용이 월 $500을 초과하고, 비용 최적화(모델 다운그레이드, 턴 수 축소)로도 해결 불가한 경우
...
- **Generator**: Claude Code CLI (`--bare` + `ANTHROPIC_API_KEY`) / Claude API 직접 호출 (Fallback)
...
| 비용 항목 | 금액 | 비고 |
|-----------|------|------|
| Anthropic API 키 | ~$10~50/월 | Haiku 주력 (건당 ~$0.5~1), Sonnet 최종 Round만 |
| Builder Server (VPS) | ~$20~50/월 | 8GB RAM, 50GB SSD |
| Cloudflare Pages | 무료 (500 deploys/월) | 프로젝트당 무료 |
| 다중 AI Fallback 예산 | ~$20~50/월 (비상) | GPT-4/Gemini 전환 시에만 발생 |

### 6.4 보안/컴플라이언스
- Builder Server에 `ANTHROPIC_API_KEY` + Cloudflare API Token → **환경변수 관리, 최소 권한 원칙**
...
**"API 키 기반 + Haiku 비용 최적화 + 다중 Fallback"** — `ANTHROPIC_API_KEY`로 Claude Code CLI를 `--bare` 모드로 서버에서 실행. Haiku 모델 주력으로 건당 비용 최소화, CLI 실패 시 Claude API 직접 호출 또는 다중 AI로 전환.

> **설계 변경 (2026-04-06)**: 원안의 "Max 구독 기반 $0" 전략은 Anthropic ToS 조사 결과 서버 자동화에 부적합 — OAuth 토큰 8~12시간 만료, Agent SDK에서 구독 OAuth 금지 (2026-02 정책). API 키 방식으로 전환.
...
└──────────────────────┬───────────────────────────────────┘
                       ...
                       ▼
┌──────────────────────────────────────────────────────────┐
│    Prototype Builder Server (Linux, ANTHROPIC_API_KEY)    │
│                                                          │
...
### 7.2 비용 최적화 전략 + Fallback 아키텍처

**모델별 비용 (API 키 기반, 2026-04 기준):**

| 모델 | 입력 | 출력 | Prototype 1건 추정 (~30턴) |
|------|------|------|---------------------------|
| Haiku 4.5 | $0.80/MTok | $4/MTok | ~$0.5~1 |
| Sonnet 4.6 | $3/MTok | $15/MTok | ~$2~5 |
| Opus 4.6 | $15/MTok | $75/MTok | ~$10~25 |

**최적화 전략**: Generator는 **Haiku** (속도+비용), 최종 Round만 **Sonnet** (품질). 월 20건 기준 **~$10~20/월**.

**3단계 Fallback:**

```
Primary:   Claude Code CLI --bare (ANTHROPIC_API_KEY, Haiku, ~$0.5~1/건)
    │ 실패 (CLI 오류 / 빌드 반복 실패)
    ▼
Fallback 1: Claude API 직접 호출 (Sonnet, ~$2~5/건)
    │ 실패 (API 장애 / rate limit)
    ▼
Fallback 2: 다중 AI 앙상블 (GPT-4 / Gemini, ~$3~8/건) [Phase D]
```
...
| 항목 | 결과 |
|------|------|
| `--print --dangerously-skip-permissions` 동작 | ✅ 정상 (서버 환경에서는 `--bare --allowedTools` 권장) |
| CLI 비대화형 실행 | ✅ 동작 확인 (서버에서는 `ANTHROPIC_API_KEY` 인증 필수) |
...
| # | 리스크 | 영향 | 완화 방안 |
|---|--------|------|-----------|
| R1 | **API 비용 증가** — Haiku→Sonnet 전환 또는 사용량 증가 시 비용 상승 | 🟡 예산 초과 | Haiku 주력 비용 최적화, 월 예산 상한 설정, 사용량 모니터링 |
| R2 | **API Rate Limit** — 동시 요청 제한 초과 | 🟡 생성 지연 | 큐 순차 처리, 야간 배치, 다중 AI Fallback |
...
| # | 이슈 | 담당 | 상태 | 해소 결과 |
|---|------|------|------|-----------|
| 1 | Anthropic CLI 서버 자동화 허용 여부 | BD팀 | ✅ 해소 | **허용** — CLI는 공식 자동화 도구. 단, 서버에서는 API 키 필수 (구독 OAuth 불가). `--bare` 모드 권장 |
| 2 | Max 구독 Rate Limit | BD팀 | ✅ 해소 | Max 5x: 225msg/5h, Max 20x: 900msg/5h (claude.ai+CLI 공유). **본 PRD는 API 키 방식으로 전환하여 구독 제한 무관** |
| 3 | `--dangerously-skip-permissions` 장기 지원 | BD팀 | ✅ 해소 | **Auto Mode로 대체 예정**. 서버에서는 `--bare --allowedTools --permission-mode` 조합 사용 |
| 4 | 생성 코드 IP/라이선스 | BD팀/법무 | ✅ 해소 | API 키 사용 시 Commercial Terms에서 **상업적 사용 명시적 허용**. 멀티 모델 라우팅도 ToS 준수 |
| 5 | Builder Server 호스팅 환경 결정 | 인프라팀 | ✅ 해소 | **WSL 로컬** (PoC/MVP). Docker 설치 필요. 프로덕션 전환 시 VPS 검토 |
| 6 | Cloudflare Pages 프로젝트 자동 생성 API | DevOps | ✅ 해소 | `wrangler pages deploy` 시 프로젝트 자동 생성. REST API도 지원 (`POST /accounts/{id}/pages/projects`) |
...
| v4 (Final) | 2026-04-06 | 오픈 이슈 5/6 해소: API 키 전환, 비용 모델 변경, CLI 플래그 현행화 | Ready |
...
│  ┌─── Round Loop (max 3) ─────────────────────────────┐  │
│  │                                                     │  │
│  │  Generator (claude --bare -p, Haiku/Sonnet):         │  │
│  │    --allowedTools "Bash,Read,Edit,Write"             │  │
│  │    - Round 0: 전체 생성 (max-turns 15)              │  │
│  │    - Round N: 새 파일로 전체 재생성 (Edit 금지)      │  │
│  │                                                     │  │
│  │  Discriminator (claude --bare -p, Haiku):            │  │
