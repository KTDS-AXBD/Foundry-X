...
**참조:** `docs/specs/AI_Foundry_OS_DeepDive_v0.3.html`, **Decode-X 레포** (https://github.com/KTDS-AXBD/Decode-X — "AI Foundry"는 상위 포털명, Decode-X는 Input Plane 정찰 엔진)

---

## 0. 착수 전 선행 확보 (Sinclair 직접, H+2 마감)

> 2026-04-15 Decode-X 레포 스캔 결과 확정된 정보를 기반으로 한 체크리스트.

- [x] `wrangler whoami` — ✅ **계정 일치 확정** (2026-04-16): 양쪽 모두 `b6c06059b413892a92f150e5ca496236` / ktds.axbd@gmail.com. **Service Binding 경로 확정** → F546은 `[[services]] binding = "SVC_EXTRACTION"` 방식으로 구현 (fetch+secret 불필요)
- [x] `wrangler deployments list` — ✅ **3개 서비스 배포 확정** (2026-04-16 확인): svc-extraction / svc-ontology / svc-mcp-server 모두 2026-03-19 배포 + 이후 secret rotation. Worker 이름 정확 일치 → Service Binding config 그대로 사용 가능
- [x] `wrangler secret list` — ✅ **시나리오 B 확정** (2026-04-16): 3개 서비스 모두 `INTERNAL_API_SECRET` 존재. **Service Binding 경로라도 `X-Internal-Secret` 헤더 필수** (routes.test.ts:286 `returns 401 without X-Internal-Secret`). Decode-X 자체 서비스 간 binding도 header 전달 패턴 사용 (doc-spec-extractor.ts, source-aggregator.ts). F546 구현 시 Foundry-X에 `INTERNAL_API_SECRET` 값 동기 필요 (Sinclair 최초 설정 값 확보)
- [x] svc-ontology에 **`NEO4J_DATABASE` secret 실존** (2026-04-16) — KG 실데이터 운영 중. **F549 mock → 실데이터 상향 가능** (svc-ontology Neo4j 조회 라우트 확인 필요)
- [⏸️] svc-mcp-server 엔드포인트 확인 — **Sprint 298 Design 단계 이월** (P2). MCP 프로토콜 구현은 42시간 내 부담. F549 KG XAI 뷰어는 svc-ontology HTTP API 경유로 충분
- [⏸️] Decode-X `docs/AI_Foundry_PRD_TDS_v0.7.4.docx` + `docs/AI_Foundry_Identity.md` 접근 — **Sprint 298 내 처리** (용어 동기화는 F545 랜딩 페이지 카피 작성 시점에)
- [x] 파일럿 도메인 확정 — ✅ **LPON(온누리상품권 취소) 선정** (2026-04-16): Decode-X `반제품-스펙/pilot-lpon-cancel/` 에 **완성된 Type 1 반제품** 이미 존재 (01-business-logic.md ~ 06-screens.md 6종 Spec + working-version/src/ Code + __tests__/ Test + migrations/0001_init.sql + docs/api.md). 퇴직연금은 DeepDive v0.3 A-4 예시이나 실구현물 없음 → LPON 전환으로 **"이미 돌아가는 내외 시연" 즉시 가능**. F547 시연 페이지 `/ai-foundry-os/demo/lpon`으로 재정의. DeepDive와 불일치는 보고에서 "실제 완성된 사례는 LPON입니다. 퇴직연금은 A-4의 설명 예시" 로 설명

**해소 상태:** ✅ **전부 완료** (2026-04-16). P0 필수 4건 해소, P2 이월 2건. Sprint 298 착수 가능.
...
**한 줄 정의:**
AX사업1/2팀 수행인력이 SI/ITO 고객사에 투입될 때 "가방에 넣어가는 3대 자산(Harness·AI Engine·Ontology) + Spec"을 현실에서 돌아가는 제품으로 만들고, '26-04-17 대표 보고에서 "온누리상품권 취소(LPON)" Type 1 반제품을 시연한다.
...
**목표:**
- **1차(42시간, 4/17 09:00):** 대표 보고 데모 가능 — 3-Plane 통합 대시보드 + LPON Type 1 산출 플로우가 Foundry-X 웹에서 시연됨
...
### 2.2 목표 상태 (To-Be)
- **대표 보고(4/17 09:00) 시연 경로**: Foundry-X 웹 → "AI Foundry OS" 진입 → Decode-X API 호출 → "온누리상품권 취소(LPON)" 문서/코드/암묵지 → 3종 Spec 산출 → Type 1 반제품(Code+Test) 다운로드가 화면에서 1-path로 돌아감.
...
| 3 | **"LPON Type 1" 시연 페이지** | `/ai-foundry-os/demo/lpon` — 3-Pass 결과 (scoring·diagnosis·comparison) 시각화 + "Type 1 반제품(코드+테스트) 다운로드" 버튼 | P0 |
...
| LPON Type 1 시연 | - 3-Pass 분석 결과(Scoring/Diagnosis/Comparison)가 정상 시각화되는지 확인<br>- "Type 1 반제품(코드+테스트) 다운로드" 버튼 클릭 시 실제 zip 파일 다운로드 가능 여부<br>- 산출물 포맷(코드+테스트+spec zip) 및 파일 내 내용 일관성 검증 |
...
- **외부 고객사 실제 적용** — 이번 MVP는 kt ds 내부 1개 도메인(LPON)에 한정
...
|------|--------|---------------------|---------------------|----------|
| 시연 경로 완결성 | 0% | **100%** (3-Plane → LPON Type 1 다운로드까지 1-path 작동) | — | 수동 rehearsal 체크리스트 통과 |
| fx-decode-bridge 라우트 수 | 0 | **5개** (analyze/summary/findings/compare/core-processes) | 8+ | API 테스트 |
| Type 1 반제품 산출 건수 | 0 | **1건** (온누리상품권 취소(LPON)) | 3건+ | 다운로드 가능한 zip/directory |
...
- [ ] "라이브 시연" 버튼 클릭 시 LPON Type 1 경로로 진입
...
- LPON Type 1 데모 미완성(2차): 방법론 자기증명 실패 — 외부 수주 목표 재조정
...
- M3 (H+20~H+32): LPON 데모 페이지 + Type 1 반제품 생성 스크립트 + Harness UI
...
- **고객사 개인정보**: LPON 데모는 **가명·모크 데이터 전용** (Decode-X PRD 1.6 따라 org_id/document_id 모두 synthetic)
...
| 3 | "LPON Type 1 반제품"의 구체 산출물 포맷 — code-only? code+test+spec zip? (실제 zip 샘플 포함) | Sinclair + 대표 보고 담당 | H+8 |
...
| R3 | LPON Type 1 반제품 산출 실패 | 중 | 높음 | Decode-X 기존 분석 결과(있으면)를 그대로 "반제품 프리뷰"로 시각화, zip 생성은 스크립트 한 번 |
...
| R7 | AI 생성 코드 품질이 시연에 부적합 | 중 | 중 | Type 1 반제품은 AI 자동생성이 아니라 **Decode-X `반제품-스펙/pilot-lpon-cancel/working-version/` 디렉토리 패키징** 으로 1단계 단순화 |
...
- **F547** (P0): LPON Type 1 시연 페이지 + 반제품 생성 스크립트
...
- **실제 SI/ITO 고객사 도메인(LPON 외, 퇴직연금 등) 적용 확장**
