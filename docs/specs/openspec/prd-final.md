...
#### Implementation Phase 1: MVP (Sprint 103~104)

Track A(메트릭 수집) + Track D(스킬 레지스트리) — 검증된 기능 우선 구현

| Track | Sprint | 핵심 산출물 | 의존성 |
|-------|--------|-----------|--------|
| A: 메트릭 수집 | 103 | skill_executions 테이블 + 대시보드 연동 | F143, F158~F161 |
| D: 스킬 레지스트리 | 104 | ax-marketplace 확장 + 시맨틱 검색 | Track A |

#### Implementation Phase 2: PoC 결과 기반 (Sprint 105~107)

Track C(DERIVED+CAPTURED) + Track E(BD ROI) — PoC 검증 후 진행 여부 결정

| Track | Sprint | 핵심 산출물 | 의존성 | 진행 조건 |
|-------|--------|-----------|--------|-----------|
| C: DERIVED+CAPTURED | 105~106 | 패턴 추출 + 워크플로우 캡처 | Track A, O-G-D(F270~F273) | PoC에서 유의미한 패턴 추출 실증 |
| E: BD ROI 벤치마크 | 107 | Cold/Warm 비교 + ROI 정량화 | Track A, C | Track C 성공 시 |

#### PoC/Spike (Sprint 103 병행)

Sprint 103에서 Track A 구현과 병행하여 Track C의 핵심 가정을 검증한다:
- 기존 BD 스킬 10개 이상의 실행 데이터로 "반복 성공 패턴" 자동 추출 가능성 확인
- 추출된 패턴의 품질을 수동 작성 스킬과 비교 평가
- **PoC 통과 기준**: 추출된 패턴 중 50% 이상이 재사용 가치 있다고 판단(사용자 평가)
- **PoC 실패 시**: Track C를 룰 기반 단순 추출로 축소하거나 보류, Track A+D에 집중

**총 5 Sprint (103~107)**, O-G-D(Sprint 101~102) 이후 시작. Phase 분리로 리스크 관리.

> **착수 전략**: 3사 AI 검토에서 공통적으로 "Track C의 PoC 없이는 Ready 불가"라는 Conditional 판정. Phase 1(A+D) MVP를 먼저 구현하여 메트릭 인프라를 확보하고, 병행 PoC 결과에 따라 Phase 2(C+E) 진행 여부를 결정한다.
...
| 라운드 | 날짜 | 주요 변경사항 | 스코어 |
|--------|------|--------------|--------|
| 초안(v1) | 2026-04-02 | 인터뷰 기반 최초 작성. 참조: FX-ANLS-OPENSPACE-001 | 77 (R1) |
| v2 | 2026-04-02 | AI 3사 피드백 반영: 문제/목표 구체화, Traceability, 운영/장애/테스트/보안/교육/리스크 등 보강 | 77 (R2) |
| v3(final) | 2026-04-02 | Phase 분리 전략 적용: Phase 1(A+D MVP) → Phase 2(C+E PoC 기반). 3사 Conditional→Phase 분리 착수 확정 | - |
