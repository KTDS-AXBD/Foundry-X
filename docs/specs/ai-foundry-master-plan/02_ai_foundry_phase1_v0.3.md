...
#### 2.2.2 Ontology (온톨로지) — 시멘틱 레이어 (v0.3 재정의)

> v0.3 변경: 본 절은 BeSir 정합성 분석(A1)에 따라 **"도메인 지식의 그래프 표현"에서 "사람을 복제하는 시멘틱 레이어"로 재정의**됩니다.

온톨로지는 단순한 데이터 구조나 스키마가 아니라, **실제 그 데이터를 가지고 일하는 사람의 워크플로우·용어·업무 방식·룰을 담은 시멘틱 레이어**입니다. 즉 **온톨로지 = 사람을 복제하는 자산**.

- **본질 정의**: 도메인 담당자의 머릿속에 있는 워크플로우·용어·룰의 문서화·코드화
- **노드 타입(9 타입, v0.3)**: **Domain · Fact · Dimension · Workflow · Event · Actor · Policy · Support · Decision** (§3.4 상세)
- **현행만 보유**: 온톨로지는 현재 시점만. 과거 비교는 시멘틱 레이어 차원에서 의미 없음 (과거 데이터는 원천 RDB에)
- **데이터 표준화 입장**: RDB 표준화·정규화·현행화는 "이상에 가깝다" — 현실은 담당자 머릿속 암묵지(System Knowledge §2.2.5)
- **저장 (v0.3 변경)**: **메타 객체는 파일(Knowledge Map) + Git 버전관리**, 인스턴스/관계는 PostgreSQL. **Graph DB 미사용**(LLM의 그래프 쿼리 생성 한계 — §3.4.1)
- **활용**: LLM 컨텍스트 주입(retrieve), 정책 간 관계 추적, 도메인 학습 곡선 단축
- **공유 단위**: 도메인 단위 export/import 가능 (`.ontology.json` + `knowledge-map/` 디렉터리)
...
#### 2.2.5 System Knowledge (v0.3 신규 — 5번째 자산)

> v0.3 변경: 4-Asset Model → **5-Asset Model**로 격상. BeSir 정합성 분석(A3) 반영.

RDB·문서·표준 정책에 매핑되지 않는 **도메인 암묵지**를 별도 자산으로 관리합니다.

**예시**:
- "DSBR(대시보드) 테이블은 다른 솔루션이 만든 결과 데이터인데, 이 테이블을 신뢰해서 직접 쿼리해야 한다"는 운영 룰
- "신용평가 시 표준 모형 점수가 580 미만이어도 이 카테고리 신청자는 추가 룰로 재검토" 같은 부서 관행
- "월말 마감 직전엔 X 조회 쿼리는 피해야 한다 — 운영 DB 부하 우려" 같은 시간적 제약

**구조**:
- 단위: 자유 텍스트 + 메타데이터(작성자·시점·연관 엔티티·연관 정책·신뢰도)
- 저장: 파일 + Git (변경 이력 추적)
- 활용: AI가 retrieve해서 적절한 API/스킬을 생성하거나 의사결정 컨텍스트로 주입

**왜 별도 자산인가**:
- Policy Pack은 **명문화된 룰** (POL-* 코드)
- Ontology는 **개념 관계** (노드·관계 그래프)
- System Knowledge는 **명문화 안 된 암묵지** — 이게 빠지면 시스템이 현실과 어긋남
- BeSir 사례에서 가장 큰 가치를 만든 영역 (포스코DX 사례 참조)
...
| **P6. 분석을 넘어 자동화까지 한다 (v0.3)** | 데이터 원본 DB 직결 정책 — 분석만이 아니라 상태 변경(예: 프로젝트 상태 "리스크" 마킹, 구매 금액 증액)까지 지원. 복제 DB가 아닌 원천 DB와 직접 연결, 트랜잭션 보장 |
| **P7. 운영 호출 정합성은 CQ 5축으로 검증된다 (v0.3)** | 빌드 단계 4대 진단(P5)에 더해 운영 단계는 CQ(Critical Question) 5축 100점 평가로 검증, 평균 90점 이상이면 자동 핸드오프 (§4.5) |
...
### 3.4 Layer 2 — Ontology Layer (v0.3 전면 재작성)

> v0.3 변경: BeSir 정합성 분석(A2 + A6)에 따라 Layer 2를 **"Graph DB 기반 12 노드"에서 "파일+Git 기반 9 타입 시멘틱 레이어"로 전면 재작성**합니다.

#### 책임
- Layer 1 산출물을 도메인 시멘틱 레이어에 통합
- 신규 객체/관계 추론 + 기존 시멘틱 레이어와의 연결
- 파일 retrieve 인터페이스 (Knowledge Map 직접 조회 + LLM 컨텍스트 주입)
- **현행만 보유** — 과거 시점 비교는 시멘틱 레이어 차원에서 미지원

#### 9 Type Objects (v0.3 신규 — BeSir 7타입 + Domain + Decision)

```
1. Domain        도메인의 최상위 단위 (예: 신용평가, 운영, 정책관리)
2. Fact          숫자로 나타나는 객체 (매출액, 만족도, 처리량 등 계량 지표)
3. Dimension     Fact를 가르는 기준 (프로젝트별·고객별·시간별 — v0.3 신규 핵심)
4. Workflow      업무 흐름 (의사결정·처리 프로세스)
5. Event         시간 단위로 발생한 사건
6. Actor         주체/담당자 (사람 역할 + AI Agent)
7. Policy        룰·규칙·제약 (POL-* 코드 단위, 법령·내규·SLA 포함)
8. Support       위 타입을 보조하는 정보 (참조 자료·산출 문서·외부 링크)
9. Decision      특정 결정 인스턴스 (Decision Case와 연결 — §3.2.1)
```

> v0.2 12 노드와의 매핑: Fact ≈ Metric+Outcome / **Dimension 신규** / Workflow ≈ Process / Policy ≈ Policy+Constraint / Support ≈ Reference+Artifact

#### 3.4.1 Graph DB·Vector DB 미사용 결정 (v0.3 핵심 정책)

> **이유** (BeSir 회의록 발췌): "그래프 DB의 목적은 관계 탐색인데, LLM이 그래프 쿼리(Cypher)를 만드는 게 오히려 성능이 더 떨어집니다. 파일 기반 retrieve가 LLM 친화적입니다."

| 질문 | v0.3 결정 |
|---|---|
| Graph DB(Neo4j 등) 사용? | **미사용** — LLM의 Cypher 쿼리 생성 정확도 한계 |
| Vector DB 사용? | **미사용** — 임베딩 압축 시 시멘틱 의미 손실 |
| 그렇다면 무엇으로? | **파일(Knowledge Map) + Git 버전관리 + LLM이 retrieve** (Claude Code 동일 철학) |

#### 3.4.2 저장 아키텍처 (v0.3)

| 저장 대상 | 저장소 | 비고 |
|---|---|---|
| **온톨로지 메타 객체** (스키마·타입·관계 정의) | **파일 (Knowledge Map)** | Git 버전관리, AI가 retrieve |
| **인스턴스 데이터** (객체-객체 관계, 가중치) | **PostgreSQL** | 운영 DB, 빠른 쿼리 |
| **운영 로그 / 도구 호출 이력** | **PostgreSQL** | Browser 콘솔(§3.6) 조회 |
| **캐시** | **Redis** | 데이터 원천 캐싱 |
| **데이터 원천(Truth)** | **고객사 원본 DB (Connect)** | **복제하지 않음** — 자동화 정책(P6) |
| **System Knowledge (암묵지)** | **파일 + Git** | §2.2.5 |

#### 핵심 모듈 (v0.3 갱신)

| 모듈 | 역할 |
|---|---|
| **Knowledge Map Store** (변경) | 파일 시스템 기반 메타 객체 저장 + Git 버전관리. Graph Store 대체 |
| **Schema Manager** | 9 타입 + 관계 타입 스키마 |
| **Linker** | 신규 입력의 기존 객체 매칭 (entity resolution) |
| **Reasoner** | 추론 룰 기반 신규 관계 자동 생성 |
| **Retriever** (신규) | LLM이 자연어로 Knowledge Map을 retrieve — 파일 기반 탐색 |
| **Postgres Adapter** (신규) | 인스턴스/관계/가중치를 PostgreSQL에 저장·조회 |
| **Exporter / Importer** | `.ontology.json` + `knowledge-map/` 디렉터리 단위 export/import |

#### 입력 / 출력 / SLA
- **입력**: Document Schema 인스턴스 (Layer 1)
- **출력**: 9 타입 객체 + 관계 (with confidence) — 파일 변경 + Postgres 인스턴스 동기 갱신
- **SLA(가설)**: 1 문서당 평균 50 객체 생성, 10 객체/초, retrieve 응답 ≤ 200ms
...
- **80-20-80 단계적 검수 룰 (v0.3 신규, A5)**

#### 3.6.0 80-20-80 검수 룰 (v0.3 신규)

> v0.3 변경: BeSir 정합성 분석(A5)에 따라, AI 단독 결정 → 사람 일괄 검수가 아닌 **3단계 압축 검수 룰**을 모든 HITL 흐름의 표준으로 채택합니다.

| 단계 | 주체 | 작업 | 도달 수준 |
|---|---|---|---|
| **Stage 1** | AI (Tier 1·2) | 1차 추출·분류 + **자체 평가** (리소스 충실도 + 할루시네이션 가능성) | 80% |
| **Stage 2** | AI (Tier 2·3) | Stage 1의 약점 영역에 **집중 검수**. 신뢰도 낮은 20%만 재검토 | +80% (남은 20%의 80%) |
| **Stage 3** | 사람 (Reviewer) | 최종 사각지대(잔여 ~4%) 검토 + 도메인 판단 | 100% |

**원칙**:
- AI는 처음부터 100%를 못 만든다는 전제
- AI가 자체 평가까지 책임 (Stage 1 끝에 confidence + flag 출력)
- 사람은 마지막 4%만 보면 됨 — 검수 부담 최소화
- Stage 1 confidence가 매우 높은 케이스(예: ≥0.95)는 Stage 2 생략 가능 (Approver 직권)
...
### 3.7.5 MCP Tools Interface (v0.3 신규, A8)

> v0.3 변경: BeSir과의 통합 모델로 **MCP 표준 인터페이스** (Q1 결정 — 느슨한 연동)를 채택. AI Foundry는 외부 시스템(특히 BeSir)에 도구를 노출·호출할 때 MCP를 사용합니다.

#### 3.7.5.1 통합 모델 결정 — MCP

- **Option A (BeSir OEM/임베디드)** — 미채택
- **Option B (BeSir API 외부 통합)** — 미채택
- **Option C (MCP 표준 인터페이스)** — **채택**
- **Option D (양사 공동개발 신제품)** — 미채택

#### 3.7.5.2 MCP 양방향 인터페이스

```
[AI Foundry] ─── MCP Tools ───▶ [BeSir Browser]
[AI Foundry] ◀── MCP Tools ─── [BeSir Knowledge Map]

각 시스템은 자체 자산(온톨로지·스킬·정책팩)을 MCP 도구로 노출.
호출자는 MCP 클라이언트로 자연어 + 자동 라우팅.
```

#### 3.7.5.3 노출되는 MCP Tools (AI Foundry → 외부)

| Tool | 입력 | 출력 |
|---|---|---|
| `af.ontology.search` | 자연어 질의 + domain | 9-Type 객체 + 관계 |
| `af.policy.lookup` | POL-* ID 또는 조건 | Policy Pack JSON |
| `af.skill.invoke` | skill_id + input | 결정 결과 + Decision Log entry |
| `af.diagnostic.findings` | 필터 (severity·type·domain) | 진단 카드 목록 |
| `af.audit.log.query` | 시간 범위 + 필터 | audit log |

#### 3.7.5.4 호출하는 MCP Tools (외부 → AI Foundry)

BeSir이 노출하는 MCP 도구를 AI Foundry가 호출할 수 있도록 클라이언트 구성:
- `besir.knowledge_map.retrieve`
- `besir.cq.evaluate`
- `besir.browser.invoke`

#### 3.7.5.5 BeSir 파트너 도구 차용 (Phase 3+ 옵션)

> Q2 결정 — X1 + 일부 BeSir 도구 활용

| BeSir 도구 | 용도 | AI Foundry 위치 |
|---|---|---|
| **Code Agent** | 기존 시스템 코드에서 암묵지 자동 채굴 | Layer 1 Data + System Knowledge 입력 |
| **System Analyzer** | 외부 시스템 답변 자동 스냅샷 (CQ 정답 확보) | §4.5 CQ 운영 검증 |
| **DB Connect** | DB 연결 후 시스템 맥락 자동 채굴 | Layer 1 Data Connector |

> 차용 결정·라이선스 협상은 **Phase 4 사업 라운드**에서 다룸 (사업 후순위).
...
### 4.6 CQ (Critical Question) 운영 검증 (v0.3 신규, A9)

> v0.3 변경: BeSir 정합성 분석(A9)에 따라 **검증 체계를 두 층위로 분리**합니다 — 4대 진단(§4.1~4.5)이 **빌드 단계** 정책 세트의 정합성을 보고, **§4.6 CQ는 운영 단계** 도구 호출 결과의 정합성을 봅니다. Q4 결정(4대 진단 메인 + CQ 보조).

#### 4.6.1 CQ란 무엇인가

- **CQ(Critical Question)** = 시스템에 던지는 핵심 질문 + 그에 대한 정답지 쌍
- **작성 주체**: 사람 (End User) — **AI 작성 금지**
  - 출처: C-Level + 경영분석실 + 도메인 PM (다층 수집)
  - 이유: 진짜 요구사항(Requirement)은 End User만 안다
- **정답지 확보**: 사람이 시스템 직접 조회로 정답 구함, **특정 시점에 고정**
  - 시점 고정 이유: "지난달 전망한 MOU"처럼 시점에 따라 답이 달라지는 질문 대응

#### 4.6.2 좋은 CQ vs 나쁜 CQ

| 분류 | 예시 |
|---|---|
| 나쁜 CQ | "우리 매출 얼마야?" — 너무 일반적, 도메인 가치 없음 |
| 좋은 CQ | "전망 대비 실적 갭이 큰 사업실은?" — 도메인 용어 + 다단계 추론 필요 |

좋은 CQ의 특징:
- **그들만 사용하는 도메인 용어** 포함
- 단순 1-hop 조회로는 답할 수 없음 (다단계 추론 필요)
- 결과의 정답이 명확히 존재 (감사·내부통제 추적 가능)

#### 4.6.3 5축 100점 벤치마크

운영 시점에 별도 평가 에이전트(문맥 완전 분리)가 100점 만점으로 채점:

| 축 | 평가 내용 | 가중치 (가설) |
|---|---|---|
| 1. 온톨로지 활용도 | 사용자 질문의 의미·맥락을 온톨로지로 잘 이해했는가 | 25 |
| 2. 툴 선택 | 적합한 툴(API)을 선택했는가 | 20 |
| 3. 코드 품질 | 그 툴을 가공하는 코드를 잘 만들었는가 | 15 |
| 4. 결과값 일치 | 결과가 사용자가 말한 정답과 일치하는가 | 30 |
| 5. 거버넌스(권한) | 권한 없는 정보를 답하지 않았는가 | 10 |

#### 4.6.4 90점 핸드오프 룰

- **평균 ≥ 90점** → 핸드오프(자동화 운영 진입)
- **평균 < 90점** → 분석:
  - (a) 휴먼 에러 (정답 잘못 구함, 날짜 오선택 등) — 정답지 갱신
  - (b) 인프라 이슈 (DB 조회 시간 과다 등) — 에이전트 책임 아님
  - (c) 모델·온톨로지 결함 — 정책팩·온톨로지 보강 신호

#### 4.6.5 4대 진단 vs CQ — 두 층위의 차이

| 차원 | 4대 진단 (§4.1~4.5) | CQ 5축 (§4.6) |
|---|---|---|
| 단계 | **빌드** — 정책 세트 자체의 정합성 | **운영** — 도구 호출 결과의 정합성 |
| 입력 | Policy Pack 묶음 + 과거 케이스 분포 | 운영 중 LLM 호출 + 응답 |
| 출력 | Missing/Duplicate/Overspec/Inconsistency 카드 | 5축 점수 + 핸드오프 결정 |
| 빈도 | 주기적·이벤트 기반 (백그라운드) | 호출 단위 실시간 + 표본 검사 |
| 책임 | 정책팩 작성자·검수자 | 운영 에이전트·평가 에이전트 |

→ 두 검증은 **서로 보완**. 정책팩은 빌드 단계에서 깨끗해야 하고, 그 정책팩을 사용하는 운영 호출은 다시 5축으로 측정.
...
### 8.2 Layer 2 — Ontology Layer 스펙 (v0.3 갱신)

> v0.3 변경: Graph DB → 파일(Knowledge Map) + Git + PostgreSQL 아키텍처로 전면 갱신 (§3.4 참조).

#### 8.2.1 책임 범위
시멘틱 레이어 관리, 9 타입 객체 매칭, 추론 룰 실행, Knowledge Map export/import. **현행만 보유**.
...
#### 8.2.4 내부 모델 (v0.3 갱신)
- **Knowledge Map Store**: 파일 시스템 + Git 버전관리 (메타 객체)
- **Postgres Adapter**: PostgreSQL (인스턴스·관계·가중치)
- **Retriever**: LLM 자연어 retrieve (파일 기반 탐색, Cypher 미사용)
- Linker: 임베딩 + 룰 기반 entity resolution
- Reasoner: 추론 룰 (SPARQL/Cypher 아닌 자연어 룰 + 파일 패턴)
- Schema Manager: **9 타입** (Domain·Fact·Dimension·Workflow·Event·Actor·Policy·Support·Decision) + 사용자 정의 확장

#### 8.2.5 외부 의존성 (v0.3 갱신)
- **Git 저장소** (파일 메타 객체)
- **PostgreSQL** (인스턴스 데이터)
- **Redis** (캐시)
- 임베딩 모델 (Layer 1 공유)
...
#### 8.2.6 SLA 가설
- 1 문서당 평균 50 객체 생성, 10 객체/초
- Retrieve 응답: ≤ 200ms (파일 기반)
- 가용성: 99.0%
- 그래프 쿼리 (단순): 200ms 이내
- 가용성: 99.0%
...
| ~~그래프 DB~~ | ~~관리형 서비스~~ — **v0.3에서 미사용 결정 (§3.4.1)** |
| **Knowledge Map (파일)** | Git 저장소 (사내 또는 GitHub Enterprise) |
| **PostgreSQL** | 관리형 서비스 (RDS/Cloud SQL 또는 사내) — 인스턴스·로그 |
| **Redis** | 관리형 서비스 — 캐시 |
| **고객사 원본 DB Connect** | 자동화 정책(P6) — 직결, 복제 X |
...
| **R-16** | **BeSir 정합성 미달성으로 5월 W19 미팅 실패** (v0.3 신규) | 거버넌스 | High | Med | 본 06 분석 문서 + v0.3 본문이 5월 W18까지 BeSir·서민원에 사전 회람 / Sinclair |
...
| LLM 공급자 1·2차 선정 | 5월 W20 | LLM 코어 + Sinclair | Phase 1 |
| **PostgreSQL + Git 저장소 인프라 결정** (v0.3, Graph DB 대체) | 5월 W20 | Ontology 코어 + 인프라 | Phase 1 |
| 가상 도메인 1·2 데이터 사양 | 5월 W19 | Sinclair + 서민원 | Phase 1 |
| KT 사업본부 카운터파트 지정 | 5월 W22 이전 | 본부장 | Phase 1→2 |
| **BeSir 정합성 v0.3 합의** (v0.3 신규) | 5월 W18 | KT DS AX컨설팅팀 + 임원 | Phase 1 |
| **첫 실제 도메인 1개 확정** | 7월 W29 (G4) | 임원 + Sinclair + 본부 | Phase 3 진입 |
| 도메인 데이터 NDA + 접근 권한 | 7월 W28 | 법무 + 본부 | Phase 3 진입 |
| **BeSir과 MCP 통합 인터페이스 표준화** (v0.3 신규) | 5월 W19 미팅 후 | Sinclair + BeSir | Phase 1→2 |
| BeSir 파트너 도구 차용 결정 | Phase 4 | 사업 라운드 | Phase 4 |
| 가격·라이선스 모델 | 9월~ | 법무 + 임원 | Phase 4 진입 |
| 산업별 표준 인증 | Phase 4 | Phase 4 | Phase 4 |

### 11.3 다음 1주 액션 (Sinclair)

- [ ] 본 정의서 v0.3을 서민원·임원·코어 후보에 공유 + W18 회의 세팅
- [ ] **06 분석 문서 + v0.3 본문을 BeSir에 사전 회람** (NDA 후, W18 화)
- [ ] 5 모듈 코어 후보 명단 + 임원 보고 1쪽
- [ ] 가상 도메인 1·2 데이터/시나리오 초안
- [ ] **PostgreSQL + Git 저장소 인프라 후보 비교** (v0.3 변경, Graph DB → 파일 기반)
- [ ] 본 문서 → 마크다운 회람 (한글 인코딩 안전)
- [ ] **5월 W19 BeSir 차기 미팅 일정 조율**

### 11.4 사용자 답변 필요 질문 (Counterquestions)

| Q | 질문 | 결정 시점 | 담당 |
|---|---|---|---|
| Q1 | 6월 Prototype을 4주 안에 못 만든다는 신호 시 범위 축소 정책? | W18 합의 회의 | Sinclair + 모듈 코어 |
| Q2 | 첫 외부 고객사에 보여줄 "압도적으로 명확한 하나의 가치"? | 7월 W29 G4 | Sinclair + 본부 카운터파트 |
| Q3 | core_differentiator 보호 vs 플랫폼 학습 향상의 경계? | 9월 이후 (Phase 4) | Sinclair + 법무 + 임원 |
| **Q4 (v0.3 신규 — Q1~Q3 답변)** | BeSir 통합 옵션 (A/B/C/D)? | **2026-04-30 인터뷰로 결정 완료** | **C. MCP** |
| **Q5 (v0.3 신규)** | AI Foundry 위상 (X1/X2/X3)? | **결정 완료** | **X1 + 일부 BeSir 도구** |

> Q1·Q2·Q3은 v0.2 시점의 답변 가설 유지. Q4·Q5는 2026-04-30 인터뷰로 즉시 결정 완료.

---

## 12. 부록

### 12.1 부록 A — 용어 사전 (v0.3 갱신)

| 용어 | 정의 |
|---|---|
| AI Foundry | 본 시스템 |
| Decision Case | AI Foundry가 처리하는 한 건의 의사결정 단위 (§3.2.1) |
| Policy Pack | 의사결정 룰의 코드화 자산. POL-{DOMAIN}-{TYPE}-{SEQ} |
| Ontology | **사람을 복제하는 시멘틱 레이어** (v0.3 재정의, §2.2.2) |
| Skill Package | 의사결정 작업의 실행 단위. `.skill.json` |
| Decision Log | 모든 결정의 영구 기록. append-only |
| **System Knowledge (v0.3 신규)** | RDB·표준 정책에 매핑되지 않는 도메인 암묵지 (§2.2.5) |
| HITL | Human-in-the-Loop |
| 5-Layer | Data → Ontology → LLM → Workflow → Agent |
| **9 Type Objects (v0.3 신규)** | Domain · Fact · Dimension · Workflow · Event · Actor · Policy · Support · Decision (§3.4) |
| **Knowledge Map (v0.3 신규)** | 온톨로지 메타 객체의 파일 저장소 (Git 버전관리) |
| Tier 1/2/3 | LLM 모델 비용·정확도 등급 |
| 4대 진단 | Missing / Duplicate / Overspec / Inconsistency (빌드 단계) |
| **CQ (Critical Question, v0.3 신규)** | 시스템에 던지는 핵심 질문+정답지 쌍, End User 작성, 5축 100점 (§4.6) |
| **5축 평가 (v0.3 신규)** | 온톨로지·툴·코드·결과·거버넌스 (§4.6.3) |
| **90점 핸드오프 (v0.3 신규)** | CQ 5축 평균 ≥90점이면 자동화 진입 |
| **80-20-80 검수 (v0.3 신규)** | AI 1차 80% → AI 2차 (남은 20%의 80%) → 사람 최종 (§3.6.0) |
| 5-Asset Model | Policy Pack · Ontology · Skill Package · Decision Log · **System Knowledge (v0.3 신규)** |
| Cross-Org | 다조직 정책의 4그룹 분류 |
| 5 RBAC | Reviewer · Approver · Auditor · Domain Expert · Admin |
| Honeycomb Network | KT·고객·파트너 양방향 순환 자산 공유 |
| Multi-Evidence Triangulation | E1+E2+E3 가중 신뢰도 (§3.5.1) |
| **MCP Tools Interface (v0.3 신규)** | AI Foundry ↔ 외부 시스템 (BeSir 등) MCP 표준 인터페이스 (§3.7.5) |
| DoD₁/₂/₃ | Phase별 종료 기준 |
| G1+G2 / G3 / G4 / G5 | Phase 게이트 |

### 12.2 부록 B — 가상 도메인 상세 (v0.2와 동일)

#### 12.2.1 가상 도메인 1: "Grants Review" (보조금 심사)
- 가상 공공기관, 매년 1만 건 보조금 신청 심사
- 신청서 PDF 1,000건 + 정책 50개 + 5만 행 과거 결과
- 예상: 9-Type 객체 약 3,000개, Policy 280개, 진단 critical 12

#### 12.2.2 가상 도메인 2: "Ops Decisions" (운영 의사결정)
- 가상 인프라 운영, 24/7, 일 알람 2,000건, runbook 80개
- 예상: 9-Type 객체 5,000개, Policy 720→502, 자동/반자동/수동 60/25/15

### 12.3 부록 C — 인터페이스 카탈로그 (v0.3 갱신)
...
| `ontology.object.added` (v0.3) | Layer 2 | Layer 3 | object_id, type (9-Type 중 하나), domain, confidence |
| `policy.triple.extracted` | Layer 3 | Layer 4 | pol_id, triple, confidence (E1·E2·E3 통합), cost |
...
| **`cq.evaluated` (v0.3 신규)** | Layer 5 + Eval Agent | Browser Console | cq_id, 5축 점수, 핸드오프 여부 |
...
#### 12.3.2 핵심 API + MCP Tools (v0.3 갱신)

| API/Tool | 메서드 | 설명 |
|---|---|---|
| `/v1/data/ingest` | POST | 문서 업로드 |
| **`/v1/knowledge-map/retrieve` (v0.3)** | POST | 자연어 retrieve (Cypher 대체) |
| `/v1/policy/{id}` | GET | 정책팩 조회 |
| `/v1/diagnostic/findings` | GET | 진단 카드 |
| **`/v1/cq/evaluate` (v0.3)** | POST | CQ 5축 평가 실행 |
| `/v1/skill/{id}/invoke` | POST | 스킬 호출 |
| `/v1/audit/log` | GET | audit log 조회 |
| `/v1/case/{id}` | GET | Decision Case 단건 |
| `/v1/case/{id}/timeline` | GET | Decision Case의 5-Layer 통과 timeline |
| **MCP `af.ontology.search` (v0.3)** | MCP | 외부에서 호출 가능 |
| **MCP `af.policy.lookup` (v0.3)** | MCP | 외부에서 호출 가능 |
| **MCP `af.skill.invoke` (v0.3)** | MCP | 외부에서 호출 가능 |
| **MCP `af.diagnostic.findings` (v0.3)** | MCP | 외부에서 호출 가능 |
| **MCP `af.audit.log.query` (v0.3)** | MCP | 외부에서 호출 가능 (read-only) |

#### 12.3.3 핵심 데이터 스키마
- Decision Case (§3.2.1)
- Document (Layer 1 출력) (§8.1.3)
- Policy Pack (Layer 4 산출) (§2.2.1)
- Decision Log Entry (Layer 5) (§2.2.4)
- **System Knowledge Entry (v0.3 신규)** (§2.2.5)
- Diagnostic Finding (§4.2)
- **CQ Evaluation Result (v0.3 신규)** — 5축 점수 + 핸드오프 결정 (§4.6)
- **9-Type Object (v0.3 신규)** — Knowledge Map 단위 (§3.4)

### 12.4 부록 D — Phase 2~4 미리보기

#### Phase 2 (6월) — Prototype
1. 5-Layer 통합 빌드 (가상 도메인 1개 E2E)
2. 시그니처 동작 (4대 진단 + Cross-Org + **CQ 5축 검증 가상 케이스**)
3. **MCP Tools Interface 가동** (외부 호출 가능 도구 5개)
4. 시연 가능 (30분 시나리오)

#### Phase 3 (7~8월) — 실제 사업 적용
1. 첫 실제 도메인 1개 인스턴스화
2. HITL 운영 시작 (80-20-80 룰 적용)
3. CQ 5축 평가 운영 진입 + 90점 핸드오프
4. **Knowledge Map + PostgreSQL** 운영 안정화
5. GTM 1차 패키지

#### Phase 3+ — BeSir 파트너 도구 차용 옵션 (v0.3 신규, A10)

| BeSir 도구 | 용도 | AI Foundry 흡수 위치 |
|---|---|---|
| Code Agent | 기존 시스템 코드에서 암묵지 자동 채굴 | Layer 1 + System Knowledge |
| System Analyzer | 외부 시스템 답변 자동 스냅샷 (CQ 정답 확보) | §4.6 CQ 운영 검증 |
| DB Connect | DB 연결 후 시스템 맥락 자동 채굴 | Layer 1 Connector |

차용 결정·라이선스 협상은 **Phase 4 사업 라운드에서** (사업 후순위).

#### Phase 4 (9월~) 가설 — 본 정의서 범위 밖
1. 자산 재사용 가속화 입증 (두 번째 도메인)
2. **Multi-Agent Orchestration** (BeSir Agent-to-Agent와 MCP 호환)
3. **Agent Brain 가설 도입** (BeSir 12월 출시 동기화)
4. 다도메인 동시 운영 (3~5개)
5. 외부 마켓플레이스
6. 산업별 표준 인증
7. 가격·라이선스 풀 시스템
8. **전용 하드웨어 모델 검토** (BeSir 동향 + KT cloud 연계)
9. 다국어·다지역 운영
...
| 후속 산출물 | 시점 | 인용 위치 |
|---|---|---|
| Phase 2 모듈 구현 코드 | 6월 | §8 모듈 스펙 + §12.3 인터페이스 |
| **5월 W19 BeSir 미팅 자료 (v0.3)** | 5월 W19 | 본 정의서 v0.3 + 06 분석 문서 |
| Phase 3 도메인별 사업기획안 | 7~8월 | §0.1 + §2 정의 + §3 5-Layer + §4·5 시그니처 |
| Phase 3 KT 본부 협업 제안서 | 7월 | §0.3 허니콤 + §5.4 표준화 금지 약속 |
| Phase 4 가격·라이선스 모델 | 9월~ | §2.2 5-Asset + §5 4그룹 |
...
**근거 문서**: 06_architecture_alignment_with_besir_v1.md

**통합 모델 결정 (Q4·Q5 인터뷰)**:
- Q4. 통합 방향 = **C. MCP 표준 인터페이스 (느슨한 연동)**
- Q5. AI Foundry 위상 = **X1 자체 엔진 + BeSir 도구 부분 차용**
- 노드 모델 = **BeSir 7타입 + Domain + Decision = 9타입**
- 검증 체계 = **4대 진단(빌드) + CQ(운영) 이층**

**P0 반영 (10건)**:
- A1 §2.2.2 — 온톨로지 = 시멘틱 레이어(사람 복제) 재정의
- A2 §3.4 — 12노드 → 9타입 (Dimension 신규)
- A3 §2.2.5 — System Knowledge 5번째 자산
- A4 §2.4 (P6+P7) — 자동화 정책 + 원본 DB 직결
- A5 §3.6.0 — HITL 80-20-80 검수 룰
- **A6 §3.4 / §3.4.1 / §3.4.2 / §8.2** — Graph DB 미사용 → 파일+Git+PostgreSQL
- A7 §10.5 — 인프라 스택 갱신
- **A8 §3.7.5** — MCP Tools Interface 신규
- **A9 §4.6** — CQ 5축 운영 검증 신규
- A10 §12.4 — BeSir 파트너 도구 차용 옵션

**Down-prioritize (사업 후순위)**:
- §0.4.1 / §1.3.1 / §5.4·§5.5 / §11.4 Q2·Q3 / §12.4 Phase 3·4 GTM — 모두 Phase 4 안건으로 이연

**리스크 신규**:
- R-16 BeSir 정합성 미달성 시 5월 W19 미팅 실패
...
ChatGPT·Gemini·DeepSeek 통합 검증 결과(04 문서) 반영. P0 5건(Decision Case·R-13·R-14·R-15·Prototype 범위 축소 fallback) + P1 5건(Multi-Evidence·허니콤 양방향·진짜 경쟁자·외부 한 줄 가치·core_differentiator 경계).

### v0.1 (2026-04-29) — 초안

AI Foundry 정의서 초안 (시스템 정의 + 5-Layer + 4대 진단 + Cross-Org + 가상 시나리오 + 5~8월 로드맵). Decision Foundry 시기의 v0.1 deprecated 후 신규 시리즈 시작.

---

## 끝맺음 — 본 문서의 위상

이 문서는 **AI Foundry 정의서 v0.3**입니다 (BeSir 정합성 P0 10건 반영판, 5월 W19 BeSir 차기 미팅 사전판).

v1.0이 아니라 v0.3로 둔 것은:

v1.0이 아닌 v0.3 사유 (요약):
1. 5 모듈 코어 미지정 (W18 결정 후 v1.0)
2. 가상 도메인 1·2 사양 미작성
3. 5월 W19 BeSir 미팅 결과 미반영
4. Phase 2 측정 결과 미통합 (6월 W26 별도)
5. Phase 3 첫 실제 도메인 미확정 (7월 W29)
6. Counterquestions Q1·Q2·Q3 미해소 (Q4·Q5는 인터뷰로 완료)

본 문서는 Phase 3 1차 마감(8월 W34)까지 living document.

**Phase 2~4의 모든 후속 산출물이 본 정의서를 SSOT로 인용합니다.** 본 문서가 흔들리면 Phase 2~4가 흔들립니다.
