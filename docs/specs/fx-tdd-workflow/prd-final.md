...
> **버전**: prd-final (3-AI Round 1 피드백 반영, 2026-04-12)
...
## 14. 리스크 및 보완 사항 (3-AI 검토 반영)

> Round 1 검토 결과: ChatGPT Conditional / Gemini Ready / DeepSeek Conditional
> 아래 6건은 3-AI 공통 지적 사항을 반영한 보완 섹션이다.

### 14-1. E2E TDD Red Phase의 현실적 한계

**지적 (DeepSeek)**: Playwright spec의 `page.route()` mock 설정 자체가 구현 로직을 포함하므로 "구현 zero" 원칙 위반.

**보완 방침**:
- E2E의 Red Phase는 **"라우트/컴포넌트 구현 zero"**로 범위를 재정의한다
- `page.route()` mock 설정은 "테스트 인프라"로 간주하여 Red Phase에서 허용
- 다만 실제 React 컴포넌트/라우트 파일은 Red 시점에 존재하면 안 됨
- 적용 등급 §6에서 E2E는 "필수"에서 **"권장"**으로 하향 조정 검토 (Pilot 결과에 따라 결정)

### 14-2. 자동화 Fail-safe

**지적 (ChatGPT, DeepSeek)**: 커밋 순서 위반을 사람의 주의력에만 의존. 자동 감지 메커니즘 부재.

**보완 방침**:
- **Phase 1 (즉시)**: `.claude/rules/tdd-workflow.md` 규칙으로 Claude Code가 Red→Green 순서를 자율 준수
- **Phase 2 (Pilot 이후 검토)**: `git log --oneline` 기반 커밋 메시지 패턴 검증 스크립트 검토
  - `test(scope): F### red` 뒤에 `feat(scope): F### green`이 오는지 확인
  - PR description에 Red/Green 커밋 해시 자동 기록
- **Phase 3 (효과 확인 후)**: PostToolUse hook 또는 PR bot으로 자동 경고
- **설계 원칙**: 혼자 개발 모드에서는 Claude Code 규칙이 사실상 Fail-safe. 팀 확장 시 자동화 우선순위 상향

### 14-3. 성과 측정 KPI

**지적 (ChatGPT, Gemini)**: "생산성 저하 없음"의 기준이 모호. 정량 지표 필요.

**측정 지표** (Phase 4 회고에서 수집):

| 지표 | 측정 방법 | 기준선 (TDD 전) | 목표 |
|------|----------|----------------|------|
| Red/Green 커밋 분리율 | `git log --grep "red\|green"` 패턴 | 0% | ≥ 80% (필수 영역) |
| T-NN 계약 이행률 | Analyze 단계에서 산출 | N/A | ≥ 90% |
| Sprint 소화량 (F-item/Sprint) | SPEC.md 실적 | 최근 3 Sprint 평균 | ±20% 이내 |
| PR cycle time | `gh pr list --json createdAt,mergedAt` | 최근 10 PR 평균 | 150% 이내 |
| 회귀 버그 건수 | PR merge 후 1주 내 fix 커밋 수 | 최근 5 Sprint 평균 | 감소 추세 |

**판정**: Sprint 소화량 20% 초과 저하 시 적용 등급 하향 검토.

### 14-4. 예외 및 롤백 정책

**지적 (ChatGPT, DeepSeek)**: hotfix, legacy 변경, 도입 실패 시 복귀 플랜 없음.
...
| **Legacy 대규모 리팩토링** | 선택 | 기존 코드 Red 작성이 비효율적. 회귀 테스트로 대체 |
| **D1 migration only** | 면제 | SQL DDL은 TDD 대상 아님 (기존 §6 면제와 동일) |
| **1-line 버그픽스** | 면제 | 오버헤드 > 이득. 회귀 테스트 1건만 추가 |

**롤백 플랜**: 3 Sprint 후 회고(Phase 4)에서 KPI 미달이 지속되면:
1. 적용 등급 전체 1단계 하향 (필수→권장, 권장→선택)
2. 추가 2 Sprint 관찰 후에도 미달이면 TDD 강제를 해제하고 "권장 사항"으로 전환
3. `.claude/rules/tdd-workflow.md` 파일은 유지하되 적용 범위를 축소

### 14-5. 규칙 SSOT 정리

**지적 (DeepSeek)**: 5개 파일(tdd-workflow.md, testing.md, sdd-triangle.md, git-workflow.md, /tdd 스킬)에 TDD 관련 규칙이 분산.

**SSOT 원칙**:
- **tdd-workflow.md** = TDD 규칙의 SSOT (상세 절차, 적용 등급, 커밋 컨벤션)
- **testing.md** = TDD 참조 포인터만 (3줄 cross-reference, 상세는 tdd-workflow.md로 위임)
- **sdd-triangle.md** = TDD 순서 원칙만 (2줄, "Test가 Code보다 먼저")
- **git-workflow.md** = 변경 없음 (Red/Green 커밋은 기존 feature 브랜치 → squash merge 흐름 그대로)
- **/tdd 스킬** = 실행 도구 (규칙은 tdd-workflow.md 참조, 스킬은 오케스트레이션만 담당)

규칙 중복 시 **tdd-workflow.md가 우선**.

### 14-6. shared 패키지 테스트 부재

**지적 (DeepSeek)**: shared 패키지 테스트 0개가 치명적 리스크. 타입 의존만으로 런타임 오류 방지 불가.

**방침**:
- shared는 순수 타입 정의(`types/`, `web/`, `agent/` 등)로 구성. 런타임 로직이 없어 §6 "면제" 등급 유지
- 단, 향후 shared에 런타임 유틸리티(validator, formatter 등)가 추가되면 해당 함수에 한해 TDD "필수" 적용
- TypeScript strict mode + 소비자 패키지(api, cli, web)의 테스트가 간접 검증 역할 수행 중
- **즉시 조치 불필요** — 현재 shared에 런타임 로직이 없으므로 TDD 도입과 무관
...
- 기존 테스트의 TDD 소급 적용 (514개 기존 테스트는 현행 유지)
- 테스트 커버리지 100% 달성 (TDD는 커버리지 도구가 아님)
- shared 패키지 테스트 프레임워크 구축 (별도 C-track 후보)
- AI 코딩 어시스턴트 벤더 전환 (Claude Code 전용 워크플로우)
- 팀 온보딩/교육 커리큘럼 (현재 1인 개발 모드, 팀 확장 시 별도 수립)
- pre-commit hook / PR bot 자동화 (Pilot 결과 후 Phase 2에서 검토)

---

## 16. 3-AI 검토 요약 (Round 1, 2026-04-12)

| AI | 판정 | 핵심 피드백 |
|----|------|-----------|
| ChatGPT | Conditional | 변화 관리/교육, 자동화 Fail-safe, 성과 KPI, 예외 정책 보완 필요 |
| Gemini | Ready | 시장 적합성 높음, AI+TDD 트렌드 정확. 비용/보안/윤리 장기 모니터링 권장 |
| DeepSeek | Conditional | E2E Red Phase 원칙 위반, shared 테스트 부재, 규칙 SSOT 위반, CI 충돌 가능성 |

**반영 결과**: §14에 6건 보완, §15 Out-of-scope 신규 추가. Conditional 조건의 핵심 사항(KPI, 예외 정책, E2E 한계, SSOT)은 모두 반영 완료.
