...
- [x] F398: 이벤트 카탈로그 8종 스키마 + EventBus(D1 Event Table + Cron Trigger) PoC
- [x] F399: Strangler Fig 프록시 레이어 + harness-kit 이벤트 유틸리티
- [x] F400: E2E 서비스별 태깅 + 전체 회귀 테스트 + Gate-X scaffold PoC
...
| Sprint | F-items | 작업 | 산출물 |
|--------|---------|------|--------|
| 185 | F398 | 이벤트 카탈로그 8종 + EventBus PoC | `packages/shared/events/` |
| 186 | F399 | Strangler Fig 프록시 + harness-kit 이벤트 | 프록시 미들웨어 |
| 187 | F400 | E2E 태깅 + 회귀 테스트 + Gate-X PoC | PoC 서비스 동작 확인 |
