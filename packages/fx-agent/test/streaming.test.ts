...
import { streamingRoute } from "../src/routes/streaming.js";
import { AgentStreamHandler } from "../src/streaming/agent-stream-handler.js";
import { createTestEnv } from "./helpers/test-app.js";
import type { AgentStreamEvent } from "@foundry-x/shared";

// ─── AgentMetricsService mock ───
vi.mock("../src/streaming/agent-metrics-service.js", () => ({
...
describe("F532 — AgentStreamHandler 이벤트 순서", () => {
  it("test 5: createHooks → run_started → run_completed 순서 보장", async () => {
    const { AgentMetricsService } = await import("../src/streaming/agent-metrics-service.js");
