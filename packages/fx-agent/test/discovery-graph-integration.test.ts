...
const MOCK_AI_RESULT = { summary: "테스트 요약", details: "테스트 상세", confidence: 85 };
...
describe("F531: createDiscoveryGraph — 실제 핸들러 연동", () => {
  ...
  it("test 1: createDiscoveryGraph(runner, db) - stage-2-1 노드 실행 시 LLM runner.execute() 호출됨", async () => {
    const { createDiscoveryGraph } = await import(
      "../src/orchestration/graphs/discovery-graph.js"
    );

    const graph = createDiscoveryGraph(runner, db as unknown as D1Database);

    const { GraphEngine } = await import("../src/orchestration/graphs/graph-engine.js");
    ...
    await engine.run(input, "session-1", "test-api-key", db as unknown as D1Database);
  ...
  it("test 2: createDiscoveryGraph(runner, db) - coordinator 노드 실행 후 stage-2-0 실행됨", async () => {
    ...
    const input = { bizItemId: "biz1", orgId: "org1", discoveryType: "I" };
    const result = await engine.run(input, "session-1", "test-api-key", db as unknown as D1Database);
