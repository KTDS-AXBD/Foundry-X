...
bizItemsRoute.get("/biz-items/:id/shaping-artifacts", async (c) => {
  const orgId = c.get("orgId");
...
bizItemsRoute.post("/biz-items/:id/starting-point", async (c) => {
  ...
  try {
    ...
    // F479: discovery_stages 2-0 완료 동기화
    try {
      const stageSvc = new DiscoveryStageService(c.env.DB);
